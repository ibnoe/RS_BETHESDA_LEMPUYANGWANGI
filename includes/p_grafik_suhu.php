...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_grafik_suhu&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_grafik_suhu.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_grafik_suhu.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
	...
	if ($rec1 > 0) {
        ...
        echo "<form name='Form10' action='actions/p_grafik_suhu.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT SUHU, NADI, PERNAFASAN PASIEN</div>";
		//detail riwayat
		...
		//$f = new Form($SC, "GET");
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","PERNAFASAN (/MENIT)","NADI (/MENIT)","SUHU (&deg;C)","DETAIL");
			   	$t->ColAlign = array("center","center","center","left","left","left","","center");
...
//====iqbal tambah tab konsultasi untuk rawat inap 15/02/12====
    	...
    	$sql2 =	"SELECT A.* FROM C_VISIT_RI A 
    				WHERE A.ID_RI='{$_GET["ri"]}' AND A.NO_REG='{$_GET["rg1"]}'AND A.TANGGAL_REG='{$_GET["tmp26"]}' ";	
    	}else{	
    	$sql2 =	"SELECT A.* FROM C_VISIT_RI A 
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_grafik_suhu.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
						$f->hidden("f_no_reg",$d2["no_reg"]);
					    $f->hidden("f_tanggal_reg",$_GET["tmp26"]);
			...
			$f->textarea("f_vis_1",$visit_ri_grafik_suhu["vis_1"],1,50,$_GET["tmp0"],$ext);
			$f->calendar("f_vis_2","Tanggal",10,15,$_GET["tmp1"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			if($d2["vis_3"] != ''){
			$f->textinfo("f_vis_3",$visit_ri_grafik_suhu["vis_3"],10,10,$_GET["tmp2"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_3",$visit_ri_grafik_suhu["vis_3"],10,10,"08:00","(Jam:Menit, 08:09)",$ext);	
			}		
			$f->textinfo("f_vis_4",$visit_ri_grafik_suhu["vis_4"],10,10,$_GET["tmp3"],"/Menit",$ext);
			$f->textinfo("f_vis_5",$visit_ri_grafik_suhu["vis_5"],10,10,$_GET["tmp4"],"/Menit",$ext);
			$f->textinfo("f_vis_6",$visit_ri_grafik_suhu["vis_6"],10,10,$_GET["tmp5"],"&deg;C",$ext);
			$f->text("f_vis_7",$visit_ri_grafik_suhu["vis_7"],30,50,$_GET["tmp6"],$ext);
			$f->text("f_vis_8",$visit_ri_grafik_suhu["vis_8"],30,50,$_GET["tmp7"],$ext);
			$f->text("f_vis_9",$visit_ri_grafik_suhu["vis_9"],30,50,$_GET["tmp8"],$ext);
			$f->text("f_vis_10",$visit_ri_grafik_suhu["vis_10"],30,50,$_GET["tmp9"],$ext);
			$f->text("f_vis_11",$visit_ri_grafik_suhu["vis_11"],30,50,$_GET["tmp10"],$ext);
			$f->text("f_vis_12",$visit_ri_grafik_suhu["vis_12"],30,50,$_GET["tmp11"],$ext);
			$f->text("f_vis_13",$visit_ri_grafik_suhu["vis_13"],30,50,$_GET["tmp12"],$ext);
			$f->text("f_vis_14",$visit_ri_grafik_suhu["vis_14"],30,50,$_GET["tmp13"],$ext);
			$f->text("f_vis_15",$visit_ri_grafik_suhu["vis_15"],30,50,$_GET["tmp14"],$ext);
			$f->text("f_vis_16",$visit_ri_grafik_suhu["vis_16"],30,50,$_GET["tmp15"],$ext);
			$f->text("f_vis_17",$visit_ri_grafik_suhu["vis_17"],30,50,$_GET["tmp16"],$ext);
			$f->text("f_vis_18",$visit_ri_grafik_suhu["vis_18"],30,50,$_GET["tmp17"],$ext);
			$f->text("f_vis_19",$visit_ri_grafik_suhu["vis_19"],30,50,$_GET["tmp18"],$ext);
			$f->text("f_vis_20",$visit_ri_grafik_suhu["vis_20"],30,50,$_GET["tmp19"],$ext);
			$f->text("f_vis_21",$visit_ri_grafik_suhu["vis_21"],30,50,$_GET["tmp20"],$ext);
			$f->text("f_vis_22",$visit_ri_grafik_suhu["vis_22"],30,50,$_GET["tmp21"],$ext);
			$f->text("f_vis_23",$visit_ri_grafik_suhu["vis_23"],30,50,$_GET["tmp22"],$ext);
			$f->text("f_vis_24",$visit_ri_grafik_suhu["vis_24"],30,50,$_GET["tmp23"],$ext);
			$f->textinfo("f_vis_25",$visit_ri_grafik_suhu["vis_25"],10,10,$_GET["tmp24"],"Kg",$ext);
			$f->textarea("f_vis_26",$visit_ri_grafik_suhu["vis_26"],1,50,$_GET["tmp25"],$ext);
			...
			echo"</div>";
			echo "<div align=right><b>RM.01</b></div>";
			...
			echo "<table border=1 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'>";
			$f->textarea("f_vis_1",$visit_ri_grafik_suhu["vis_1"],1,50,$d2["vis_1"],$ext);
			$f->calendar("f_vis_2","Tanggal",10,15,$d2["vis_2"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$f->textinfo("f_vis_3",$visit_ri_grafik_suhu["vis_3"],10,10,"12:00","(Jam:Menit, 08:09)",$ext);	
			$f->textinfo("f_vis_4",$visit_ri_grafik_suhu["vis_4"],10,10,"","/Menit",$ext);
			$f->textinfo("f_vis_5",$visit_ri_grafik_suhu["vis_5"],10,10,"","/Menit",$ext);
			$f->textinfo("f_vis_6",$visit_ri_grafik_suhu["vis_6"],10,10,"","&deg;C",$ext);
			$f->text("f_vis_7",$visit_ri_grafik_suhu["vis_7"],30,50,"",$ext);
			$f->text("f_vis_8",$visit_ri_grafik_suhu["vis_8"],30,50,"",$ext);
			$f->text("f_vis_9",$visit_ri_grafik_suhu["vis_9"],30,50,"",$ext);
			$f->text("f_vis_10",$visit_ri_grafik_suhu["vis_10"],30,50,"",$ext);
			$f->text("f_vis_11",$visit_ri_grafik_suhu["vis_11"],30,50,"",$ext);
			$f->text("f_vis_12",$visit_ri_grafik_suhu["vis_12"],30,50,"",$ext);
			$f->text("f_vis_13",$visit_ri_grafik_suhu["vis_13"],30,50,"",$ext);
			$f->text("f_vis_14",$visit_ri_grafik_suhu["vis_14"],30,50,"",$ext);
			$f->text("f_vis_15",$visit_ri_grafik_suhu["vis_15"],30,50,"",$ext);
			$f->text("f_vis_16",$visit_ri_grafik_suhu["vis_16"],30,50,"",$ext);
			$f->text("f_vis_17",$visit_ri_grafik_suhu["vis_17"],30,50,"",$ext);
			$f->text("f_vis_18",$visit_ri_grafik_suhu["vis_18"],30,50,"",$ext);
			$f->text("f_vis_19",$visit_ri_grafik_suhu["vis_19"],30,50,"",$ext);
			$f->text("f_vis_20",$visit_ri_grafik_suhu["vis_20"],30,50,"",$ext);
			$f->text("f_vis_21",$visit_ri_grafik_suhu["vis_21"],30,50,"",$ext);
			$f->text("f_vis_22",$visit_ri_grafik_suhu["vis_22"],30,50,"",$ext);
			$f->text("f_vis_23",$visit_ri_grafik_suhu["vis_23"],30,50,"",$ext);
			$f->text("f_vis_24",$visit_ri_grafik_suhu["vis_24"],30,50,"",$ext);
			$f->textinfo("f_vis_25",$visit_ri_grafik_suhu["vis_25"],10,10,"","Kg",$ext);
			$f->textarea("f_vis_26",$visit_ri_grafik_suhu["vis_26"],1,50,"",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			...
			echo "<div align=right><b>RM.01</b></div>";
				...
				//---------------------------------------------------------------------------------			
					...
					$sql = "SELECT A.NO_REG,A.VIS_1,A.VIS_2,A.VIS_3,A.VIS_4,A.VIS_5,A.VIS_6,A.VIS_7,A.VIS_8,A.VIS_9,A.VIS_10,A.VIS_11, ".
						   "A.VIS_12,A.VIS_13,A.VIS_14,A.VIS_15,A.VIS_16,A.VIS_17,A.VIS_18,A.VIS_19,A.VIS_20,A.VIS_21,A.VIS_22,A.VIS_23,A.VIS_24,A.VIS_25,A.VIS_26,A.tanggal_reg,'DUMMY'". 
					       "FROM C_VISIT_RI A ".
					       "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					       "WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' ";
			    ...
			    $t->ColHidden[26]=true;$t->ColHidden[27]=true;$t->ColHidden[28]=true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","","JAM","PERNAFASAN (/Menit)","NADI (/Menit)","SUHU (&deg;C)","EDIT");
			   	...
			   	$t->ColAlign[6]="left"; 
			   	$t->ColAlign[28]="center"; 
				$t->ColFormatHtml[28] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=edit&mr=".$_GET["mr"]."&rg=<#0#>
				...
				&tmp23=<#24#>&tmp24=<#25#>&tmp25=<#26#>&tmp26=<#27#>'>".icon("edit","Edit")."</A>";	
				$t->execute();
			...
			if($d2["vis_3"] != ''){
			$f->textinfo("f_vis_3",$visit_ri_grafik_suhu["vis_3"],10,10,$d2["vis_3"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_3",$visit_ri_grafik_suhu["vis_3"],10,10,"08:00","(Jam:Menit, 08:09)",$ext);	
			}		
			$f->textinfo("f_vis_4",$visit_ri_grafik_suhu["vis_4"],10,10,$d2["vis_4"],"/Menit",$ext);
			$f->textinfo("f_vis_5",$visit_ri_grafik_suhu["vis_5"],10,10,$d2["vis_5"],"/Menit",$ext);
			$f->textinfo("f_vis_6",$visit_ri_grafik_suhu["vis_6"],10,10,$d2["vis_6"],"&deg;C",$ext);
			$f->text("f_vis_7",$visit_ri_grafik_suhu["vis_7"],30,50,$d2["vis_7"],$ext);
			$f->text("f_vis_8",$visit_ri_grafik_suhu["vis_8"],30,50,$d2["vis_8"],$ext);
			$f->text("f_vis_9",$visit_ri_grafik_suhu["vis_9"],30,50,$d2["vis_9"],$ext);
			$f->text("f_vis_10",$visit_ri_grafik_suhu["vis_10"],30,50,$d2["vis_10"],$ext);
			$f->text("f_vis_11",$visit_ri_grafik_suhu["vis_11"],30,50,$d2["vis_11"],$ext);
			$f->text("f_vis_12",$visit_ri_grafik_suhu["vis_12"],30,50,$d2["vis_12"],$ext);
			$f->text("f_vis_13",$visit_ri_grafik_suhu["vis_13"],30,50,$d2["vis_13"],$ext);
			$f->text("f_vis_14",$visit_ri_grafik_suhu["vis_14"],30,50,$d2["vis_14"],$ext);
			$f->text("f_vis_15",$visit_ri_grafik_suhu["vis_15"],30,50,$d2["vis_15"],$ext);
			$f->text("f_vis_16",$visit_ri_grafik_suhu["vis_16"],30,50,$d2["vis_16"],$ext);
			$f->text("f_vis_17",$visit_ri_grafik_suhu["vis_17"],30,50,$d2["vis_17"],$ext);
			$f->text("f_vis_18",$visit_ri_grafik_suhu["vis_18"],30,50,$d2["vis_18"],$ext);
			$f->text("f_vis_19",$visit_ri_grafik_suhu["vis_19"],30,50,$d2["vis_19"],$ext);
			$f->text("f_vis_20",$visit_ri_grafik_suhu["vis_20"],30,50,$d2["vis_20"],$ext);
			$f->text("f_vis_21",$visit_ri_grafik_suhu["vis_21"],30,50,$d2["vis_21"],$ext);
			$f->text("f_vis_22",$visit_ri_grafik_suhu["vis_22"],30,50,$d2["vis_22"],$ext);
			$f->text("f_vis_23",$visit_ri_grafik_suhu["vis_23"],30,50,$d2["vis_23"],$ext);
			$f->text("f_vis_24",$visit_ri_grafik_suhu["vis_24"],30,50,$d2["vis_24"],$ext);
			$f->textinfo("f_vis_25",$visit_ri_grafik_suhu["vis_25"],10,10,$d2["vis_25"],"Kg",$ext);
			$f->textarea("f_vis_26",$visit_ri_grafik_suhu["vis_26"],1,50,$d2["vis_26"],$ext);
