...
//-------------------------------       	
$_GET["mPOLI"]=$setting_poli["akupunktur"];
...
title("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  HEMODIALISA");
echo "</td></tr></table>";

unset($_GET["layanan"]);

$reg = $_GET["rg"];
$reg2 = $_GET["rg"];//identifikasi untuk input rincian2.php
...
if ($reg > 0) {
    $r = pg_query($con,
       "select a.id,a.mr_no,a.nama,a.umur,a.tgl_lahir,a.tmp_lahir,a.tanggal_reg,a.status_akhir,a.diagnosa_sementara, ".
		...
		"from rsv_pasien2 a  ".
		//"where a.id= '{$_GET['rg']}'";
        //edited 140210 solve : menghilangkan fungsi lpad()
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_akupunktur.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
	...
	$T->show(2);
        ...
        echo "<form name='Form9' action='actions/p_akupunktur.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=list VALUE='icd'>";
     ...
     /*  if (isset($_SESSION["SELECT_LAYANAN"]) && $is_range) {
        ...
        echo "<form name='Form10' action='actions/p_akupunktur.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<input type=hidden name=rawatan value='".$rawatan."'>";
		...
		*/
    	if ($_GET["act"] == "detail") {
			...
			$f->text($visit_akupunktur["vis_8"],$d[10]."&nbsp;/Menit" );
			$f->execute();
			echo "</td><td valign=top>";
			$f = new ReadOnlyForm();
			$f->text($visit_akupunktur["vis_9"],$d[11]);
			$f->text($visit_akupunktur["vis_10"],$d[12]."&nbsp;Kg" );
			$f->text($visit_akupunktur["vis_11"],$d[13]."&deg;C");
		...
		*/
		...
		/*
		-- edited 150210
		-- mengganti fungsi substr() menjadi TO_CHAR
		-- mengganti type data d.id menjadi integer
		-- mengarahkan query string mPOLI
		*/		
    	...
    	$f = new Form("actions/p_akupunktur.insert.php", "POST", "NAME=Form2");
					$f->hidden("act","new1");
...
echo "<table border=0 width='100%'><tr>";
  ...
  echo "<form name='Form9' action='actions/320RJAKUPUNKTUR.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<input type=hidden name=rawatan value='".$rawatan."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
 ...
 include("rincianobat.php"); 
    	...
    	echo "<td rowspan='3'>";    
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
					...
					$f->textarea("f_vis_".$i,$visit_akupunktur["vis_".$i] ,1, $visit_akupunktur["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);

					}
			$i++ ;
		}
		$f->title1("<U>PEMERIKSAAN FISIK</U>","left");
		$f->text("f_vis_5","Kesadaran",30,50,ucfirst($d2["vis_5"]),$ext);
		$f->textinfo("f_vis_6","Tensi",10,10,$d2["vis_6"],"mm Hg",$ext);
		$f->textinfo("f_vis_7","Nadi",10,10,$d2["vis_7"],"/Menit",$ext);
		$f->text("f_vis_8","Respirasi",10,10,$d2["vis_8"],$ext);
		$f->text("f_vis_9","Kulit",30,50,ucfirst($d2["vis_9"]),$ext);
		$f->textinfo("f_vis_10","Berat Badan",10,10,$d2["vis_10"],"Kg",$ext);
		$f->textinfo("f_vis_11","Suhu",10,10,$d2["vis_11"],"&deg;C",$ext);
		$f->text("f_vis_12","Lain-Lain",30,50,ucfirst($d2["vis_12"]),$ext);
		$f->title1("<U>DIAGNOSA KERJA</U>","left");
		$f->textarea("f_vis_13","DD",1,48,ucfirst($d2["vis_13"]),$ext);
		$f->textarea("f_vis_14","Lain-Lain",1,48,ucfirst($d2["vis_14"]),$ext);
		$f->textarea("f_vis_15","Diagnosa",1,48,ucfirst($d2["vis_15"]),$ext);
		$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
		$f->execute();
		echo"</div>";
    	echo "</td><td ><IMG BORDER=0 SRC='images/orang_03.gif'></td>";
    	echo "<td><IMG BORDER=0 SRC='images/orang_04.gif'></td></tr>";
  		echo "<tr><td><IMG BORDER=0 SRC='images/orang_02.gif'></td>";
    	echo "<td><IMG BORDER=0 SRC='images/orang_01.gif'></td>";
  		echo "</tr></table>";
