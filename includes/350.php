...
if ($_GET["httpHeader"] == "1") {
	...
	if (isset($_GET["jam_pengadaan"])) {
        $_SESSION["ob4"]["jam-pengadaan"] = $_GET["jam_pengadaan"];
    }
	if (isset($_GET["jenis"])) {
        $_SESSION["ob4"]["jenis"] = $_GET["jenis"];
    }
...
$rowsJePes = pg_query($con, "SELECT tc AS jenis_id, tdesc AS jenis_nama, tc AS komen FROM rs00001 WHERE tt = 'JSP' AND tc NOT IN ('000') ORDER BY tdesc ASC");
...
echo "<tr><td>Jenis Pesanan</td><td class=FORM>:</td><td><select name=jenis id=jenis ><option value=''></option>";
                            while($rowJePes=pg_fetch_array($rowsJePes)){
                                echo "<option onClick='nopo()'  value =".$rowJePes["jenis_id"]." ";
                                        if($_SESSION["ob4"]["jenis"] == $rowJePes['jenis_id']){
                                            echo 'selected="selected"';
                                        }
                                echo '>'.$rowJePes["jenis_nama"].'</option>';
                            }
echo "<option value='".$_SESSION["ob4"]["jenis"]."'></option></select></td></tr>";

echo "<tr><td class=design10a>Nomor PO</td><td class=FORM>:</td>";
echo "    <td class=design10 colspan=2><input type=TEXT name=nomor_po id=nomor_po size=15 maxlength=20 value='".$_SESSION["ob4"]["nomor_po"]."' readonly></td></tr>";
...
echo "<TR ><TD CLASS=design10a>Jam Pengadaan (format = 24)</TD><TD CLASS=FORM>:</TD>\n";
echo "<TD CLASS=design10 VALIGN='ABSMIDDLE'><INPUT TYPE=TEXT NAME=jam_pengadaan id=jam_pengadaan SIZE=10 MAXLENGTH=12 VALUE='".$jam."'>\n";
echo "</TD>\n";
echo "</TR>\n\n";
...
$tah=date('y');
$bul=date('m');
$po_id = getFromTable("select count(po_id) from c_po where date_part('year',tanggal_entry)='".(int) date('Y')."' and date_part('month',tanggal_entry)='".(int) date('m')."' ");
$no_po = nopo();
?>
<script language='javascript'>
function nopo()
{
	//var nomor_po=document.getElementById("jenis").value
	if (document.getElementById("jenis").value=='001')
	{
	nomor_po="B"
    document.formx.nomor_po.value=nomor_po+'/'+<?=$tah;?>+'/'+<?=$bul;?>+'/'+<?=$no_po;?>;
	} else if (document.getElementById("jenis").value=='002')
	{
	nomor_po="PSI"
    document.formx.nomor_po.value=nomor_po+'/'+<?=$tah;?>+'/'+<?=$bul;?>+'/'+<?=$no_po;?>;
	} else if (document.getElementById("jenis").value=='003')
	{
	nomor_po="N"
    document.formx.nomor_po.value=nomor_po+'/'+<?=$tah;?>+'/'+<?=$bul;?>+'/'+<?=$no_po;?>;
	} else if (document.getElementById("jenis").value=='004')
	{
	nomor_po="PF"
    document.formx.nomor_po.value=nomor_po+'/'+<?=$tah;?>+'/'+<?=$bul;?>+'/'+<?=$no_po;?>;
	}
