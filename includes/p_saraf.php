...
//-------------------------------       	
$_GET["mPOLI"]=$setting_poli["saraf"];
$rg = isset($_GET["rg"])? $_GET["rg"] : $_POST["rg"];
$mr = isset($_GET["mr"])? $_GET["mr"] : $_POST["mr"];

// Tambahan BHP
$POLI=$setting_poli["saraf"];
// ======================================

include ("session.php");

echo "<table border=0 width='100%'><tr><td>";
title_print("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK SARAF");
title_excel("p_saraf&list=".$_GET['list']."&rg=".$_GET['rg']."&poli=".$_GET['poli']."&mr=".$_GET['mr']."&sub=".$_GET['sub']."&act=".$_GET['act']."&polinya=".$_GET['polinya']."&tblstart=".$_GET['tblstart']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_saraf.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
	...
	$f->hidden("sub",$_GET["sub"]);
    	...
    	if ($_GET["act"] == "detail") {
			...
			$f->text($visit_saraf["vis_1"],$d[3] );
			$f->text($visit_saraf["vis_2"],$d[4]);
			$f->text($visit_saraf["vis_3"],$d[5]);
			$f->text($visit_saraf["vis_4"],$d[6] );
    		//$f->title1("PEMERIKSAAN FISIK","LEFT");	
			$f->text($visit_saraf["vis_5"],$d[7] );
		...
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
				...
				$sql = "SELECT A.NO_REG,TO_CHAR(A.TANGGAL_REG,'DD MON YYYY')AS TANGGAL,TO_CHAR(A.TANGGAL_REG,'HH:MM:SS') AS WAKTU,A.VIS_2,'DUMMY' ". 
					   "FROM C_VISIT A ".
	...
	include ("p_saraf.konsultasi.php");
 ...
 include("rincianobat.php"); 
    	...
    	$T->show(0);
    		...
    		$sql2 =	"SELECT A.*,B.NAMA, D.nama as perawat FROM C_VISIT A 
    				...
    				WHERE A.ID_POLI={$_GET["mPOLI"]} AND A.NO_REG='$rg' and user_id!='' "; 
	    	$r=pg_query($con,$sql2);
	    	$n = pg_num_rows($r);		    	
				...
				if ($_GET['act'] == "edit"){
						...
						$f = new Form("actions/p_saraf.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
				...
				//---------------------------------------------------------------------------------			
					...
					$f->calendar("f_vis_1","Tanggal Datang Pertama",15,15,$d2["vis_1"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
					$f->textarea("f_vis_2",$visit_saraf["vis_2"] ,1, $visit_saraf["vis_2"."W"],ucfirst($d2["vis_2"]),$ext);
					//Febri 24112012
					$f->textarea("f_vis_7",$visit_saraf["vis_7"] ,1,  $visit_saraf["vis_7"."W"],ucfirst($d2["vis_7"]),$ext);
					$f->textarea("f_vis_3",$visit_saraf["vis_3"] ,1, $visit_saraf["vis_3"."W"],ucfirst($d2["vis_3"]),$ext);
					$f->textarea("f_vis_4",$visit_saraf["vis_4"] ,1, $visit_saraf["vis_4"."W"],ucfirst($d2["vis_4"]),$ext);
					$f->textarea("f_vis_5",$visit_saraf["vis_5"] ,1, $visit_saraf["vis_5"."W"],ucfirst($d2["vis_5"]),$ext);
    ...
    $f->hidden("list2",tab1);
	    ...
	    if (!$GLOBALS['print']){
	    	$f->search("search","Pencarian",20,20,$_GET["search"],"icon/ico_find.gif","Cari","OnChange='Form4.submit();'");
	    		    		    	//title_excel("p_saraf&tblstart=".$_GET['tblstart']);
