...
if ($_GET["httpHeader"] == "1") {
    if (isset($_GET["nomor_bukti"])) {
        $_SESSION["ob6"]["nomor-bukti"] = $_GET["nomor_bukti"];
    }
    if (strlen($_GET["ob6_id"]) > 0 && $_GET["ob6_jumlah"] > 0) {
        if (is_array($_SESSION["ob6"]["obat"])) {
            $cnt = count($_SESSION["ob6"]["obat"]);
        } else {
            $cnt = 0;
        }
        $r1 = pg_query($con, "select * from rsv0004 where id = '".$_GET["ob6_id"]."'");
        $d1 = pg_fetch_object($r1);
        pg_free_result($r1);
        $_SESSION["ob6"]["obat"][$cnt]["id"]     = $d1->id;
        $_SESSION["ob6"]["obat"][$cnt]["obat"]   = $d1->obat;
        $_SESSION["ob6"]["obat"][$cnt]["satuan"] = $d1->satuan;
        $_SESSION["ob6"]["obat"][$cnt]["jumlah"] = $_GET["ob6_jumlah"];
        $_SESSION["ob6"]["obat"][$cnt]["harga"]  = $d1->harga;
        $_SESSION["ob6"]["obat"][$cnt]["total"]  = $d1->harga * $_GET["ob6_jumlah"];
        unset($_SESSION["SELECT_OBAT"]);
    }
    if (isset($_GET["del"])) {
        $temp = $_SESSION["ob6"]["obat"];
        unset($_SESSION["ob6"]["obat"]);
        $cnt = 0;
        foreach ($temp as $k => $v) {
            if ($k != $_GET["del"]) {
                $_SESSION["ob6"]["obat"][$cnt] = $v;
                $cnt++;
            }
        }
    }
    if (isset($_GET["editrow"])) {
        $_SESSION["ob6"]["obat"][$_GET["editrow"]]["jumlah"] = $_GET["editjumlah"];
        $_SESSION["ob6"]["obat"][$_GET["editrow"]]["total"]  =
            $_SESSION["ob6"]["obat"][$_GET["editrow"]]["jumlah"] *
            $_SESSION["ob6"]["obat"][$_GET["editrow"]]["harga"];
    }
    header("Location: $SC?p=$PID");
    exit;
}

title("Transaksi Pengadaan Barang Inst. Gizi");
echo "<br>";

if (isset($_SESSION["SELECT_SUPPLIER"])) {
    $_SESSION["ob6"]["supplier"]["id"]   = $_SESSION["SELECT_SUPPLIER"];
    $_SESSION["ob6"]["supplier"]["name"] =
...
echo "    <td class=FORM width=1><input style='text-align:center' type=TEXT name=supplier size=5 maxlength=10 value='".$_SESSION["ob6"]["supplier"]["id"]."' DISABLED></td>";
...
echo "    <td class=FORM colspan=2>".$_SESSION["ob6"]["supplier"]["name"]."</td></tr>";
echo "<tr><td class=FORM>Nomor Bukti</td><td class=FORM>:</td>";
echo "    <td class=FORM colspan=2><input type=TEXT name=nomor_bukti size=30 maxlength=30 value='".$_SESSION["ob6"]["nomor-bukti"]."'></td></tr>";
...
if (isset($_SESSION["ob6"]["supplier"]["id"]) && isset($_SESSION["ob6"]["nomor-bukti"])) {
    ...
    if (is_array($_SESSION["ob6"]["obat"])) {
        $total = 0.00;
        foreach($_SESSION["ob6"]["obat"] as $k => $o) {
    ...
    if (strlen($_GET["edit"]) == 0) {
        ...
        echo "<input type=hidden name=httpHeader value=1>";
        $t->printRow(
            Array( "<input type=text size=5 maxlength=10 name=ob6_id style='text-align:center' value=$d1->id>".
                "&nbsp;<a href='javascript:selectObat()'>".icon("view")."</a>",
                $d1->obat,
                "<input type=text size=5 maxlength=10 name=ob6_jumlah value=1 style='text-align:right'>",
                $d1->satuan,
                $d1->harga,
    ...
    if (is_array($_SESSION["ob6"]["obat"])) {
        ...
        echo "<form action='actions/550.insert.php' method=POST name=Form10>";
