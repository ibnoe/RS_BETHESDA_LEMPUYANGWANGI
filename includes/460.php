...
if($_GET["tc"] == "view") {
    if(!$GLOBALS['print']){
		title_print("<img src='icon/medical-record-2.gif' align='absmiddle' >Rincian Laporan Rawat Inap");
		echo "<DIV ALIGN=RIGHT OnClick='window.history.back()'>".icon("back","Kembali")."</a></DIV>";
	}else {
		title_print("<img src='icon/medical-record.gif' align='absmiddle' > Rincian Laporan Rawat Inap");
	}
    if ($_GET["x"] == "001") {
        $tp = getFromTable(
    ...
    if (!$GLOBALS['print']){
		title_print("<img src='icon/medical-record-2.gif' align='absmiddle' > LAPORAN BULANAN PELAYANAN RAWAT INAP");
		$ext = "OnChange = 'Form1.submit();'";
	}else {
		title_print(" LAPORAN BULANAN PELAYANAN RAWAT INAP","center");
		$ext = "disabled";
	}
    if (!$GLOBALS['print']){
   ...
   $SQL = "select  a.comment as layanan, 
		...
		?>			
			...
			<tr class="TBL_HEAD">     	
				...
				<td class="TBL_HEAD" width="15%" align="center" rowspan="3">JENIS PELAYANAN<BR>RAWAT INAP</td>
				<td class="TBL_HEAD" align="center" colspan="7">RAWATAN BARU</td>
				<td class="TBL_HEAD" align="center" colspan="7">RAWATAN ULANG</td>
