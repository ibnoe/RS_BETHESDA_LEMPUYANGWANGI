...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_lembar_konsultasi&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_lembar_konsultasi.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_lembar_konsultasi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        ...
        echo "<form name='Form10' action='actions/p_lembar_konsultasi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
				$sql = 	"select a.*,to_char(a.tanggal_reg,'DD MON YYYY HH24:MI:SS')as tanggal_reg,f.layanan,g.nama as merawat ".
						...
						"left join rs00017 g on CAST (a.vis_7 AS INTEGER) = g.id ".
			...
			echo"<div class=form_subtitle>DATA KONSULTASI</div>";
			//echo "</td></tr>";
    		echo "<tr><td  valign=top>";
    		$f = new ReadOnlyForm();
			$f->text("<B>".$visit_ri_lembar_konsultasi["vis_7"]."</B>",$d["merawat"]);
			$f->text($visit_ri_lembar_konsultasi["vis_1"],$d[4]);
			$f->text($visit_ri_lembar_konsultasi["vis_2"],$d[5] );
			$f->text($visit_ri_lembar_konsultasi["vis_3"],$d[6]);
			$f->text($visit_ri_lembar_konsultasi["vis_4"],$d[7]);		
			$f->text($visit_ri_lembar_konsultasi["vis_5"],$d[8]);		
			$f->text($visit_ri_lembar_konsultasi["vis_6"],$d[9]);		
			$f->execute();	
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT KONSULTASI</div>";
		//detail riwayat
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
		
		$sql = "SELECT A.NO_REG,to_char(A.TANGGAL_REG,'DD MON YYYY HH24:MI:SS')as tgl_reg,G.NAMA AS MERAWAT,A.VIS_1,'DUMMY' ". 
					   "FROM C_VISIT_RI A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					   "LEFT JOIN RS00017 G ON CAST (A.VIS_7 AS INTEGER) = G.ID ".
					   "WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' 
                                           group by A.NO_REG,A.TANGGAL_REG,G.NAMA ,A.VIS_1 ";
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL/JAM PENCATATAN","DOKTER","KONSULEN","DETAIL");
...
//====iqbal tambah tab konsultasi untuk rawat inap 15/02/12====			
    	...
    	$T->show(0);
    		...
    		$sql2 =	"SELECT A.*,C.NAMA AS merawat FROM C_VISIT_RI A 
    				LEFT JOIN RS00017 C ON CAST (A.VIS_7 AS INTEGER) = C.ID
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_lembar_konsultasi.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			if (isset($_SESSION["SELECT_EMP"])) {
    					$_SESSION["LEMBAR_KONSULTASI"]["id"] = $_SESSION["SELECT_EMP"];
    					$_SESSION["LEMBAR_KONSULTASI"]["nama"] =
        				getFromTable(
			            "select nama from rs00017 where id = '".$_SESSION["LEMBAR_KONSULTASI"]["id"]."'");
    					$f->textAndButton3("f_VIS_7","Dokter",2,10,$_SESSION["LEMBAR_KONSULTASI"]["id"],$ext,"nm2",30,70,$_SESSION["LEMBAR_KONSULTASI"]["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
			            
					}else{
						$f->textAndButton3("f_VIS_7","Dokter",2,10,$d2["vis_7"],$ext,"nm2",30,70,$d2["merawat"],$ext,"...",$ext,"OnClick='selectPegawai();';");
					}
				
			$f->text("f_vis_1",$visit_ri_lembar_konsultasi["vis_1"],50,50,$d2["vis_1"],$ext);
			$f->textarea("f_vis_2",$visit_ri_lembar_konsultasi["vis_2"] ,1, $visit_ri_lembar_konsultasi["vis_2"."W"],$d2["vis_2"],$ext);
			$f->textarea("f_vis_3",$visit_ri_lembar_konsultasi["vis_3"] ,1, $visit_ri_lembar_konsultasi["vis_3"."W"],$d2["vis_3"],$ext);
			if($d2["vis_4"]=="Ya"){
			    	$f->checkbox2($visit_ri_lembar_konsultasi["vis_4"],"f_vis_4","Ya","Ya","CHECKED","Tidak","Tidak","",$ext);
			    }elseif ($d2["vis_4"]=="Tidak"){
			    	$f->checkbox2($visit_ri_lembar_konsultasi["vis_4"],"f_vis_4","Ya","Ya","","Tidak","Tidak","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_lembar_konsultasi["vis_4"],"f_vis_4","Ya","Ya","","Tidak","Tidak","",$ext);
			    }
			$f->textarea("f_vis_5",$visit_ri_lembar_konsultasi["vis_5"] ,1, $visit_ri_lembar_konsultasi["vis_5"."W"],$d2["vis_5"],$ext);
			$f->textarea("f_vis_6",$visit_ri_lembar_konsultasi["vis_6"] ,1, $visit_ri_lembar_konsultasi["vis_6"."W"],$d2["vis_6"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			
			echo"</div>";
			echo "<div align=right><b>RM 10</b></div>";
