...
//-------------------------------       	
$_GET["mPOLI"]=$setting_ri["pengawasan_pasien_dewasa"];
...
$reg1 = (int) $_GET["rg1"];
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Data Pengawasan Pasien dewasa"	, $tab_disabled["pemeriksaan"]);
	...
	$T->addTab("$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Pengawasan Pasien dewasa"	, $tab_disabled["riwayat"]);
...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_pengawasan_dewasa&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_pengawasan_dewasa.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_pengawasan_dewasa.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        ...
        echo "<form name='Form10' action='actions/p_pengawasan_dewasa.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
				$sql = 	"select a.*,to_char(a.tanggal_reg,'DD MON YYYY HH24:MI:SS')as tanggal_reg,f.layanan,g.nama as merawat ".
						...
						"left join rs00017 g on CAST (a.vis_15 AS INTEGER) = g.id ".
			...
			echo"<div class=form_subtitle>DATA PENGAWASAN PASIEN DEWASA</div>";
			echo "</td></tr>";
    		echo "<tr><td  valign=top>";
    		$f = new ReadOnlyForm();
    		$f->text("<B>".$visit_ri_pengawasan_pasien_dewasa["vis_15"]."</B>",$d["merawat"]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_16"],$d[19]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_17"],$d[20]);
			$f->title1("<U>OBSERVASI</U>");
    		$f->text($visit_ri_pengawasan_pasien_dewasa["vis_1"],$d[4]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_2"],$d[5]."&nbsp;mm Hg");
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_3"],$d[6]."&nbsp;/Menit");
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_4"],$d[7]."&nbsp;/Menit" );
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_5"],$d[8]."&nbsp;&deg;C");
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_6"],$d[9]);
			$f->execute();
			echo"</td><td valign=top>";
			$f =new ReadOnlyForm();
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_7"],$d[10]);
			$f->title1("<U>KESEIMBANGAN CAIRAN</U>");
    		$f->text($visit_ri_pengawasan_pasien_dewasa["vis_8"],$d[11]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_9"],$d[12]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_10"],$d[13]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_11"],$d[14]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_12"],$d[15]);			
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_13"],$d[16]);
			$f->text($visit_ri_pengawasan_pasien_dewasa["vis_14"],$d[17]);
			$f->execute();	
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT PENGAWASAN PASIEN DEWASA</div>";
		//detail riwayat
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
		
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,A.VIS_16,A.VIS_17,A.VIS_1,A.VIS_2,A.VIS_3,A.VIS_4,A.VIS_5,A.VIS_6,A.TANGGAL_REG,'DUMMY'". 
					   "FROM C_VISIT_RI A ".
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","KESADARAN","TENSI (mm Hg)","NADI (/Menit)","PERNAPASAN (/Menit)","SUHU (&deg;C)","CVP","DETAIL");
			   	$t->ColAlign = array("center","center","center","left","left","left","left","left","left","left","center");
				$t->ColFormatHtml[10] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=detail&mr=".$_GET["mr"]."&rg=<#0#>&tgl=<#9#>'>".icon("view","Detail")."</A>";	
				$t->execute();
    	...
    	$sql2 =	"SELECT A.*,C.NAMA AS merawat FROM C_VISIT_RI A 
    				LEFT JOIN RS00017 C ON CAST (A.VIS_15 AS INTEGER) = C.ID
        ...
        $r=pg_query($con,$sql2);
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_pengawasan_dewasa.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			$_SESSION["PENGAWASAN_DEWASA"]["id"] = $_SESSION["SELECT_EMP"];
			$_SESSION["PENGAWASAN_DEWASA"]["nama"] =
			getFromTable(
			"select nama from rs00017 where id = '".$_SESSION["PENGAWASAN_DEWASA"]["id"]."'");
			$f->textAndButton3("f_vis_15","Perawat",2,10,$_SESSION["PENGAWASAN_DEWASA"]["id"],$ext,"nm2",30,70,$_SESSION["PENGAWASAN_DEWASA"]["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");

			}else{
			$f->textAndButton3("f_vis_15","Perawat",2,10,$_GET["tmp17"],$ext,"nm2",30,70,$d2["merawat"],$ext,"...",$ext,"OnClick='selectPegawai();';");
			}
			$f->calendar("f_vis_16","Tanggal",10,15,$d2["vis_16"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$f->textinfo("f_vis_17",$visit_ri_pengawasan_pasien_dewasa["vis_17"],10,15,$_GET["tmp1"],"(Jam:Menit, 08:09)",$ext);
			$f->title1("<U>OBSERVASI</U>");
			$f->text_6i("","f_vis_1",$visit_ri_pengawasan_pasien_dewasa["vis_1"],15,30,$_GET["tmp2"],"","f_vis_2",$visit_ri_pengawasan_pasien_dewasa["vis_2"],15,15,$_GET["tmp3"],"mm Hg",
			"f_vis_3",$visit_ri_pengawasan_pasien_dewasa["vis_3"],15,30,$_GET["tmp4"],"/Menit","f_vis_4",$visit_ri_pengawasan_pasien_dewasa["vis_4"],15,30,$_GET["tmp5"],"/Menit",
			"f_vis_5",$visit_ri_pengawasan_pasien_dewasa["vis_5"],15,30,$_GET["tmp6"],"&deg;C","f_vis_6",$visit_ri_pengawasan_pasien_dewasa["vis_6"],15,15,$_GET["tmp7"],"",$ext);
			$f->text("f_vis_7",$visit_ri_pengawasan_pasien_dewasa["vis_7"],30,30,$_GET["tmp8"],$ext);
			$f->title1("<U>KESEIMBANGAN CAIRAN</U>");
			$f->text("f_vis_8",$visit_ri_pengawasan_pasien_dewasa["vis_8"],30,30,$_GET["tmp9"],$ext);
			$f->text("f_vis_9",$visit_ri_pengawasan_pasien_dewasa["vis_9"],30,30,$_GET["tmp10"],$ext);
			$f->text("f_vis_10",$visit_ri_pengawasan_pasien_dewasa["vis_10"],6,30,$_GET["tmp11"],$ext);
			$f->text("f_vis_11",$visit_ri_pengawasan_pasien_dewasa["vis_11"],30,30,$_GET["tmp12"],$ext);
			$f->text("f_vis_12",$visit_ri_pengawasan_pasien_dewasa["vis_12"],30,30,$_GET["tmp13"],$ext);
			$f->text("f_vis_13",$visit_ri_pengawasan_pasien_dewasa["vis_13"],30,30,$_GET["tmp14"],$ext);
			$f->text("f_vis_14",$visit_ri_pengawasan_pasien_dewasa["vis_14"],6,30,$_GET["tmp15"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			
			echo"</div>";
			echo "<div align=right><b>RM 09</b></div>";
			...
			$f->textAndButton3("f_vis_15","Perawat",2,10,$d2["vis_15"],$ext,"nm2",30,70,$d2["merawat"],$ext,"...",$ext,"OnClick='selectPegawai();';");
			}
			$f->calendar("f_vis_16","Tanggal",10,15,$d2["vis_16"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$f->textinfo("f_vis_17",$visit_ri_pengawasan_pasien_dewasa["vis_17"],10,15,"00:00","(Jam:Menit, 08:09)",$ext);
			$f->title1("<U>OBSERVASI</U>");
			$f->text_6i("","f_vis_1",$visit_ri_pengawasan_pasien_dewasa["vis_1"],15,30,"","","f_vis_2",$visit_ri_pengawasan_pasien_dewasa["vis_2"],15,15,"","mm Hg",
			"f_vis_3",$visit_ri_pengawasan_pasien_dewasa["vis_3"],15,30,"","/Menit","f_vis_4",$visit_ri_pengawasan_pasien_dewasa["vis_4"],15,30,"","/Menit",
			"f_vis_5",$visit_ri_pengawasan_pasien_dewasa["vis_5"],15,30,"","&deg;C","f_vis_6",$visit_ri_pengawasan_pasien_dewasa["vis_6"],15,15,"","",$ext);
			$f->text("f_vis_7",$visit_ri_pengawasan_pasien_dewasa["vis_7"],30,30,"",$ext);
			$f->title1("<U>KESEIMBANGAN CAIRAN</U>");
			$f->text("f_vis_8",$visit_ri_pengawasan_pasien_dewasa["vis_8"],30,30,"",$ext);
			$f->text("f_vis_9",$visit_ri_pengawasan_pasien_dewasa["vis_9"],30,30,"",$ext);
			$f->text("f_vis_10",$visit_ri_pengawasan_pasien_dewasa["vis_10"],6,30,"",$ext);
			$f->text("f_vis_11",$visit_ri_pengawasan_pasien_dewasa["vis_11"],30,30,"",$ext);
			$f->text("f_vis_12",$visit_ri_pengawasan_pasien_dewasa["vis_12"],30,30,"",$ext);
			$f->text("f_vis_13",$visit_ri_pengawasan_pasien_dewasa["vis_13"],30,30,"",$ext);
			$f->text("f_vis_14",$visit_ri_pengawasan_pasien_dewasa["vis_14"],6,30,"",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			...
			$sql = "SELECT A.NO_REG,A.VIS_16,A.VIS_17,A.VIS_1,A.VIS_2,A.VIS_3,A.VIS_4,A.VIS_5,A.VIS_6,A.VIS_7,A.VIS_8,A.VIS_9,
					A.VIS_10,A.VIS_11,A.VIS_12,A.VIS_13,A.VIS_14,A.TANGGAL_REG,A.VIS_15,'DUMMY'". 
					"FROM C_VISIT_RI A ".
					"LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					"WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' ORDER BY VIS_17 ";
				$t = new PgTable($con, "100%");
			    $t->SQL = $sql ;
			    $t->setlocale("id_ID");
			    $t->ShowRowNumber = true;
			    ...
			    $t->ColHidden[19]=true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","KESADARAN","TENSI (mm Hg)","NADI (/Menit)","PERNAPASAN (/Menit)","SUHU (&deg;C)","EDIT");
			   	$t->ColAlign = array("center","center","center","left","left","left","left","left","center","","","","","","","","","","","center");
				$t->ColFormatHtml[19] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=edit&mr=".$_GET["mr"]."&rg=<#0#>
				&tmp1=<#2#>&tmp2=<#3#>&tmp3=<#4#>&tmp4=<#5#>&tmp5=<#6#>&tmp6=<#7#>&tmp7=<#8#>&tmp8=<#9#>&tmp9=<#10#>&tmp10=<#11#>&tmp11=<#12#>&tmp12=<#13#>&tmp13=<#14#>&tmp14=<#15#>&tmp15=<#16#>&tmp16=<#17#>&tmp17=<#18#>'>".icon("edit","Edit")."</A>";	
				$t->execute();
				echo "<br>";
			...
			if($d2["vis_16"]!=''){
				...
				$f->calendar("f_vis_16","Tanggal",10,15,$tglhariini,"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			}
			
			if($d2["vis_17"] != ''){
			$f->textinfo("f_vis_17",$visit_ri_pengawasan_pasien_dewasa["vis_17"],10,15,$d2["vis_17"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_17",$visit_ri_pengawasan_pasien_dewasa["vis_17"],10,15,"00:00","(Jam:Menit, 08:09)",$ext);
			}
			$f->title1("<U>OBSERVASI</U>");
			$f->text_6i("","f_vis_1",$visit_ri_pengawasan_pasien_dewasa["vis_1"],15,30,$d2["vis_1"],"","f_vis_2",$visit_ri_pengawasan_pasien_dewasa["vis_2"],15,15,$d2["vis_2"],"mm Hg",
			"f_vis_3",$visit_ri_pengawasan_pasien_dewasa["vis_3"],15,30,$d2["vis_3"],"/Menit","f_vis_4",$visit_ri_pengawasan_pasien_dewasa["vis_4"],15,30,$d2["vis_4"],"/Menit",
			"f_vis_5",$visit_ri_pengawasan_pasien_dewasa["vis_5"],15,30,$d2["vis_5"],"&deg;C","f_vis_6",$visit_ri_pengawasan_pasien_dewasa["vis_6"],15,15,$d2["vis_6"],"",$ext);
			$f->text("f_vis_7",$visit_ri_pengawasan_pasien_dewasa["vis_7"],30,30,$d2["vis_7"],$ext);
			$f->title1("<U>KESEIMBANGAN CAIRAN</U>");
			$f->text("f_vis_8",$visit_ri_pengawasan_pasien_dewasa["vis_8"],30,30,$d2["vis_8"],$ext);
			$f->text("f_vis_9",$visit_ri_pengawasan_pasien_dewasa["vis_9"],30,30,$d2["vis_9"],$ext);
			$f->text("f_vis_10",$visit_ri_pengawasan_pasien_dewasa["vis_10"],6,30,$d2["vis_10"],$ext);
			$f->text("f_vis_11",$visit_ri_pengawasan_pasien_dewasa["vis_11"],30,30,$d2["vis_11"],$ext);
			$f->text("f_vis_12",$visit_ri_pengawasan_pasien_dewasa["vis_12"],30,30,$d2["vis_12"],$ext);
			$f->text("f_vis_13",$visit_ri_pengawasan_pasien_dewasa["vis_13"],30,30,$d2["vis_13"],$ext);
			$f->text("f_vis_14",$visit_ri_pengawasan_pasien_dewasa["vis_14"],6,30,$d2["vis_14"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
