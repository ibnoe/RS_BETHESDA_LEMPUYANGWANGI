...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl100011a where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/lab.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Patologi Klinik");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/lab.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Patologi Klinik");
        $f->hidden("id","new");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select jenis, sederhana, sedang, canggih, id as href FROM rl100011a order by id";            
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
	...
	$data = getFromTable("select jenis from rl100011a where id='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/lab.delete.php' method='get'>";
