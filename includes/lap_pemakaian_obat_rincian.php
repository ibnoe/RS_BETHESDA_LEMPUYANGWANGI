...
$PID = "lap_pemakaian_obat_rincian";
...
//--start
if (!$GLOBALS['print']){
    title_print("<img src='icon/keuangan-2.gif' align='absmiddle' > <b>LAPORAN RINCIAN PEMAKAIAN OBAT</b>");
    title_excel("lap_pemakaian_obat_rincian&tanggal1D=".$_GET["tanggal1D"]."&tanggal1M=".$_GET["tanggal1M"]."&tanggal1Y=".$_GET["tanggal1Y"]."".
    ...
    title("<img src='icon/keuangan.gif' align='absmiddle' > Laporan Rincian Pemakaian Obat");
    title_excel("lap_pemakaian_obat_rincian&tanggal1D=".$_GET["tanggal1D"]."&tanggal1M=".$_GET["tanggal1M"]."&tanggal1Y=".$_GET["tanggal1Y"]."".
        "&tanggal2D=".$_GET["tanggal2D"]."&tanggal2M=".$_GET["tanggal2M"]."&tanggal2Y=".$_GET["tanggal2Y"]."".
        "&mUNIT=".$_GET["mUNIT"]."&mPASIEN=".$_GET["mPASIEN"]."&mKATEGORY=".$_GET["mKATEGORY"]."&mOBAT=".$_GET["mOBAT"]."&mDOKTER=".$_GET["mDOKTER"]."&mSPESIALIS=".$_GET["mSPESIALIS"]."");
}
...
if(!$GLOBALS['print']){
    ...
    $f->selectSQL("mKATEGORY", "Kategori Obat",
        ...
        //"where tt='GOB' and tc in ('007','014')", $_GET["mKATEGORY"],
...
//-- rows semua obat
$rowsObat = pg_query($con, "select e.obat as obat, ".
          "     b.diagnosa_sementara as dokter, sum(a.qty) as qty ".
          "from rs00008 a ".
          "     left join rs00006 b ON a.no_reg = b.id ".
...
<table id="list-pasien" width="100%">
    <thead>
        <tr>
            ...
            <td style="font-size: 11px;" align="CENTER" class="TBL_HEAD" width="130">NAMA OBAT</td>
            <td style="font-size: 11px;" align="CENTER" class="TBL_HEAD" width="150">DOKTER</td>
    ...
    <tbody>
        <?php
            if(!empty($rowsObat)){
                 ...
                 while($row=pg_fetch_array($rowsObat)){
                 	...
                 	$newNamaObat = $row['obat'];
                 	
                 	if ($oldNamaObat == $row['obat'] && $oldNamaObat != '') {
						$ii='';
					 } else {
					 	$ii=$i++;
					 }
        ...
        <tr>
            ...
            <td style="font-size: 11px;" align="left">&nbsp;&nbsp;&nbsp;<?php echo $newNamaObat;?></td>
            <td style="font-size: 11px;" align="left">&nbsp;&nbsp;&nbsp;<?php echo $row['dokter'];?></td>
            <td style="font-size: 11px;" align="right"><?php echo $row['qty'];?>&nbsp;&nbsp;&nbsp;</td>
        </tr>
        <?php
        	$oldNamaObat	= $row['obat'];
                 }
            }
        ?>
        <tr>
