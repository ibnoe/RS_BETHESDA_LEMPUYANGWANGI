...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl100010 where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/pelayanan.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Pelayanan Khusus");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/pelayanan.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Pelayanan Khusus");
        $f->hidden("id","new");
        $f->text("id","No",12,12,"<OTOMATIS>","DISABLED");
    }
    $f->PgConn = $con;
    $f->text("f_jenis_kegiatan","Jenis",40,50,$d->jenis_kegiatan);
    ...
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan Pelayanan Khusus");
lihat_laporan("pelayanan_khusus");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select jenis_kegiatan, jumlah, id as href FROM rl100010 order by id";            
              
    $t->ColHeader = array("Jenis Kegiatan", "Jumlah", "Edit");
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
    ...
    $t->ColFormatHtml[2] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#2#>'>".icon("edit","Edit")."</A>".
            ...
            "actions/pelayanan.delete.php?p=$PID".
            "&e=<#2#>".
            "'>".icon("delete","Hapus")."</A>".
            "</nobr>"; 
    /*
    ...
    */
	...
	$data = getFromTable("select jenis_kegiatan from rl100010 where id='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/pelayanan.delete.php' method='get'>";
