...
if ($_GET['act'] ==  "detail"){
		if (!$GLOBALS['print']){
			title_print("<img src='icon/medical-record-2.gif' align='absmiddle' > REKAM MEDIS KLINIK LABORATORIUM");
			echo "<DIV ALIGN=RIGHT OnClick='window.history.back()'>".icon("back","Kembali")."</a></DIV>";
		}else{
			title_print("<img src='icon/medical-record.gif' align='absmiddle' > REKAM MEDIS KLINIK LABORATORIUM");
		}
		    //echo "<br>";
			$sql = "select a.*,b.nama 
		    		from c_visit a
			...
			/*	
				$f = new Form($SC,"POST","");
				$f->info("Upload Hasil Pemeriksaan Laboratorium","");
				$f->file("vis_3","Foto",35);
				$f->submit("Simpan","");
				$f->execute();
			*/	
				echo "</td></tr></table>"; 
	...
	if ($_GET["v"]){ // v from 430.php
    	subtitle2("KLINIK LABORATORIUM","left");		
