...
if(strlen($_GET["e"]) > 0) {
    ...
    title("<img src='icon/informasi-2.gif' align='absmiddle' >  PENGELUARAN");
    if (isset($_GET["e"])) {
        $ext = "DISABLED";
    } else {
...
********/
    ...
    $t->ColHeader = array("NAMA AKUN", "KODE AKUN", "KATEGORI", "KENA PAJAK","KETERANGAN","E d i t");

    $t->execute();
 
    echo "<BR><DIV ALIGN=LEFT><img src=\"icon/inventory.gif\" align=absmiddle ><A CLASS=SUB_MENU ".
         "HREF='index2.php?p=$PID&e=new&o=".$_GET["mOBT"]."&s=".$qr->kategori_stock_id."'> Input $jdl </A></DIV>";
