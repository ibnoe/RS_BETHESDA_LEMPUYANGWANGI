...
// Nugraha, Sun Apr 18 18:58:42 WIT 2004
...
if (!$GLOBALS['print']) {
    title("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK JANTUNG");
    title_print("");
}
...
if ($reg > 0) {
    ...
    //echo "<div class=box>";
    if ($_GET["sub"] == "byr") {
        title("Total Tagihan: Rp. " . number_format($total,
                        2));
        echo "<br><br><br>";
        $f = new Form("actions/p_jantung.insert.php");
        $f->hidden("rg",
                $_GET["rg"]);
        $f->hidden("mr",
                $_GET["mr"]);
        ...
        if ($_GET["act"] == "detail") {
            ...
            $f = new ReadOnlyForm();
            $f->text("Tanggal Pemeriksaan",
                    "<b>" . $d["tanggal_reg"]);
            $f->title1("<U>ANAMNESA</U>",
                    "LEFT");
            $f->text($visit_jantung["vis_1"],
                    (ucfirst($d[3])));
            $f->text($visit_jantung["vis_2"],
                    (ucfirst($d[4])));
            $f->text($visit_jantung["vis_3"],
                    (ucfirst($d[5])));
            $f->text($visit_jantung["vis_4"],
                    (ucfirst($d[6])));
            $f->title1("<U>PEMERIKSAAN FISIK</U>",
                    "LEFT");
            $f->text($visit_jantung["vis_5"],
                    (ucfirst($d[7])));
            $f->text($visit_jantung["vis_6"],
                    $d[8] . "&nbsp;mm Hg");
            $f->text($visit_jantung["vis_7"],
                    $d[9] . "&nbsp;/Menit");
            $f->text($visit_jantung["vis_8"],
                    (ucfirst($d[10])) . "&nbsp;/Menit");
            $f->execute();
            echo "</td><td valign=top>";
            $f = new ReadOnlyForm();
            $f->text($visit_jantung["vis_9"],
                    (ucfirst($d[11])));
            $f->text($visit_jantung["vis_10"],
                    $d[12] . "&nbsp;Kg");
            $f->text($visit_jantung["vis_11"],
                    $d[13] . "&deg;C");
            $f->text($visit_jantung["vis_12"],
                    (ucfirst($d[14])));
            $f->title1("<U>DIAGNOSA KERJA</U>",
                    "LEFT");
            $f->text($visit_jantung["vis_13"],
                    (ucfirst($d[15])));
            $f->text($visit_jantung["vis_14"],
                    (ucfirst($d[16])));
            $f->text($visit_jantung["vis_15"],
                    (ucfirst($d[17])));
            $f->title1("<U>DOKTER PEMERIKSA</U>",
                    "LEFT");
            $f->text("Nama Dokter",
                    $d["nama"]);
            $f->text("Nama Perawat",
            ...
            $t->ColFormatHtml[4] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=riwayat&act=detail&mr=" . $_GET["mr"] . "&poli=" . $_GET["mPOLI"] . "&rg=<#0#>'>" . icon("view",
                            "View") . "</A>";
            $t->execute();
	...
	include ("unit_rujukan.php");
        ...
        include ("p_jantung.konsultasi.php");
        ...
        if ($_GET['act'] == "edit") {
            ...
            $f = new Form("actions/p_jantung.insert.php", "POST", "NAME=Form2");
            $f->hidden("act",
                    "edit");
            $f->hidden("f_no_reg",
                    $d2["no_reg"]);
            $f->hidden("f_tanggal_reg",
                    $d2["tanggal_reg"]);
            $f->hidden("list",
                    "pemeriksaan");
            $f->hidden("mr",
        ...
        echo "<table border=1 width='100%' cellspacing=0 cellpadding=0 class='design10a'><tr><td valign=top width='100%'>";
        ...
        while ($i <= $max) {
            if ($visit_jantung["vis_" . $i . "F"] == "memo") {
                $f->textarea("f_vis_" . $i,
                        $visit_jantung["vis_" . $i],
                        1,
                        $visit_jantung["vis_" . $i . "W"],
                        ucfirst($d2[2 + $i]),
                        $ext);
            } elseif ($visit_jantung["vis_" . $i . "F"] == "memo1") {
                $f->textarea1("<U>DIAGNOSA KERJA</U>",
                        "f_vis_" . $i,
                ...
                $f->text_4("<U>PEMERIKSAAN FISIK</U>",
                        "f_vis_10",
                        $visit_jantung["vis_10"],
                        10,
                        20,
                        $d2["vis_10"],
                        "Kg",
                ...
                $f->text_4("",
                        ...
                        "",
                        "f_vis_11",
                        $visit_jantung["vis_11"],
                        10,
                        20,
                        $d2["vis_11"],
                        "&deg;C",
                        "f_vis_12",
                        $visit_jantung["vis_12"],
                        10,
                        20,
                        $d2["vis_12"],
                        "",
    ...
    $T1 = new TabBar();
    $T1->addTab("$SC?p=$PID&list2=tab1&list=layanan",
            "Daftar Pasien Konsul",
            $tab_disabled["tab1"]);
    $T1->addTab("$SC?p=$PID&list2=tab2&list=layanan",
    ...
    if ($_GET['list2'] == "tab1") {
        ...
        if (!$GLOBALS['print']) {
            $f->search("search",
                    ...
                    "icon/ico_find.gif",
                    "Cari",
                    "OnChange='Form4.submit();'");
            //title_print("");
            title_excel("p_jantung");
        } else {
            $f->search("search",
                    "Pencarian",
                    20,
        ...
        //---------------------
        echo "<br>";
	//Note: 04 Januari 2013 by Me
	//Pasien ditampilkan per tanggal konsul hari ini
	//Menampilkan unit asal pasien
