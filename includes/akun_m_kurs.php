...
$r = pg_query($con,"select * from rs00001 where tt='KUR' and tc='000'");
$d = pg_fetch_object($r);
pg_free_result($r);
...
if(strlen($_GET["e"]) > 0 || strlen($_GET["tc"]) > 0) {
    if($_GET["e"] == "new") {
        if (strlen($_GET["tc"]) == 0) {
            $r8 = pg_query($con,"select max(tc) as tc from rs00001 where tt='KUR'");
            $d8 = pg_fetch_object($r8);
            pg_free_result($r8);
            $_GET["tc"] = str_pad(((int) $d8->tc) + 1, 3, "0", STR_PAD_LEFT);
        }
        $f = new Form("actions/akun_m_kurs.insert.php");
        $f->hidden("tt",KUR);
        ...
        $r2 = pg_query($con,
            "select * ".
            "from rs00001 ".
            "where tt='KUR' and tc='".$_GET["e"]."'");
        $d2 = pg_fetch_object($r2);
        pg_free_result($r2);
        $f = new Form("actions/akun_m_kurs.update.php");
        $f->hidden("tt",KUR);
        $f->hidden("tc",$_GET["e"]);
...
//    echo "<TD WIDTH=1><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select tdesc, tc as dummy from rs00001 where tt='KUR' and tc!='000'".
   ...
   "<A CLASS=TBL_HREF HREF='actions/akun_m_kurs.delete.php?p=$PID&tt=$d->tt&tc=<#1#>'>".icon("delete","Hapus")."</A>"; 
	...
	$data = getFromTable("select tdesc from rs00001 where tt='KUR' and tc='".$_GET[tc]."'");

    echo "<div align=center>";
    echo "<form action='actions/akun_m_kurs.delete.php' method='get'>";
