...
$reg1 = (int) $_GET["rg1"];
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Catatan Harian Penyakit"	, $tab_disabled["pemeriksaan"]);
	...
	$T->addTab("$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Catatan Harian Penyakit"	, $tab_disabled["riwayat"]);
...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_catatan_harian&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_catatan_harian.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_catatan_harian.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        ...
        echo "<form name='Form10' action='actions/p_catatan_harian.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
				$sql = 	"select a.*,to_char(a.tanggal_reg,'DD MON YYYY HH24:MI:SS')as tanggal_reg,f.layanan,(g.nama)as merawat ".
						...
						"left join rs00017 g on a.vis_5 = 'g.id' ".
			...
			echo"<div class=form_subtitle>DATA CATATAN HARIAN PENYAKIT</div>";
			echo "</td></tr>";
    		echo "<tr><td  valign=top>";
    		$f = new ReadOnlyForm();
			
    		$f->text("<B>".$visit_ri_catatan_harian_perjalanan_penyakit["vis_5"]."</B>",$d["merawat"]);
			$f->text($visit_ri_catatan_harian_perjalanan_penyakit["vis_6"],$d[9]);
    		$f->text($visit_ri_catatan_harian_perjalanan_penyakit["vis_1"],$d[4]);
			$f->text($visit_ri_catatan_harian_perjalanan_penyakit["vis_2"],$d[5]);
			$f->text($visit_ri_catatan_harian_perjalanan_penyakit["vis_3"],$d[6]);
			$f->text($visit_ri_catatan_harian_perjalanan_penyakit["vis_4"],$d[7] );
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT CATATAN HARIAN PENYAKIT</div>";
		//detail riwayat
    	...
    	$sql2 =	"SELECT A.*,(C.NAMA)AS merawat FROM C_VISIT_RI A 
    				LEFT JOIN RS00017 C ON A.VIS_5::text = C.ID::text
    	...
    	$sql2 =	"SELECT A.*,(C.NAMA)AS merawat FROM C_VISIT_RI A 
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						echo "<font color='#000000' size='2'> Edit Catatan Harian Penyakit</font>";
						
						$f = new Form("actions/p_catatan_harian.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			if (isset($_SESSION["SELECT_EMP"])) {
    					$_SESSION["CATATAN_HARIAN"]["id"] = $_SESSION["SELECT_EMP"];
    					$_SESSION["CATATAN_HARIAN"]["nama"] =
        				getFromTable(
			            "select nama from rs00017 where id = '".$_SESSION["CATATAN_HARIAN"]["id"]."'");
    					$f->textAndButton3("f_vis_5","Dokter / Bidan",2,10,$_SESSION["CATATAN_HARIAN"]["id"],$ext,"nm2",30,70,$_SESSION["CATATAN_HARIAN"]["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
			            
					}else{
						$f->textAndButton3("f_vis_5","Dokter / Bidan",2,10,$_GET["tmp7"],$ext,"nm2",30,70,$d2["merawat"],$ext,"...",$ext,"OnClick='selectPegawai();';");
					}
			
			$f->calendar("f_vis_6","Tanggal",10,15,$_GET["tmp1"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);		
			if($d2["vis_1"] != ''){
			$f->textinfo("f_vis_1",$visit_ri_catatan_harian_perjalanan_penyakit["vis_1"],10,10,$_GET["tmp2"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_1",$visit_ri_catatan_harian_perjalanan_penyakit["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			}
			$f->textarea("f_vis_2",$visit_ri_catatan_harian_perjalanan_penyakit["vis_2"] ,1, $visit_ri_catatan_harian_perjalanan_penyakit["vis_2"."W"],$_GET["tmp3"],$ext);
			$f->textarea("f_vis_3",$visit_ri_catatan_harian_perjalanan_penyakit["vis_3"] ,1, $visit_ri_catatan_harian_perjalanan_penyakit["vis_3"."W"],$_GET["tmp4"],$ext);
			$f->text("f_vis_4",$visit_ri_catatan_harian_perjalanan_penyakit["vis_4"],30,50,$_GET["tmp5"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			echo"</div>";
			echo "<div align=right><b>RM.05</b></div>";
					   
				}elseif ($_GET['act'] == "tambah") {
					echo "<font color='#000000' size='2'>Tambah Catatan Harian Penyakit</font>";
					$f = new Form("actions/p_catatan_harian.insert.php", "POST", "NAME=Form2");
					$f->hidden("act","new");
			...
			echo "<table border=1 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'>";
						...
						$f->textAndButton3("f_vis_5","Dokter / Bidan",2,10,"",$ext,"nm2",30,70,"",$ext,"...",$ext,"OnClick='selectPegawai();';");
					}
			
			$f->calendar("f_vis_6","Tanggal",10,15,"","Form2","icon/calendar.gif","Pilih Tanggal",$ext);		
			$f->textinfo("f_vis_1",$visit_ri_catatan_harian_perjalanan_penyakit["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			$f->textarea("f_vis_2",$visit_ri_catatan_harian_perjalanan_penyakit["vis_2"] ,1, $visit_ri_catatan_harian_perjalanan_penyakit["vis_2"."W"],"",$ext);
			$f->textarea("f_vis_3",$visit_ri_catatan_harian_perjalanan_penyakit["vis_3"] ,1, $visit_ri_catatan_harian_perjalanan_penyakit["vis_3"."W"],"",$ext);
			$f->text("f_vis_4",$visit_ri_catatan_harian_perjalanan_penyakit["vis_4"],30,50,"",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			...
			echo "<div align=right><b>RM.05</b></div>";
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$sql = "SELECT A.NO_REG,A.VIS_6,A.VIS_1,C.NAMA,A.VIS_4,A.VIS_2,A.VIS_3,A.tanggal_reg,A.VIS_5,'DUMMY'". 
					   "FROM C_VISIT_RI A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
			    ...
			    $t->ColHidden[9]=true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","DOKTER PEMBERI INSTRUKSI","YANG MENERIMA INSTRUKSI","EDIT");
			   	$t->ColAlign = array("center","center","center","left","left","","","","","center");
				$t->ColFormatHtml[9] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=edit&mr=".$_GET["mr"]."&rg=<#0#>
				&tmp1=<#1#>&tmp2=<#2#>&tmp3=<#5#>&tmp4=<#6#>&tmp5=<#4#>&tmp6=<#7#>&tmp7=<#8#>'>".icon("edit","Edit")."</A>";	
				$t->execute();
				echo "<br>";
			...
			if (isset($_SESSION["SELECT_EMP"])) {
        				...
        				getFromTable(
			            "select nama from rs00017 where id = '".$_SESSION["CATATAN_HARIAN"]["id"]."'");
    					$f->textAndButton3("f_vis_5","Dokter Pemberi Instruksi",2,10,$_SESSION["CATATAN_HARIAN"]["id"],$ext,"nm2",30,70,$_SESSION["CATATAN_HARIAN"]["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
			            
					}else{
						$f->textAndButton3("f_vis_5","Dokter Pemberi Instruksi",2,10,$d2["vis_5"],$ext,"nm2",30,70,$d2["merawat"],$ext,"...",$ext,"OnClick='selectPegawai();';");
					}
			if($d2["vis_6"]!= ''){
				$f->calendar("f_vis_6","Tanggal",10,15,$d2["vis_6"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			}else{
				$f->calendar("f_vis_6","Tanggal",10,15,$tglhariini,"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			}
					
			if($d2["vis_1"] != ''){
			$f->textinfo("f_vis_1",$visit_ri_catatan_harian_perjalanan_penyakit["vis_1"],10,10,$d2["vis_1"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_1",$visit_ri_catatan_harian_perjalanan_penyakit["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			}
			$f->textarea("f_vis_2",$visit_ri_catatan_harian_perjalanan_penyakit["vis_2"] ,1, $visit_ri_catatan_harian_perjalanan_penyakit["vis_2"."W"],$d2["vis_2"],$ext);
			$f->textarea("f_vis_3",$visit_ri_catatan_harian_perjalanan_penyakit["vis_3"] ,1, $visit_ri_catatan_harian_perjalanan_penyakit["vis_3"."W"],$d2["vis_3"],$ext);
			$f->text("f_vis_4",$visit_ri_catatan_harian_perjalanan_penyakit["vis_4"],30,50,$d2["vis_4"],$ext);
