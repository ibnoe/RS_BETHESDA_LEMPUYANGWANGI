...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl100009b where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/radiotherapi.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Radiotherapi");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/radiotherapi.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Radiotherapi");
        $f->hidden("id","new");
    ...
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan Radiotherapi");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select jenis, jumlah, id as href FROM rl100009b order by id";            
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
    ...
    $t->ColFormatHtml[2] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#2#>'>".icon("edit","Edit")."</A>".
            ...
            "actions/radiotherapi.delete.php?p=$PID".
            "&e=<#2#>".
            "'>".icon("delete","Hapus")."</A>".
            "</nobr>"; 
    /*
    ...
    */
	...
	$data = getFromTable("select jenis from rl100009b where id='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/radiotherapi.delete.php' method='get'>";
