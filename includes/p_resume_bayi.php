...
/*	$tab_disabled = array("pemeriksaan"=>true, "layanan"=>true, "icd"=>true, "riwayat"=>true,"riwayat_klinik"=>true);
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Input Resume Bayi"	, $tab_disabled["pemeriksaan"]);
	...
	$T->addTab("$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Resume Bayi"	, $tab_disabled["riwayat"]);
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_resume_bayi.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_resume_bayi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
	...
	if ($rec1 > 0) {
     ...
     /*   if (isset($_SESSION["SELECT_LAYANAN"]) && $is_range) {
        ...
        echo "<form name='Form10' action='actions/p_resume_bayi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT RESUME BAYI</div>";
		//detail riwayat
		...
		$sql = "SELECT A.NO_REG,to_char(A.TANGGAL_REG,'DD MON YYYY HH24:MI:SS')as tgl_reg,(G.NAMA)AS MERAWAT,(H.NAMA)AS MENGIRIM,'DUMMY' ". 
					   ...
					   "LEFT JOIN RS00017 G ON CAST (A.id_dokter AS INTEGER) = G.ID ".
					   "LEFT JOIN RS00017 H ON CAST (A.id_perawat AS INTEGER) = H.ID ".
...
//====iqbal tambah tab konsultasi untuk rawat inap 15/02/12====	
    	...
    	$T->show(0);
    		...
    		$sql2 =	"SELECT A.*,(C.NAMA)AS nama,(D.NAMA)AS perawat FROM C_VISIT_RI A 
    				LEFT JOIN RS00017 C ON CAST (A.id_dokter AS INTEGER) = C.ID
    				LEFT JOIN RS00017 D ON CAST (A.id_perawat as INTEGER) = D.ID
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_resume_bayi.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			$f->text("f_vis_3",$visit_ri_resume_bayi["vis_3"],50,50,$d2["vis_3"],$ext);
			$f->title1("<U>DIAGNOSA</U>");
				$f->textarea("f_vis_4",$visit_ri_resume_bayi["vis_4"] ,1, $visit_ri_resume_bayi["vis_4"."W"],$d2["vis_4"],$ext);
			$f->textarea("f_vis_5",$visit_ri_resume_bayi["vis_5"] ,1, $visit_ri_resume_bayi["vis_5"."W"],$d2["vis_5"],$ext);
			$f->textarea("f_vis_6",$visit_ri_resume_bayi["vis_6"] ,1, $visit_ri_resume_bayi["vis_6"."W"],$d2["vis_6"],$ext);
			$f->title1("<U>RIWAYAT KELAHIRAN</U>");
			$f->textinfo("f_vis_7",$visit_ri_resume_bayi["vis_7"],15,15,$d2["vis_7"],"Minggu",$ext);
			if($d2["vis_24"]=="Belakang-Kepala"){
			    	$f->checkbox2($visit_ri_resume_bayi["vis_24"],"f_vis_24","Belakang Kepala","Belakang-Kepala","CHECKED","Sungsang Lahir","Sungsang-Lahir","",$ext);
			    }elseif ($d2["vis_24"]=="Sungsang-Lahir"){
			    	$f->checkbox2($visit_ri_resume_bayi["vis_24"],"f_vis_24","Belakang Kepala","Belakang-Kepala","","Sungsang Lahir","Sungsang-Lahir","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_resume_bayi["vis_24"],"f_vis_24","Belakang Kepala","Belakang-Kepala","","Sungsang Lahir","Sungsang-Lahir","",$ext);
			    }
			$f->calendar("f_vis_8",$visit_ri_resume_bayi["vis_8"],15,15,$d2["vis_8"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			if($d2["vis_9"] != ''){
			$f->textinfo("f_vis_9",$visit_ri_resume_bayi["vis_9"],15,15,$d2["vis_9"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_9",$visit_ri_resume_bayi["vis_9"],15,15,"00:00","(Jam:Menit, 08:09)",$ext);	
			}
			
			$f->text("f_vis_10",$visit_ri_resume_bayi["vis_10"],50,50,$d2["vis_10"],$ext);
			$f->text("f_vis_11",$visit_ri_resume_bayi["vis_11"],10,10,$d2["vis_11"],$ext);
			$f->text("f_vis_12",$visit_ri_resume_bayi["vis_12"],50,50,$d2["vis_12"],$ext);
			$f->title1("<U>PEMERIKSAAN</U>");
			if($d2["vis_25"]=="Laki-Laki"){
			    	$f->checkbox2($visit_ri_resume_bayi["vis_25"],"f_vis_25","Laki-Laki","Laki-Laki","CHECKED","Perempuan","Perempuan","",$ext);
			    }elseif ($d2["vis_25"]=="Perempuan"){
			    	$f->checkbox2($visit_ri_resume_bayi["vis_25"],"f_vis_25","Laki-Laki","Laki-Laki","","Perempuan","Perempuan","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_resume_bayi["vis_25"],"f_vis_25","Laki-Laki","Laki-Laki","","Perempuan","Perempuan","",$ext);
			    }
			$f->textinfo("f_vis_13",$visit_ri_resume_bayi["vis_13"],15,15,$d2["vis_13"],"Gram",$ext);
			$f->textinfo("f_vis_14",$visit_ri_resume_bayi["vis_14"],15,15,$d2["vis_14"],"Cm",$ext);
			$f->textinfo("f_vis_15",$visit_ri_resume_bayi["vis_15"],15,15,$d2["vis_15"],"Cm",$ext);
			$f->text("f_vis_16",$visit_ri_resume_bayi["vis_16"],50,50,$d2["vis_16"],$ext);
			$f->textarea("f_vis_17",$visit_ri_resume_bayi["vis_17"] ,1, $visit_ri_resume_bayi["vis_17"."W"],$d2["vis_17"],$ext);
			$f->textarea("f_vis_18",$visit_ri_resume_bayi["vis_18"] ,1, $visit_ri_resume_bayi["vis_18"."W"],$d2["vis_18"],$ext);
			$f->title1("<U>TINDAK LANJUT (FOLLOW UP)</U>");
			$f->text("f_vis_26",$visit_ri_resume_bayi["vis_26"],70,100,$d2["vis_26"],$ext);
			$f->text("f_vis_19",$visit_ri_resume_bayi["vis_19"],50,50,$d2["vis_19"],$ext);
			$f->text("f_vis_21",$visit_ri_resume_bayi["vis_21"],10,10,$d2["vis_21"],$ext);
			$f->textinfo("f_vis_20",$visit_ri_resume_bayi["vis_20"],15,15,$d2["vis_20"],"Gram",$ext);
			$f->text("f_vis_22",$visit_ri_resume_bayi["vis_22"],10,10,$d2["vis_22"],$ext);
			$f->textinfo("f_vis_23",$visit_ri_resume_bayi["vis_23"],15,15,$d2["vis_23"],"Gram",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			echo"</div>";
			echo "<div align=right><b>RM x.h</b></div>";
