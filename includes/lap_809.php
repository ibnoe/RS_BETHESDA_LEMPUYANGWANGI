...
$is_selected = getFromTable(
    ...
    // search box
    if (!$GLOBALS['print']){
    	title_print("<img src='icon/informasi-2.gif' align='absmiddle' > Laporan Data Pegawai");
		
		title_excel("lap_809&mPEG=".$_GET["mPEG"]."&mJAB=".$_GET["mJAB"]."");		
	    } else {
    	title("<img src='icon/informasi-2.gif' align='absmiddle' > Laporan Data Pegawai");
    }
...
*/
    ...
    if ($is_selected) {
		if (!$GLOBALS['print']){
		echo "<br><DIV ALIGN=CENTER><TABLE BORDER=0><FORM ACTION=$SC NAME=Form2 ><TR>";
        echo "<TD ALIGN=CENTER CLASS=TBL_JUDUL>DATA PEGAWAI RSUD Dr. ACHMAD MOCHTAR BUKITTINGGI</TD>";
        echo "</TR><TR><TD ALIGN=CENTER CLASS=TBL_JUDUL>BULAN ".$bln." </TD>";
        echo "</TR><tr>";
		
		echo "</tr></FORM></TABLE></DIV> <br>";
		}
        $t = new PgTable($con, "100%");
        $t->SQL =
            "select a.nama,a.nip, c.tdesc as gol,a.pangkat, to_char(a.tmt_pangkat,'dd/mm/yyyy') as tmt_pangkat,a.jabatan ,to_char(a.tmt_jabatan,'dd/mm/yyyy') as tmt_jabatan,
					b.tdesc || ' - ' ||f.jabatan_medis_fungsional  as pendidikan,to_char(a.tmt_cpns,'dd/mm/yyyy') as tmt_cpns,a.tempat_lahir, to_char(tanggal_lahir,'DD MON YYYY') as lahir, 
						case when a.jns_kelamin = 'L' then 'Laki-laki'
						 when a.jns_kelamin = 'P' then 'Perempuan'
					end as jns_kelamin, a.alamat,a.phone
				from rs00017 a 
            ...
            "where a.jabatan_medis_fungsional_id='".$_GET["mJAB"]."' ".
				...
				group by a.nama,a.nip,gol,a.pangkat, tmt_pangkat,a.jabatan ,tmt_jabatan,
				pendidikan,tmt_cpns, lahir,  jns_kelamin, a.alamat,a.phone,a.tempat_lahir	";
        ...
        $t->RowsPerPage = 500;
        $t->ColHeader = Array( "NAMA","NRP/NIP","GOL", "PANGKAT","TMT<br>PANGKAT","JABATAN","TMT<br>JABATAN","PENDIDIKAN", "TMT<br>CPNS","TEMPAT<br>LAHIR","TANGGAL<br>LAHIR",
								"JENIS<br>KELAMIN","ALAMAT","TELEPON");
        $t->execute();
