...
/*	$tab_disabled = array("pemeriksaan"=>true, "layanan"=>true, "icd"=>true, "riwayat"=>true,"riwayat_klinik"=>true);
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Input Resume Pasien"	, $tab_disabled["pemeriksaan"]);
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_resume_keb.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_resume_keb.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
    ...
    /*    if (isset($_SESSION["SELECT_LAYANAN"]) && $is_range) {
        ...
        echo "<form name='Form10' action='actions/p_resume_keb.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
    	...
    	if ($_GET["act"] == "detail") {
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT RESUME KEBIDANAN</div>";
		//detail riwayat
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,to_char(A.TANGGAL_REG,'DD MON YYYY HH24:MI:SS')as tgl_reg,G.NAMA AS MERAWAT,H.NAMA AS MENGIRIM,'DUMMY' ". 
					   ...
					   "WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' 
                                           group by A.NO_REG,tgl_reg,MERAWAT,MENGIRIM";
				$t = new PgTable($con, "100%");
			    $t->SQL = $sql ;
			    $t->setlocale("id_ID");
			    $t->ShowRowNumber = true;
			   	//$t->ColHidden[4]= true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL/JAM KUNJUNGAN","DOKTER YANG MERAWAT","DOKTER PENGIRIM","DETAIL");
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,TO_CHAR(A.TANGGAL_REG,'DD MON YYYY')AS TANGGAL,TO_CHAR(A.TANGGAL_REG,'HH:MM:SS')AS WAKTU,C.TDESC,D.NAMA,'DUMMY' ". 
					   ...
					   "WHERE B.MR_NO = '".$_GET["mr"]."'
                                               group by A.NO_REG,TANGGAL,WAKTU,C.TDESC,D.NAMA ";
...
//====iqbal tambah tab konsultasi untuk rawat inap 15/02/12====
    	...
    	$T->show(0);
			...
			/* $sql2 =	"SELECT A.*,(C.NAMA)AS merawat,(D.NAMA)AS mengirim FROM C_VISIT_RI A 
    				LEFT JOIN RS00017 C ON A.VIS_1 = C.ID::text
					LEFT JOIN RS00017 D ON A.VIS_2 = D.ID::text
    				WHERE A.ID_RI='{$_GET["ri"]}' AND A.NO_REG='{$_GET["rg1"]}'"; */
			$r=pg_query($con,$sql2);
			$n = pg_num_rows($r);
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_resume_keb.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			if (isset($_SESSION["SELECT_EMP"])) {
    					$_SESSION["RESUME_KEB"]["id"] = $_SESSION["SELECT_EMP"];
    					$_SESSION["RESUME_KEB"]["nama"] =
        				getFromTable(
			            "select nama from rs00017 where id = '".$_SESSION["RESUME_KEB"]["id"]."'");
    					$f->textAndButton3("f_id_dokter","Dokter Yang Merawat",2,10,$_SESSION["RESUME_KEB"]["id"],$ext,"nm2",30,70,$_SESSION["RESUME_KEB"]["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
			            
					
                    }elseif ($d2["id_dokter"] != '') {
							$f->textAndButton3("f_id_dokter","Dokter Yang Merawat",2,10,$d2["id_dokter"],$ext,"nm2",30,70,$d2["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
					}else{
			...
			if (isset($_SESSION["SELECT_EMP2"])) {
    					$_SESSION["RESUME_KEB"]["id2"] = $_SESSION["SELECT_EMP2"];
    					$_SESSION["RESUME_KEB"]["nama2"] =
        				getFromTable(
			            "select nama from rs00017 where id = '".$_SESSION["RESUME_KEB"]["id2"]."'");
    					$f->textAndButton3("f_id_perawat","Dokter Yang Mengirim",2,10,$_SESSION["RESUME_KEB"]["id2"],$ext,"nm3",30,70,$_SESSION["RESUME_KEB"]["nama2"],$ext,"...",$ext,"OnClick='selectPegawai2();';");
			            
					}elseif ($d2["id_perawat"] != '') {
						$f->textAndButton3("f_id_perawat","Dokter Yang Mengirim",2,10,$d2["id_perawat"],$ext,"nm3",30,70,$d2["perawat"],$ext,"...",$ext,"OnClick='selectPegawai2();';");
					}else{
						$f->textAndButton3("f_id_perawat","Dokter Yang Mengirim",2,10,0,$ext,"nm3",30,70,$d2["perawat"],$ext,"...",$ext,"OnClick='selectPegawai2();';");
					}
                                        
			$f->text("f_vis_43",$visit_ri_resume_keb["vis_43"],50,50,$d2["vis_43"],$ext);
			$f->title1("<U>DIAGNOSA OBSTETRIK</U>");
			if($d2["vis_41"] != ''){
			$f->textinfo("f_vis_41",$visit_ri_resume_keb["vis_1"],15,15,$d2["vis_41"],"(Jam:Menit, 08:09",$ext);
			}else{
			$f->textinfo("f_vis_41",$visit_ri_resume_keb["vis_1"],15,15,"00:00","(Jam:Menit, 08:09",$ext);	
			}
			if($d2["vis_42"] != ''){
			$f->textinfo("f_vis_42",$visit_ri_resume_keb["vis_2"],15,15,$d2["vis_42"],"(Jam:Menit, 08:09",$ext);
			}else{
			$f->textinfo("f_vis_42",$visit_ri_resume_keb["vis_2"],15,15,"00:00","(Jam:Menit, 08:09",$ext);	
			}
			
			$f->textarea("f_vis_3",$visit_ri_resume_keb["vis_3"] ,1, $visit_ri_resume_keb["vis_3"."W"],$d2["vis_3"],$ext);
			$f->calendar("f_vis_4",$visit_ri_resume_keb["vis_4"],15,15,$d2["vis_4"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			if($d2["vis_5"]=="Laki-Laki"){
			    	$f->checkbox2($visit_ri_resume_keb["vis_5"],"f_vis_5","Laki-Laki","Laki-Laki","CHECKED","Perempuan","Perempuan","",$ext);
			    }elseif ($d2["vis_5"]=="Perempuan"){
			    	$f->checkbox2($visit_ri_resume_keb["vis_5"],"f_vis_5","Laki-Laki","Laki-Laki","","Perempuan","Perempuan","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_resume_keb["vis_5"],"f_vis_5","Laki-Laki","Laki-Laki","","Perempuan","Perempuan","",$ext);
			    }
			
			$f->textinfo("f_vis_6",$visit_ri_resume_keb["vis_6"],15,15,$d2["vis_6"],"Gram",$ext);
			$f->textinfo("f_vis_7",$visit_ri_resume_keb["vis_7"],15,15,$d2["vis_7"],"Cm",$ext);
			if($d2["vis_8"]=="Normal"){
			    	$f->checkbox2($visit_ri_resume_keb["vis_8"],"f_vis_8","Normal","Normal","CHECKED","Kelainan","Kelainan","",$ext);
			    }elseif ($d2["vis_8"]=="Kelainan"){
			    	$f->checkbox2($visit_ri_resume_keb["vis_8"],"f_vis_8","Normal","Normal","","Kelainan","Kelainan","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_resume_keb["vis_8"],"f_vis_8","Normal","Normal","","Kelainan","Kelainan","",$ext);
			    }
			$f->title1("<U>RIWAYAT OBSTETRIK / RIWAYAT PENYAKIT TERDAHULU</U>");
			$f->textarea("f_vis_9",$visit_ri_resume_keb["vis_9"] ,1, $visit_ri_resume_keb["vis_9"."W"],$d2["vis_9"],$ext);
			$f->textarea("f_vis_10",$visit_ri_resume_keb["vis_10"] ,1, $visit_ri_resume_keb["vis_10"."W"],$d2["vis_10"],$ext);
			$f->title1("<U>RIWAYAT KEHAMILAN SEKARANG</U>");
			$f->calendar("f_vis_11",$visit_ri_resume_keb["vis_11"],15,15,$d2["vis_11"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$f->textinfo("f_vis_12",$visit_ri_resume_keb["vis_12"],15,15,$d2["vis_12"],"Hari",$ext);
			$f->calendar("f_vis_13",$visit_ri_resume_keb["vis_13"],15,15,$d2["vis_13"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$f->text("f_vis_14",$visit_ri_resume_keb["vis_14"],70,100,$d2["vis_14"],$ext);
			$f->text("f_vis_15",$visit_ri_resume_keb["vis_15"],70,100,$d2["vis_15"],$ext);
			$f->text("f_vis_16",$visit_ri_resume_keb["vis_16"],70,100,$d2["vis_16"],$ext);
			$f->title1("<U>HASIL LABORATORIUM</U>");
			$f->text_8i("f_vis_17",$visit_ri_resume_keb["vis_17"],20,30,$d2["vis_17"],"g%","f_vis_18",$visit_ri_resume_keb["vis_18"],20,30,$d2["vis_18"],"",
			"f_vis_19",$visit_ri_resume_keb["vis_19"],20,30,$d2["vis_19"],"","f_vis_20",$visit_ri_resume_keb["vis_20"],20,30,$d2["vis_20"],"/mm",
			"f_vis_21",$visit_ri_resume_keb["vis_21"],20,30,$d2["vis_21"],"","f_vis_22",$visit_ri_resume_keb["vis_22"],20,30,$d2["vis_22"],"",
			"f_vis_23",$visit_ri_resume_keb["vis_23"],20,30,$d2["vis_23"],"","f_vis_24",$visit_ri_resume_keb["vis_24"],20,30,$d2["vis_24"],"",$ext);
			$f->text("f_vis_25",$visit_ri_resume_keb["vis_25"],21,30,$d2["vis_25"],$ext);
			$f->text("f_vis_26",$visit_ri_resume_keb["vis_26"],30,30,$d2["vis_26"],$ext);
			$f->text("f_vis_27",$visit_ri_resume_keb["vis_27"],30,30,$d2["vis_27"],$ext);
			$f->title1("<U>RIWAYAT PERSALINAN SEKARANG</U>");
			$f->title1("Kala 1");
			$f->text("f_vis_28",$visit_ri_resume_keb["vis_28"],21,30,$d2["vis_28"],$ext);
			$f->text("f_vis_29",$visit_ri_resume_keb["vis_29"],30,30,$d2["vis_29"],$ext);
			$f->textarea("f_vis_30",$visit_ri_resume_keb["vis_30"] ,1, $visit_ri_resume_keb["vis_30"."W"],$d2["vis_30"],$ext);
			if($d2["vis_31"]=="Ada"){
			    	$f->checkbox2($visit_ri_resume_keb["vis_31"],"f_vis_31","Ada","Ada","CHECKED","Tidak Ada","Tidak-Ada","",$ext);
			    }elseif ($d2["vis_31"]=="Tidak-Ada"){
			    	$f->checkbox2($visit_ri_resume_keb["vis_31"],"f_vis_31","Ada","Ada","","Tidak Ada","Tidak-Ada","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_resume_keb["vis_31"],"f_vis_31","Ada","Ada","","Tidak Ada","Tidak-Ada","",$ext);
			    }
			$f->title1("Kala 2");
			$f->text("f_vis_32",$visit_ri_resume_keb["vis_32"],21,30,$d2["vis_32"],$ext);
			$f->text("f_vis_33",$visit_ri_resume_keb["vis_33"],30,30,$d2["vis_33"],$ext);
			$f->textarea("f_vis_34",$visit_ri_resume_keb["vis_34"] ,1, $visit_ri_resume_keb["vis_34"."W"],$d2["vis_34"],$ext);
			$f->title1("Kala 3");
			$f->text("f_vis_35",$visit_ri_resume_keb["vis_35"],21,30,$d2["vis_35"],$ext);
			$f->text("f_vis_36",$visit_ri_resume_keb["vis_36"],30,30,$d2["vis_36"],$ext);
			$f->textarea("f_vis_37",$visit_ri_resume_keb["vis_37"] ,1, $visit_ri_resume_keb["vis_37"."W"],$d2["vis_37"],$ext);
			$f->title1("Kala 4");
			$f->text("f_vis_38",$visit_ri_resume_keb["vis_38"],21,30,$d2["vis_38"],$ext);
			$f->text("f_vis_39",$visit_ri_resume_keb["vis_39"],30,30,$d2["vis_39"],$ext);
			$f->textarea("f_vis_40",$visit_ri_resume_keb["vis_40"] ,1, $visit_ri_resume_keb["vis_40"."W"],$d2["vis_40"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			echo"</div>";
			echo "<div align=right><b>RM x.a</b></div>";
