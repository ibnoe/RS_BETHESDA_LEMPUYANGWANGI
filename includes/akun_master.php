...
if ($_GET["action"] == "new") {
    ...
    if ($_GET["grp"] == "Y") {
		...
		$f->selectArray("f_akun_type", "DB/CR",Array(""=>"", "Debet" => "Debet", "Kredit" => "Kredit"),$_GET["akun_type"]);
    } else {
		$f->hidden("f_kode","Kode Akun",50,255,$_GET["kode"]);
    ...
    if ($_GET["grp"] == "Y") {
        ...
        $f->selectArray("f_akun_type", "DB/CR",Array(""=>"", "Debet" => "Debet", "Kredit" => "Kredit"),$d->akun_type);
    } else {
		$f->hidden("f_kode","Kode Akun",50,255,$d->kode);
...
if (empty($_GET[sure])) {
    ...
    $SQL1 = "select a.kode, a.nama, a.akun_type, a.id ".
    ...
    $SQL1Counter =
            ...
            "and substr(a.hierarchy,".(($level*3)+4).",".(15-(($level*3)+3)).") = '".
            str_repeat("0",15-(($level*3)+3))."'";
    $SQL2 = "select a.kode, a.nama, a.akun_type, a.id ".
            "from akun_master as a ".
            "where substr(a.hierarchy,1,".($level*3).") = '".substr($_GET["L$level"],0,($level*3))."' ".
            "and a.hierarchy <> '".$_GET["L$level"]."' ".
            "and substr(a.hierarchy,".(($level*3)+4).",".(15-(($level*3)+3)).") = '".
 ...
 //   echo "<input type=submit value=' CARI '>";
    ...
    if ($isGroup == "Y") {
        ...
        $t->ColFormatHtml[3] =
            ...
            "$SC?p=$PID&action=edit&parent=".$_GET["L$level"]."&grp=Y&e=<#3#>".
            "'>".icon("edit","Edit")."</A> <A CLASS=TBL_HREF HREF='".
            "actions/akun_master.delete.php?grp=Y&e=<#3#>".
            "'>".icon("delete","Hapus")."</A>";
        $t->ColHeader = Array("KODE","GRUP AKUN","DEBET/KREDIT","ACTION");
        $t->ColAlign[2] = "CENTER";
    ...
    if ($isGroup == "N") {
        ...
        */
        ...
        if (isset($_GET[L1])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/akun_master.delete.php?grp=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#3#>&L1=".$_GET[L1]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L2])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/akun_master.delete.php?grp=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#3#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L3])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/akun_master.delete.php?grp=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#3#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&L3=".$_GET[L3]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L4])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/akun_master.delete.php?grp=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#3#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&L3=".$_GET[L3]."&L4=".$_GET[L4]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L5])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/akun_master.delete.php?grp=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#3#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&L3=".$_GET[L3]."&L4=".$_GET[L4]."&L5=".$_GET[L5]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        $del_link .= "</a>";
        $t->ColFormatHtml[3] =
            "<A CLASS=TBL_HREF HREF='".
            "$SC?p=$PID&action=edit&parent=".$_GET["L$level"]."&grp=N&e=<#3#>&sort=".$_GET[sort]."&order=".$_GET[order]."&tblstart=".$_GET[tblstart].
            "'>".icon("edit","Edit")."</A> $del_link";
		$t->ColAlign[2] = "CENTER";
        $t->ColHeader = Array("KODE", "NAMA", "DEBET/KREDIT",  "ACTION<br><img src='images/spacer.gif' width=50 height=1 border=0>");
        //$t->ShowSQLExecTime = true;
        //$t->ShowSQL = true;
        if (!isset($_GET[sort])) {
