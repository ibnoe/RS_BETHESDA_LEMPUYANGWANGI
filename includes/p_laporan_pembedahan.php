...
$reg1 = (int) $_GET["rg1"];
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Input Data Pembedahan"	, $tab_disabled["pemeriksaan"]);
	...
	$T->addTab("$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Pembedahan"	, $tab_disabled["riwayat"]);
...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                   title_excel("p_laporan_pembedahan&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");

          // &rg1=.$_GET['rg1']&rg=.$_GET['rg']&ri=.$_GET['ri']&mr=.$_GET['mr']);
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_laporan_pembedahan.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(2);
        ...
        echo "<form name='Form9' action='actions/p_laporan_pembedahan.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
	...
	if ($rec1 > 0) {
        ...
        echo "<form name='Form10' action='actions/p_laporan_pembedahan.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
			...
			echo"<div class=form_subtitle>DATA PEMBEDAHAN PASIEN</div>";
			echo "</td></tr>";
    		echo "<tr><td  valign=top>";
    		$f = new ReadOnlyForm();
			
			$f->text($visit_ri_laporan_pembedahan["vis_1"],$d[4] );
			$f->text($visit_ri_laporan_pembedahan["vis_2"],$d[5]);
			$f->text($visit_ri_laporan_pembedahan["vis_3"],$d[6]);
			$f->text($visit_ri_laporan_pembedahan["vis_4"],$d[7] );
			$f->text($visit_ri_laporan_pembedahan["vis_5"],$d[8]);
			$f->text($visit_ri_laporan_pembedahan["vis_6"],$d[9]);
			$f->text($visit_ri_laporan_pembedahan["vis_7"],$d[10]);
			$f->text($visit_ri_laporan_pembedahan["vis_8"],$d[11] );
			$f->text($visit_ri_laporan_pembedahan["vis_9"],$d[12].",&nbsp;".$d[13].",&nbsp;".$d[14]);
			$f->text($visit_ri_laporan_pembedahan["vis_12"],$d[15]);
			$f->text($visit_ri_laporan_pembedahan["vis_13"],$d[16]);
			$f->text($visit_ri_laporan_pembedahan["vis_14"],$d[17]);
			$f->text($visit_ri_laporan_pembedahan["vis_15"],$d[18]);
			$f->text($visit_ri_laporan_pembedahan["vis_16"],$d[19]);
			$f->execute();	
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT PEMBEDAHAN</div>";
		//detail riwayat
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
		
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,A.VIS_12,A.VIS_13,A.VIS_1,A.VIS_2,'DUMMY' ". 
					   "FROM C_VISIT_RI A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					   "WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' 
                                           group by A.NO_REG,A.VIS_12,A.VIS_13,A.VIS_1,A.VIS_2 ";
				$t = new PgTable($con, "100%");
			    $t->SQL = $sql ;
			    $t->setlocale("id_ID");
			    $t->ShowRowNumber = true;
			   	//$t->ColHidden[4]= true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","MULAI (PUKUL)","SELESAI (PUKUL)","BEDAH","ASISTEN BEDAH","DETAIL");
			   	$t->ColAlign = array("center","center","center","left","left","center");
				$t->ColFormatHtml[5] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=detail&mr=".$_GET["mr"]."&rg=<#0#>'>".icon("view","View")."</A>";	
				$t->execute();
    	...
    	$T->show(0);
    		...
    		$sql2 =	"SELECT * FROM C_VISIT_RI  
    				WHERE ID_RI='{$_GET["ri"]}' AND NO_REG='{$_GET["rg1"]}'";
			$r=pg_query($con,$sql2);
			$n = pg_num_rows($r);
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_laporan_pembedahan.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			$f->text("f_vis_1",$visit_ri_laporan_pembedahan["vis_1"],50,100,$d2["vis_1"],$ext);
			$f->text("f_vis_2",$visit_ri_laporan_pembedahan["vis_2"],30,100,$d2["vis_2"],$ext);
			$f->text("f_vis_5",$visit_ri_laporan_pembedahan["vis_5"],30,100,$d2["vis_5"],$ext);
			$f->text("f_vis_3",$visit_ri_laporan_pembedahan["vis_3"],50,100,$d2["vis_3"],$ext);
			$f->text("f_vis_4",$visit_ri_laporan_pembedahan["vis_4"],30,100,$d2["vis_4"],$ext);
			$f->text("f_vis_6",$visit_ri_laporan_pembedahan["vis_6"],50,100,$d2["vis_6"],$ext);
			$f->textarea("f_vis_7",$visit_ri_laporan_pembedahan["vis_7"] ,1, $visit_ri_laporan_pembedahan["vis_7"."W"],$d2["vis_7"],$ext);
			$f->text("f_vis_8",$visit_ri_laporan_pembedahan["vis_8"],50,100,$d2["vis_8"],$ext);
			if($d2["vis_9"]=="Akut"){
			    	$f->checkbox2($visit_ri_laporan_pembedahan["vis_9"],"f_vis_9","Akut","Akut","CHECKED","Non Akut","Non.Akut","",$ext);
			    }elseif ($d2["vis_9"]=="Non.Akut"){
			    	$f->checkbox2($visit_ri_laporan_pembedahan["vis_9"],"f_vis_9","Akut","Akut","","Non Akut","Non.Akut","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_laporan_pembedahan["vis_9"],"f_vis_9","Akut","Akut","","Non Akut","Non.Akut","",$ext);
			    }
			if($d2["vis_10"]=="Besar"){
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_10"],"f_vis_10","Besar","Besar","CHECKED","Sedang","Sedang","","Kecil","Kecil","",$ext);
			    }elseif ($d2["vis_10"]=="Sedang"){
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_10"],"f_vis_10","Besar","Besar","","Sedang","Sedang","CHECKED","Kecil","Kecil","",$ext);
			    }elseif ($d2["vis_10"]=="Kecil"){
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_10"],"f_vis_10","Besar","Besar","","Sedang","Sedang","","Kecil","Kecil","CHECKED",$ext);
			    }else {
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_10"],"f_vis_10","Besar","Besar","","Sedang","Sedang","","Kecil","Kecil","",$ext);
			    }
			if($d2["vis_11"]=="Berencana"){
			    	$f->checkbox2($visit_ri_laporan_pembedahan["vis_11"],"f_vis_11","Berencana","Berencana","CHECKED","G.Darurat","G.Darurat","",$ext);
			    }elseif ($d2["vis_11"]=="G.Darurat"){
			    	$f->checkbox2($visit_ri_laporan_pembedahan["vis_11"],"f_vis_11","Berencana","Berencana","","Gawat Darurat","G.Darurat","CHECKED",$ext);
			    }else{
			    	$f->checkbox2($visit_ri_laporan_pembedahan["vis_11"],"f_vis_11","Berencana","Berencana","","Gawat Darurat","G.Darurat","",$ext);
			    }
			if($d2["vis_12"] || $d2["vis_13"] != ''){
			$f->textinfo("f_vis_12",$visit_ri_laporan_pembedahan["vis_12"],15,15,$d2["vis_12"],"(Jam:Menit, 08:09)",$ext);
			$f->textinfo("f_vis_13",$visit_ri_laporan_pembedahan["vis_13"],15,15,$d2["vis_13"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_12",$visit_ri_laporan_pembedahan["vis_12"],15,15,"00:00","(Jam:Menit, 08:09)",$ext);
			$f->textinfo("f_vis_13",$visit_ri_laporan_pembedahan["vis_13"],15,15,"00:00","(Jam:Menit, 08:09)",$ext);
			}
			
			$f->textinfo("f_vis_14",$visit_ri_laporan_pembedahan["vis_14"],15,15,"","",$ext);
			if($d2["vis_15"]=="Lokal"){
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_15"],"f_vis_15","Lokal","Lokal","CHECKED","Spinal","Spinal","","Umum","Umum","",$ext);
			    }elseif ($d2["vis_15"]=="Spinal"){
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_15"],"f_vis_15","Lokal","Lokal","","Spinal","Spinal","CHECKED","Umum","Umum","",$ext);
			    }elseif ($d2["vis_15"]=="Umum"){
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_15"],"f_vis_15","Lokal","Lokal","","Spinal","Spinal","","Umum","Umum","CHECKED",$ext);
			    }else {
			    	$f->checkbox3i($visit_ri_laporan_pembedahan["vis_15"],"f_vis_15","Lokal","Lokal","","Spinal","Spinal","","Umum","Umum","",$ext);
			    }
			$f->textarea("f_vis_16",$visit_ri_laporan_pembedahan["vis_16"] ,3, $visit_ri_laporan_pembedahan["vis_16"."W"],$d2["vis_16"],$ext);
			$f->info("","(*)<I> Uraian dimulai dari bagian tubuh yang dibedah, cara penemuan tindakan yang dilakukan,");
			$f->info("","<I>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;explorasi, indikasi dan tindakan macam penutup luka, dengan lengkap dan jelas, jaringan");
			$f->info("","<I>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;yang di keluarkan drainage, darah yang keluar.");
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			echo"</div>";
			echo "<div align=right><b>RM 07 a.1</b></div>";
