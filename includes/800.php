...
$r = pg_query($con,"select * from rs00001 where tt='".$_GET["tt"]."' and tc='000'");
$d = pg_fetch_object($r);
pg_free_result($r);
...
if(strlen($_GET["e"]) > 0 || strlen($_GET["tc"]) > 0) {
    if($_GET["e"] == "new") {
        if (strlen($_GET["tc"]) == 0) {
            $r8 = pg_query($con,"select max(tc) as tc from rs00001 where tt='".$_GET["tt"]."'");
            $d8 = pg_fetch_object($r8);
            pg_free_result($r8);
            $_GET["tc"] = str_pad(((int) $d8->tc) + 1, 3, "0", STR_PAD_LEFT);
        }
        $f = new Form("actions/800.insert.php");
        $f->hidden("tt",$_GET["tt"]);
        ...
        pg_free_result($r2);
        $f = new Form("actions/800.update.php");
        $f->hidden("tt",$_GET["tt"]);
        $f->hidden("tc",$_GET["e"]);
    ...
    if(strlen($_GET["tdesc"]) > 0) {
		...
		if ($_GET["tt"]=="GOB"){
			$f->text("comment","Jasa Resep",10,50,$d2->comment);
			$f->text("tc_poli","Jasa Racikan",10,50,$d2->tc_poli);
		}
    }
...
//    echo "<TD WIDTH=1><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select tdesc, tc as dummy from rs00001 where tt='".$_GET["tt"]."' and tc!='000'".
   ...
   "<A CLASS=TBL_HREF HREF='actions/800.delete.php?p=$PID&tt=$d->tt&tc=<#1#>'>".icon("delete","Hapus")."</A>"; 
    ...
    echo "<form action='actions/800.delete.php' method='get'>";
