<?php 	// Nugraha, Sun Apr 18 18:58:42 WIT 2004
        ...
        // rs00006.is_bayar = 'N'
        // sfdn, 27-12-2006
		// Agung Sunandar 14:44 06/06/2012 membetulkan tab riwayat
...
// Tambahan BHP
$POLI=$setting_poli["gizi"];
// ======================================


include ("session.php");
echo "<table border=0 width='100%'><tr><td>";
title_print("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK GIZI");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_gizi.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
 ...
 include("rincianobat.php"); 
    	...
    	$T->show(0);
	    	...
	    	$n = pg_num_rows($r);		    	
				...
				if ($_GET['act'] == "edit"){
						...
						$f = new Form("actions/p_gizi.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
					...
					$max = count($visit_gizi) ; 
					$i = 1;
					while ($i<= $max) {		
					if 		($visit_gizi["vis_".$i."F"] == "edit1") {
        					$f->text_gizi("NOMENKLATUR gizi (FDI)","IMG BORDER=0 SRC='images/no_kotak.gif'","f_vis_1",$visit_gizi["vis_1"],38,50,ucfirst($d2[2+$i]),$ext);
							
					}elseif ($visit_gizi["vis_".$i."F"] == "edit2") {
        					$f->text_gizi("ODONTOGRAM (FDI)","IMG BORDER=0 SRC='images/gizi1.gif'","f_vis_2",$visit_gizi["vis_2"],38,50,ucfirst($d2[2+$i]),$ext);
			...
			//Febri 24112012
			$f->textarea("f_vis_6","Keluhan Utama" ,1, 50,ucfirst($d2["vis_6"]),$ext);
    ...
    $f->hidden("list2",tab1);
	    ...
	    if (!$GLOBALS['print']){
	    	...
	    	//title_excel("p_gizi&list=".$_GET['list']."&rg=".$_GET['rg']."&poli=".$_GET['poli']."&mr=".$_GET['mr']."&tblstart=".$_GET['tblstart']."");
		}else { 
		   	$f->search("search","Pencarian",20,20,$_GET["search"],"icon/ico_find.gif","Cari","disabled");
		   	//title_excel("p_gizi&tblstart=".$_GET['tblstart']);
		}
	    $f->execute();
