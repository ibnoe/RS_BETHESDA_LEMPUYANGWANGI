...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_grafik_ibu&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_grafik_ibu.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_grafik_ibu.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
	...
	if ($rec1 > 0) {
        ...
        echo "<form name='Form10' action='actions/p_grafik_ibu.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,A.VIS_2,A.VIS_3,A.VIS_4,A.VIS_5,A.VIS_6,A.TANGGAL_REG,'DUMMY'". 
					   "FROM C_VISIT_RI A ".
			    ...
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","PERNAFASAN (/MENIT)","NADI (/MENIT)","SUHU (&deg;C)","DETAIL");
			   	$t->ColAlign = array("center","center","left","left","left","left","","center");
				$t->ColFormatHtml[7] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=detail&mr=".$_GET["mr"]."&rg=<#0#>&tgl=<#6#>'>".icon("view","Detail")."</A>";	
				$t->execute();
...
//====iqbal tambah tab konsultasi untuk rawat inap 15/02/12====
    	...
    	$sql2 =	"SELECT A.* FROM C_VISIT_RI A 
    				WHERE A.ID_RI='{$_GET["ri"]}' AND A.NO_REG='{$_GET["rg1"]}'AND A.TANGGAL_REG='{$_GET["tmp37"]}' ";	
    	}else{	
    	$sql2 =	"SELECT A.* FROM C_VISIT_RI A 
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_grafik_ibu.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
						$f->hidden("f_no_reg",$d2["no_reg"]);
					    $f->hidden("f_tanggal_reg",$_GET["tmp37"]);
			...
			$f->textarea("f_vis_1",$visit_ri_grafik_ibu["vis_1"],1,50,$d2["vis_1"],$ext);
			$f->calendar("f_vis_2","Tanggal",10,15,$d2["vis_2"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			if($d2["vis_3"] != ''){
			$f->textinfo("f_vis_3",$visit_ri_grafik_ibu["vis_3"],10,10,$_GET["tmp2"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_3",$visit_ri_grafik_ibu["vis_3"],10,10,"06:00","(Jam:Menit, 08:09)",$ext);	
			}		
			$f->textinfo("f_vis_4",$visit_ri_grafik_ibu["vis_4"],10,10,$_GET["tmp3"],"/Menit",$ext);
			$f->textinfo("f_vis_5",$visit_ri_grafik_ibu["vis_5"],10,10,$_GET["tmp4"],"/Menit",$ext);
			$f->textinfo("f_vis_6",$visit_ri_grafik_ibu["vis_6"],10,10,$_GET["tmp5"],"&deg;C",$ext);
			$f->title1("<U>MEDIKASI</U>","");
			$f->text("f_vis_7",$visit_ri_grafik_ibu["vis_7"],30,50,$_GET["tmp6"],$ext);
			$f->text4("f_vis_8",$visit_ri_grafik_ibu["vis_8"],30,50,$_GET["tmp7"],"f_vis_9",$visit_ri_grafik_ibu["vis_9"],30,50,$_GET["tmp8"],
					  "f_vis_10",$visit_ri_grafik_ibu["vis_10"],30,50,$_GET["tmp9"],"f_vis_11",$visit_ri_grafik_ibu["vis_11"],30,50,$_GET["tmp10"],$ext);
			$f->title1("<U>DOSIS</U>","");
			$f->text("f_vis_12",$visit_ri_grafik_ibu["vis_12"],30,50,$_GET["tmp11"],$ext);
			$f->text4("f_vis_13",$visit_ri_grafik_ibu["vis_13"],30,50,$_GET["tmp12"],"f_vis_14",$visit_ri_grafik_ibu["vis_14"],30,50,$_GET["tmp13"],
					  "f_vis_15",$visit_ri_grafik_ibu["vis_15"],30,50,$_GET["tmp14"],"f_vis_16",$visit_ri_grafik_ibu["vis_16"],30,50,$_GET["tmp15"],$ext);
			$f->title1("<U>PEMERIKSAAN LAIN</U>","");
			$f->text("f_vis_17",$visit_ri_grafik_ibu["vis_17"],30,50,$_GET["tmp16"],$ext);
			$f->text4("f_vis_18",$visit_ri_grafik_ibu["vis_18"],30,50,$_GET["tmp17"],"f_vis_19",$visit_ri_grafik_ibu["vis_19"],30,50,$_GET["tmp18"],
					  "f_vis_20",$visit_ri_grafik_ibu["vis_20"],30,50,$_GET["tmp19"],"f_vis_21",$visit_ri_grafik_ibu["vis_21"],30,50,$_GET["tmp20"],$ext);
			$f->text4("f_vis_22",$visit_ri_grafik_ibu["vis_22"],30,50,$_GET["tmp21"],"f_vis_23",$visit_ri_grafik_ibu["vis_23"],30,50,$_GET["tmp22"],
					  "f_vis_24",$visit_ri_grafik_ibu["vis_24"],30,50,$_GET["tmp23"],"f_vis_25",$visit_ri_grafik_ibu["vis_25"],30,50,$_GET["tmp24"],$ext);
			$f->text("f_vis_26",$visit_ri_grafik_ibu["vis_26"],30,50,$_GET["tmp25"],$ext);
			$f->text("f_vis_27",$visit_ri_grafik_ibu["vis_27"],30,50,$_GET["tmp26"],$ext);
			$f->title1("<U>DARAH</U>");
			$f->text("f_vis_28",$visit_ri_grafik_ibu["vis_28"],30,50,$_GET["tmp27"],$ext);
			$f->text4("f_vis_29",$visit_ri_grafik_ibu["vis_29"],30,50,$_GET["tmp28"],"f_vis_30",$visit_ri_grafik_ibu["vis_30"],30,50,$_GET["tmp29"],
					  "f_vis_31",$visit_ri_grafik_ibu["vis_31"],30,50,$_GET["tmp30"],"f_vis_32",$visit_ri_grafik_ibu["vis_32"],30,50,$_GET["tmp31"],$ext);
			$f->text("f_vis_33",$visit_ri_grafik_ibu["vis_33"],30,50,$_GET["tmp32"],$ext);
			$f->text("f_vis_34",$visit_ri_grafik_ibu["vis_34"],30,50,$_GET["tmp33"],$ext);
			$f->text("f_vis_35",$visit_ri_grafik_ibu["vis_35"],30,50,$_GET["tmp34"],$ext);
			$f->textinfo("f_vis_36",$visit_ri_grafik_ibu["vis_36"],10,10,$_GET["tmp35"],"Kg",$ext);
			$f->textarea("f_vis_37",$visit_ri_grafik_ibu["vis_37"],1,50,$_GET["tmp36"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			...
			$f->textarea("f_vis_1",$visit_ri_grafik_ibu["vis_1"],1,50,$d2["vis_1"],$ext);
			$f->calendar("f_vis_2","Tanggal",10,15,$d2["vis_2"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$f->textinfo("f_vis_3",$visit_ri_grafik_ibu["vis_3"],10,10,"11:00","(Jam:Menit, 08:09)",$ext);			
			$f->textinfo("f_vis_4",$visit_ri_grafik_ibu["vis_4"],10,10,"","/Menit",$ext);
			$f->textinfo("f_vis_5",$visit_ri_grafik_ibu["vis_5"],10,10,"","/Menit",$ext);
			$f->textinfo("f_vis_6",$visit_ri_grafik_ibu["vis_6"],10,10,"","&deg;C",$ext);
			$f->title1("<U>MEDIKASI</U>","");
			$f->text("f_vis_7",$visit_ri_grafik_ibu["vis_7"],30,50,"",$ext);
			$f->text4("f_vis_8",$visit_ri_grafik_ibu["vis_8"],30,50,"","f_vis_9",$visit_ri_grafik_ibu["vis_9"],30,50,"",
					  "f_vis_10",$visit_ri_grafik_ibu["vis_10"],30,50,"","f_vis_11",$visit_ri_grafik_ibu["vis_11"],30,50,"",$ext);
			$f->title1("<U>DOSIS</U>","");
			$f->text("f_vis_12",$visit_ri_grafik_ibu["vis_12"],30,50,"",$ext);
			$f->text4("f_vis_13",$visit_ri_grafik_ibu["vis_13"],30,50,"","f_vis_14",$visit_ri_grafik_ibu["vis_14"],30,50,"",
					  "f_vis_15",$visit_ri_grafik_ibu["vis_15"],30,50,"","f_vis_16",$visit_ri_grafik_ibu["vis_16"],30,50,"",$ext);
			$f->title1("<U>PEMERIKSAAN LAIN</U>","");
			$f->text("f_vis_17",$visit_ri_grafik_ibu["vis_17"],30,50,"",$ext);
			$f->text4("f_vis_18",$visit_ri_grafik_ibu["vis_18"],30,50,"","f_vis_19",$visit_ri_grafik_ibu["vis_19"],30,50,"",
					  "f_vis_20",$visit_ri_grafik_ibu["vis_20"],30,50,"","f_vis_21",$visit_ri_grafik_ibu["vis_21"],30,50,"",$ext);
			$f->text4("f_vis_22",$visit_ri_grafik_ibu["vis_22"],30,50,"","f_vis_23",$visit_ri_grafik_ibu["vis_23"],30,50,"",
					  "f_vis_24",$visit_ri_grafik_ibu["vis_24"],30,50,"","f_vis_25",$visit_ri_grafik_ibu["vis_25"],30,50,"",$ext);
			$f->text("f_vis_26",$visit_ri_grafik_ibu["vis_26"],30,50,"",$ext);
			$f->text("f_vis_27",$visit_ri_grafik_ibu["vis_27"],30,50,"",$ext);
			$f->title1("<U>DARAH</U>");
			$f->text("f_vis_28",$visit_ri_grafik_ibu["vis_28"],30,50,"",$ext);
			$f->text4("f_vis_29",$visit_ri_grafik_ibu["vis_29"],30,50,"","f_vis_30",$visit_ri_grafik_ibu["vis_30"],30,50,"",
					  "f_vis_31",$visit_ri_grafik_ibu["vis_31"],30,50,"","f_vis_32",$visit_ri_grafik_ibu["vis_32"],30,50,"",$ext);
			$f->text("f_vis_33",$visit_ri_grafik_ibu["vis_33"],30,50,"",$ext);
			$f->text("f_vis_34",$visit_ri_grafik_ibu["vis_34"],30,50,"",$ext);
			$f->text("f_vis_35",$visit_ri_grafik_ibu["vis_35"],30,50,"",$ext);
			$f->textinfo("f_vis_36",$visit_ri_grafik_ibu["vis_36"],10,10,$d2["vis_36"],"Kg",$ext);
			$f->textarea("f_vis_37",$visit_ri_grafik_ibu["vis_37"],1,50,"",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			...
			echo "<div align=right><b>RM.01.a</b></div>";
				...
				//---------------------------------------------------------------------------------			
					...
					$sql = "SELECT A.NO_REG,A.VIS_1,A.VIS_2,A.VIS_3,A.VIS_4,A.VIS_5,A.VIS_6,A.VIS_7,A.VIS_8,A.VIS_9,A.VIS_10,A.VIS_11, ".
						   "A.VIS_12,A.VIS_13,A.VIS_14,A.VIS_15,A.VIS_16,A.VIS_17,A.VIS_18,A.VIS_19,A.VIS_20,A.VIS_21,A.VIS_22,A.VIS_23, ".
						   "A.VIS_24,A.VIS_25,A.VIS_26,A.VIS_27,A.VIS_28,A.VIS_29,A.VIS_30,A.VIS_31,A.VIS_32,A.VIS_33,A.VIS_34,A.VIS_35,A.VIS_36,A.VIS_37,A.tanggal_reg,'DUMMY' ". 
					       "FROM C_VISIT_RI A ".
					       "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					       "WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' ";
			    ...
			    $t->ShowRowNumber = true;
			    $t->ColHidden[2]=true;
			    $t->ColHidden[8]=true;$t->ColHidden[9]=true;$t->ColHidden[10]=true;
			    $t->ColHidden[11]=true;$t->ColHidden[12]=true;$t->ColHidden[13]=true;$t->ColHidden[14]=true;$t->ColHidden[15]=true;
			    $t->ColHidden[16]=true;$t->ColHidden[17]=true;$t->ColHidden[18]=true;$t->ColHidden[19]=true;$t->ColHidden[20]=true;
			    $t->ColHidden[21]=true;$t->ColHidden[22]=true;$t->ColHidden[23]=true;$t->ColHidden[24]=true;$t->ColHidden[25]=true;
			    $t->ColHidden[26]=true;$t->ColHidden[27]=true;$t->ColHidden[28]=true;$t->ColHidden[29]=true;$t->ColHidden[30]=true;$t->ColHidden[31]=true;
			    $t->ColHidden[32]=true;$t->ColHidden[33]=true;$t->ColHidden[34]=true;$t->ColHidden[35]=true;$t->ColHidden[36]=true;$t->ColHidden[37]=true;
			    $t->ColHidden[38]=true;$t->ColHidden[39]=true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","","JAM","PERNAFASAN (/Menit)","NADI (/Menit)","SUHU (&deg;C)","EDIT");
			   	$t->ColAlign[0]="center";$t->ColAlign[2]="center";$t->ColAlign[3]="center";$t->ColAlign[4]="left";$t->ColAlign[5]="left"; 
			   	$t->ColAlign[6]="left"; 
			   	$t->ColAlign[39]="center"; 
				$t->ColFormatHtml[39] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=edit&mr=".$_GET["mr"]."&rg=<#0#>
				&tmp0=<#1#>&tmp1=<#2#>&tmp2=<#3#>&tmp3=<#4#>&tmp4=<#5#>&tmp5=<#6#>&tmp6=<#7#>&tmp7=<#8#>&tmp8=<#9#>&tmp9=<#10#>&tmp10=<#11#>	
				&tmp11=<#12#>&tmp12=<#13#>&tmp13=<#14#>&tmp14=<#15#>&tmp15=<#16#>&tmp16=<#17#>&tmp17=<#18#>&tmp18=<#19#>&tmp19=<#20#>&tmp20=<#21#>&tmp21=<#22#>&tmp22=<#23#>
				&tmp23=<#24#>&tmp24=<#25#>&tmp25=<#26#>&tmp26=<#27#>&tmp27=<#28#>&tmp28=<#29#>&tmp29=<#30#>&tmp31=<#32#>&tmp32=<#33#>&tmp33=<#34#>&tmp34=<#35#>&tmp35=<#36#>&tmp36=<#37#>&tmp37=<#38#>'>".icon("edit","Edit")."</A>";	
				$t->execute();
			...
			$f->calendar("f_vis_2","Tanggal",10,15,$d2["vis_2"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			if($d2["vis_3"] != ''){
			$f->textinfo("f_vis_3",$visit_ri_grafik_ibu["vis_3"],10,10,$d2["vis_3"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_3",$visit_ri_grafik_ibu["vis_3"],10,10,"06:00","(Jam:Menit, 08:09)",$ext);	
			}		
			$f->textinfo("f_vis_4",$visit_ri_grafik_ibu["vis_4"],10,10,$d2["vis_4"],"/Menit",$ext);
			$f->textinfo("f_vis_5",$visit_ri_grafik_ibu["vis_5"],10,10,$d2["vis_5"],"/Menit",$ext);
			$f->textinfo("f_vis_6",$visit_ri_grafik_ibu["vis_6"],10,10,$d2["vis_6"],"&deg;C",$ext);
			$f->title1("<U>MEDIKASI</U>","");
			$f->text("f_vis_7",$visit_ri_grafik_ibu["vis_7"],30,50,$d2["vis_7"],$ext);
			$f->text4("f_vis_8",$visit_ri_grafik_ibu["vis_8"],30,50,$d2["vis_8"],"f_vis_9",$visit_ri_grafik_ibu["vis_9"],30,50,$d2["vis_9"],
					  "f_vis_10",$visit_ri_grafik_ibu["vis_10"],30,50,$d2["vis_10"],"f_vis_11",$visit_ri_grafik_ibu["vis_11"],30,50,$d2["vis_11"],$ext);
			$f->title1("<U>DOSIS</U>","");
			$f->text("f_vis_12",$visit_ri_grafik_ibu["vis_12"],30,50,$d2["vis_12"],$ext);
			$f->text4("f_vis_13",$visit_ri_grafik_ibu["vis_13"],30,50,$d2["vis_13"],"f_vis_14",$visit_ri_grafik_ibu["vis_14"],30,50,$d2["vis_14"],
					  "f_vis_15",$visit_ri_grafik_ibu["vis_15"],30,50,$d2["vis_15"],"f_vis_16",$visit_ri_grafik_ibu["vis_16"],30,50,$d2["vis_16"],$ext);
			$f->title1("<U>PEMERIKSAAN LAIN</U>","");
			$f->text("f_vis_17",$visit_ri_grafik_ibu["vis_17"],30,50,$d2["vis_17"],$ext);
			$f->text4("f_vis_18",$visit_ri_grafik_ibu["vis_18"],30,50,$d2["vis_18"],"f_vis_19",$visit_ri_grafik_ibu["vis_19"],30,50,$d2["vis_19"],
					  "f_vis_20",$visit_ri_grafik_ibu["vis_20"],30,50,$d2["vis_20"],"f_vis_21",$visit_ri_grafik_ibu["vis_21"],30,50,$d2["vis_21"],$ext);
			$f->text4("f_vis_22",$visit_ri_grafik_ibu["vis_22"],30,50,$d2["vis_22"],"f_vis_23",$visit_ri_grafik_ibu["vis_23"],30,50,$d2["vis_23"],
					  "f_vis_24",$visit_ri_grafik_ibu["vis_24"],30,50,$d2["vis_24"],"f_vis_25",$visit_ri_grafik_ibu["vis_25"],30,50,$d2["vis_25"],$ext);
			$f->text("f_vis_26",$visit_ri_grafik_ibu["vis_26"],30,50,$d2["vis_26"],$ext);
			$f->text("f_vis_27",$visit_ri_grafik_ibu["vis_27"],30,50,$d2["vis_27"],$ext);
			$f->title1("<U>DARAH</U>");
			$f->text("f_vis_28",$visit_ri_grafik_ibu["vis_28"],30,50,$d2["vis_28"],$ext);
			$f->text4("f_vis_29",$visit_ri_grafik_ibu["vis_29"],30,50,$d2["vis_29"],"f_vis_30",$visit_ri_grafik_ibu["vis_30"],30,50,$d2["vis_30"],
					  "f_vis_31",$visit_ri_grafik_ibu["vis_31"],30,50,$d2["vis_31"],"f_vis_32",$visit_ri_grafik_ibu["vis_32"],30,50,$d2["vis_32"],$ext);
			$f->text("f_vis_33",$visit_ri_grafik_ibu["vis_33"],30,50,$d2["vis_33"],$ext);
			$f->text("f_vis_34",$visit_ri_grafik_ibu["vis_34"],30,50,$d2["vis_34"],$ext);
			$f->text("f_vis_35",$visit_ri_grafik_ibu["vis_35"],30,50,$d2["vis_35"],$ext);
			$f->textinfo("f_vis_36",$visit_ri_grafik_ibu["vis_36"],10,10,$d2["vis_36"],"Kg",$ext);
			$f->textarea("f_vis_37",$visit_ri_grafik_ibu["vis_37"],1,50,$d2["vis_37"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
