...
//-------------------------------       	
$_GET["mPOLI"]=$setting_ri["proses_keperawatan"];
...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_proses_keperawatan&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_proses_keperawatan.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_proses_keperawatan.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
	...
	// --- eof 27-12-2006 ---
     ...
     /*   if (isset($_SESSION["SELECT_LAYANAN"]) && $is_range) {
        ...
        echo "<form name='Form10' action='actions/p_proses_keperawatan.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
...
left join rs00017 h on a.vis_2 = 'h.id'".
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>PROSES KEPERAWATAN</div>";
		//detail riwayat
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
		
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,to_char(A.TANGGAL_REG,'DD MON YYYY HH24:MI:SS')as tgl_reg,A.VIS_2,A.VIS_3,'DUMMY' ". 
					   "FROM C_VISIT_RI A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					   "WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' 
                                           group by A.NO_REG,A.TANGGAL_REG,A.VIS_2,A.VIS_3,A.VIS_1 ORDER BY A.VIS_1 ";
				$t = new PgTable($con, "100%");
			    $t->SQL = $sql ;
			    $t->setlocale("id_ID");
			    $t->ShowRowNumber = true;
			   	//$t->ColHidden[4]= true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL/JAM PENGISIAN","RIWAYAT PENYAKIT DAHULU","RIWAYAT PENYAKIT SEKARANG","DETAIL");
...
//====iqbal tambah tab konsultasi untuk rawat inap 15/02/12====			
    	...
    	$sql2 =	"SELECT A.*,(C.NAMA)AS merawat FROM C_VISIT_RI A 
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						echo "<font color='#000000' size='2'> Edit Data Proses Keperawatan</font>";
						$f = new Form("actions/p_proses_keperawatan.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
						$f->hidden("f_no_reg",$d2["no_reg"]);
					    $f->hidden("f_tanggal_reg",$_GET["tmp11"]);
			...
			$f->calendar("f_vis_8","Tanggal",10,15,$_GET["tmp1"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);		
			if($d2["vis_1"] != ''){
			$f->textinfo("f_vis_1",$visit_ri_proses_keperawatan["vis_1"],10,10,$_GET["tmp2"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_1",$visit_ri_proses_keperawatan["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			}
			$f->textarea("f_vis_2",$visit_ri_proses_keperawatan["vis_2"] ,1, $visit_ri_proses_keperawatan["vis_2"."W"],$_GET["tmp3"],$ext);
			$f->textarea("f_vis_3",$visit_ri_proses_keperawatan["vis_3"] ,1, $visit_ri_proses_keperawatan["vis_3"."W"],$_GET["tmp4"],$ext);
			$f->textarea("f_vis_10",$visit_ri_proses_keperawatan["vis_10"] ,1, $visit_ri_proses_keperawatan["vis_10"."W"],$_GET["tmp10"],$ext);
			$f->text("f_vis_9",$visit_ri_proses_keperawatan["vis_9"],30,50,$_GET["tmp9"],$ext);
			$f->title1("Evaluasi","");
			$f->text("f_vis_4","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_4"],30,50,$_GET["tmp5"],$ext);
			$f->text("f_vis_5","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_5"],30,50,$_GET["tmp6"],$ext);
			$f->text("f_vis_6","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_6"],30,50,$_GET["tmp7"],$ext);
			$f->text("f_vis_7","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_7"],30,50,$_GET["tmp8"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			...
			echo "<table border=1 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'>";
			$f->calendar("f_vis_8","Tanggal",10,15,"","Form2","icon/calendar.gif","Pilih Tanggal",$ext);		
			$f->textinfo("f_vis_1",$visit_ri_proses_keperawatan["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			$f->textarea("f_vis_2",$visit_ri_proses_keperawatan["vis_2"] ,1, $visit_ri_proses_keperawatan["vis_2"."W"],"",$ext);
			$f->textarea("f_vis_3",$visit_ri_proses_keperawatan["vis_3"] ,1, $visit_ri_proses_keperawatan["vis_3"."W"],"",$ext);
			$f->textarea("f_vis_10",$visit_ri_proses_keperawatan["vis_10"] ,1, $visit_ri_proses_keperawatan["vis_10"."W"],"",$ext);
			$f->text("f_vis_9",$visit_ri_proses_keperawatan["vis_9"],30,50,"",$ext);
			$f->title1("Evaluasi","");
			$f->text("f_vis_4","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_4"],30,50,"",$ext);
			$f->text("f_vis_5","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_5"],30,50,"",$ext);
			$f->text("f_vis_6","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_6"],30,50,"",$ext);
			$f->text("f_vis_7","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_7"],30,50,"",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			...
			echo "<div align=right><b>RM.08.a</b></div>";
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$sql = "SELECT A.NO_REG,A.VIS_8,A.VIS_1,A.VIS_2,A.VIS_3,A.VIS_4,A.VIS_5,A.VIS_6,A.VIS_7,A.VIS_9,A.VIS_10,A.tanggal_reg,'DUMMY'". 
					   "FROM C_VISIT_RI A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
			    ...
			    $t->ColHidden[12]=true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","EVALUASI (S)","","","EVALUASI (O)","EDIT");
			   	$t->ColAlign [0] ="center" ;
			   	$t->ColAlign [1] ="center" ;
			   	$t->ColAlign [2] ="center" ;
			   	$t->ColAlign [12] ="center" ;
				$t->ColFormatHtml[12] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=edit&mr=".$_GET["mr"]."&rg=<#0#>
				&tmp1=<#1#>&tmp2=<#2#>&tmp3=<#3#>&tmp4=<#4#>&tmp5=<#5#>&tmp6=<#6#>&tmp7=<#7#>&tmp8=<#8#>&tmp9=<#9#>&tmp10=<#10#>&tmp11=<#11#>'>".icon("edit","Edit")."</A>";	
				$t->execute();
				echo "<br>";
			...
			echo "<table border=1 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'>";
			if($d2["vis_8"]!= ''){
				$f->calendar("f_vis_8","Tanggal",10,15,$d2["vis_8"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			}else{
				$f->calendar("f_vis_8","Tanggal",10,15,$tglhariini,"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			}
					
			if($d2["vis_1"] != ''){
			$f->textinfo("f_vis_1",$visit_ri_proses_keperawatan["vis_1"],10,10,$d2["vis_1"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_1",$visit_ri_proses_keperawatan["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			}
			$f->textarea("f_vis_2",$visit_ri_proses_keperawatan["vis_2"] ,1, $visit_ri_proses_keperawatan["vis_2"."W"],$d2["vis_2"],$ext);
			$f->textarea("f_vis_3",$visit_ri_proses_keperawatan["vis_3"] ,1, $visit_ri_proses_keperawatan["vis_3"."W"],$d2["vis_3"],$ext);
			$f->textarea("f_vis_10",$visit_ri_proses_keperawatan["vis_10"] ,1, $visit_ri_proses_keperawatan["vis_10"."W"],$d2["vis_10"],$ext);
			$f->text("f_vis_9",$visit_ri_proses_keperawatan["vis_9"],30,50,$d2["vis_9"],$ext);
			$f->title1("Evaluasi","");
			$f->text("f_vis_4","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_4"],30,50,$d2["vis_4"],$ext);
			$f->text("f_vis_5","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_5"],30,50,$d2["vis_5"],$ext);
			$f->text("f_vis_6","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_6"],30,50,$d2["vis_6"],$ext);
			$f->text("f_vis_7","&nbsp;&nbsp;".$visit_ri_proses_keperawatan["vis_7"],30,50,$d2["vis_7"],$ext);
