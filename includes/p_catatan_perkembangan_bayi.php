...
$reg1 = (int) $_GET["rg1"];
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Catatan Perkembangan Bayi"	, $tab_disabled["pemeriksaan"]);
	...
	$T->addTab("$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Catatan Perkembangan Bayi"	, $tab_disabled["riwayat"]);
...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_catatan_perkembangan_bayi&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_catatan_perkembangan_bayi.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_catatan_perkembangan_bayi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        ...
        echo "<form name='Form10' action='actions/p_catatan_perkembangan_bayi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT CATATAN PERKEMBANGAN BAYI</div>";
		//detail riwayat
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
		
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,A.VIS_3,A.VIS_1,A.VIS_2,A.TANGGAL_REG,'DUMMY'". 
					   "FROM C_VISIT_RI A ".
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","CATATAN","DETAIL");
			   	$t->ColAlign = array("center","center","center","left","","center");
				$t->ColFormatHtml[5] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=detail&mr=".$_GET["mr"]."&rg=<#0#>&tgl=<#4#>'>".icon("view","Detail")."</A>";	
				$t->execute();
    	...
    	$sql2 =	"SELECT A.*,(C.NAMA)AS merawat FROM C_VISIT_RI A 
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						echo "<font color='#000000' size='2'> Edit Data Catatan Perkembangan Bayi</font>";
						$f = new Form("actions/p_catatan_perkembangan_bayi.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
						$f->hidden("f_no_reg",$d2["no_reg"]);
					    $f->hidden("f_tanggal_reg",$_GET["tmp4"]);
			...
			$f->calendar("f_vis_3","Tanggal",10,15,$_GET["tmp1"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);		
			$f->textinfo("f_vis_1",$visit_ri_catatan_perkembangan_bayi["vis_1"],10,10,$_GET["tmp2"],"(Jam:Menit, 08:09)",$ext);
			$f->textarea("f_vis_2",$visit_ri_catatan_perkembangan_bayi["vis_2"] ,4, $visit_ri_catatan_perkembangan_bayi["vis_2"."W"],$_GET["tmp3"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			echo"</div>";
			echo "<div align=right><b>RM.06</b></div>";
					   
				}elseif ($_GET['act'] == "tambah") {
					echo "<font color='#000000' size='2'>Tambah Data Catatan Perkembangan Bayi</font>";
					$f = new Form("actions/p_catatan_perkembangan_bayi.insert.php", "POST", "NAME=Form2");
					$f->hidden("act","new");
			...
			echo "<table border=1 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'>";
			$f->calendar("f_vis_3","Tanggal",10,15,$d2["vis_3"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);		
			$f->textinfo("f_vis_1",$visit_ri_catatan_perkembangan_bayi["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			$f->textarea("f_vis_2",$visit_ri_catatan_perkembangan_bayi["vis_2"] ,4, $visit_ri_catatan_perkembangan_bayi["vis_2"."W"],"",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
			...
			$sql = "SELECT A.NO_REG,A.VIS_3,A.VIS_1,A.VIS_2,A.tanggal_reg,'DUMMY'". 
			    ...
			    $t->ColHidden[5]=true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL","JAM","CATATAN","EDIT");
			   	$t->ColAlign = array("center","center","center","left","","center");
				$t->ColFormatHtml[5] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=edit&mr=".$_GET["mr"]."&rg=<#0#>
				&tmp1=<#1#>&tmp2=<#2#>&tmp3=<#3#>&tmp4=<#4#>$oid<#5#>'>".icon("edit","Edit")."</A>";	
				$t->execute();
				echo "<br>";
			...
			if($d2["vis_3"]!= ''){
				...
				$f->calendar("f_vis_3","Tanggal",10,15,$tglhariini,"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			}
					
			if($d2["vis_1"] != ''){
			$f->textinfo("f_vis_1",$visit_ri_catatan_perkembangan_bayi["vis_1"],10,10,$d2["vis_1"],"(Jam:Menit, 08:09)",$ext);
			}else{
			$f->textinfo("f_vis_1",$visit_ri_catatan_perkembangan_bayi["vis_1"],10,10,"00:00","(Jam:Menit, 08:09)",$ext);	
			}
			$f->textarea("f_vis_2",$visit_ri_catatan_perkembangan_bayi["vis_2"] ,4, $visit_ri_catatan_perkembangan_bayi["vis_2"."W"],$d2["vis_2"],$ext);
			$f->info("","(*)<I> Catatan (Pemeriksaan, keadaan pasien, perubahan diagnosis dan pengobatan)");
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			//unset($_SESSION["SELECT_EMP"]);
			//unset($_SESSION["SELECT_EMP2"]);
