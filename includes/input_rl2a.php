...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl200002a where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/rl2a.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Laporan RL2a");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/rl2a.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Laporan RL2a");
        $f->hidden("id","new");
    ...
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan RL2a");
lihat_laporan("laporan_rl2a");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select no_dtd, no_daftar, golongan, a, b, c, d, e, f, g, h,i,j,k,l, id as href FROM rl200002a order by no_dtd";            
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
	...
	$data = getFromTable("select golongan from rl200002a where no='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/rl2a.delete.php' method='get'>";
