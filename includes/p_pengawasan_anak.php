...
$reg1 = (int) $_GET["rg1"];
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Data Pengawasan Pasien Anak"	, $tab_disabled["pemeriksaan"]);
	...
	$T->addTab("$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Pengawasan Pasien Anak"	, $tab_disabled["riwayat"]);
...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_resume_anak&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_pengawasan_anak.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_pengawasan_anak.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        ...
        echo "<form name='Form10' action='actions/p_pengawasan_anak.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
			...
			echo"<div class=form_subtitle>DATA PENGAWASAN PASIEN ANAK</div>";
			echo "</td></tr>";
    		echo "<tr><td  valign=top>";
    		$f = new ReadOnlyForm();
			$f->text($visit_ri_pengawasan_pasien_anak["vis_1"],$d[4]);
			$f->text($visit_ri_pengawasan_pasien_anak["vis_2"],$d[5]);
			$f->text($visit_ri_pengawasan_pasien_anak["vis_3"],$d[6]);
			$f->text($visit_ri_pengawasan_pasien_anak["vis_4"],$d[7] );
			$f->text($visit_ri_pengawasan_pasien_anak["vis_5"],$d[8]);
			$f->text($visit_ri_pengawasan_pasien_anak["vis_17"],$d[20]);
			$f->text($visit_ri_pengawasan_pasien_anak["vis_6"],$d[9]."&nbsp;&deg;C");
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT PENGAWASAN PASIEN ANAK</div>";
		//detail riwayat
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
		
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,to_char(A.TANGGAL_REG,'DD MON YYYY HH24:MI:SS')as tgl_reg,A.VIS_17,A.VIS_6,A.VIS_7,A.VIS_8,A.VIS_9,A.TANGGAL_REG,'DUMMY'". 
					   "FROM C_VISIT_RI A ".
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL/JAM PENCATATAN","JAM","SUHU (&deg;C)","NADI (/Menit)","PERNAPASAN (/Menit)","MUNTAH","DETAIL");
			   	$t->ColAlign = array("center","center","left","left","left","left","left","","center");
    	...
    	$sql2 =	"SELECT A.* FROM C_VISIT_RI A 
    				WHERE A.ID_RI='{$_GET["ri"]}' AND A.NO_REG='{$_GET["rg1"]}' AND A.TANGGAL_REG='{$_GET["tmp13"]}' ";	
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
                                    ...
                                    $f = new Form("actions/p_pengawasan_anak.insert.php", "POST", "NAME=Form2");
                                    $f->hidden("act","edit");
                                    $f->hidden("f_no_reg",$d2["no_reg"]);
                                    $f->hidden("f_tanggal_reg",$_GET["tmp13"]);
			...
			$f->title1("<U>PERATURAN</U>");
			$f->textarea("f_vis_1",$visit_ri_pengawasan_pasien_anak["vis_1"] ,1, $visit_ri_pengawasan_pasien_anak["vis_1"."W"],$d2["vis_1"],$ext);
			$f->textarea("f_vis_2",$visit_ri_pengawasan_pasien_anak["vis_2"] ,1, $visit_ri_pengawasan_pasien_anak["vis_2"."W"],$d2["vis_2"],$ext);
			$f->textarea("f_vis_3",$visit_ri_pengawasan_pasien_anak["vis_3"] ,1, $visit_ri_pengawasan_pasien_anak["vis_3"."W"],$d2["vis_3"],$ext);
			$f->text("f_vis_4",$visit_ri_pengawasan_pasien_anak["vis_4"],30,50,$d2["vis_4"],$ext);
			$f->text("f_vis_5",$visit_ri_pengawasan_pasien_anak["vis_5"],30,50,$d2["vis_5"],$ext);
			$f->title1("<U>DETAIL PENGAWASAN</U>");
			$f->text("f_vis_17",$visit_ri_pengawasan_pasien_anak["vis_17"],2,2,$_GET["tmp1"],$ext);
			$f->textinfo("f_vis_6",$visit_ri_pengawasan_pasien_anak["vis_6"],10,50,$_GET["tmp2"],"&deg;C",$ext);
			$f->textinfo("f_vis_7",$visit_ri_pengawasan_pasien_anak["vis_7"],10,50,$_GET["tmp3"],"/Menit",$ext);
			$f->textinfo("f_vis_8",$visit_ri_pengawasan_pasien_anak["vis_8"],10,50,$_GET["tmp4"],"/Menit",$ext);
			$f->text("f_vis_9",$visit_ri_pengawasan_pasien_anak["vis_9"],15,30,$_GET["tmp5"],$ext);
			$f->title1("<U>FAECES</U>");
			$f->textinfo("f_vis_10",$visit_ri_pengawasan_pasien_anak["vis_10"],20,50,$_GET["tmp6"],"",$ext);
			$f->textinfo("f_vis_11",$visit_ri_pengawasan_pasien_anak["vis_11"],20,50,$_GET["tmp7"],"",$ext);
			$f->textinfo("f_vis_12",$visit_ri_pengawasan_pasien_anak["vis_12"],20,50,$_GET["tmp8"],"",$ext);
			$f->textinfo("f_vis_13",$visit_ri_pengawasan_pasien_anak["vis_13"],20,50,$_GET["tmp9"],"",$ext);
			$f->title1("PENGAWASAN LAIN");
			$f->text("f_vis_14",$visit_ri_pengawasan_pasien_anak["vis_14"],20,30,$_GET["tmp10"],$ext);
			$f->text("f_vis_15",$visit_ri_pengawasan_pasien_anak["vis_15"],20,30,$_GET["tmp11"],$ext);
			$f->text("f_vis_16",$visit_ri_pengawasan_pasien_anak["vis_16"],20,30,$_GET["tmp12"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			
			echo"</div>";
			echo "<div align=right><b>RM 09.a</b></div>";
			...
			$f->text("f_vis_5",$visit_ri_pengawasan_pasien_anak["vis_5"],30,50,$d2["vis_5"],$ext);
			$f->title1("<U>DETAIL PENGAWASAN</U>");
			$f->text("f_vis_17",$visit_ri_pengawasan_pasien_anak["vis_17"],2,2,"",$ext);
			$f->textinfo("f_vis_6",$visit_ri_pengawasan_pasien_anak["vis_6"],10,50,"","&deg;C",$ext);
			$f->textinfo("f_vis_7",$visit_ri_pengawasan_pasien_anak["vis_7"],10,50,"","/Menit",$ext);
			$f->textinfo("f_vis_8",$visit_ri_pengawasan_pasien_anak["vis_8"],10,50,"","/Menit",$ext);
			$f->text("f_vis_9",$visit_ri_pengawasan_pasien_anak["vis_9"],15,30,"",$ext);
			$f->title1("<U>FAECES</U>");
			$f->textinfo("f_vis_10",$visit_ri_pengawasan_pasien_anak["vis_10"],20,50,"","",$ext);
			$f->textinfo("f_vis_11",$visit_ri_pengawasan_pasien_anak["vis_11"],20,50,"","",$ext);
			$f->textinfo("f_vis_12",$visit_ri_pengawasan_pasien_anak["vis_12"],20,50,"","",$ext);
			$f->textinfo("f_vis_13",$visit_ri_pengawasan_pasien_anak["vis_13"],20,50,"","",$ext);
			$f->title1("PENGAWASAN LAIN");
			$f->text("f_vis_14",$visit_ri_pengawasan_pasien_anak["vis_14"],20,30,"",$ext);
			$f->text("f_vis_15",$visit_ri_pengawasan_pasien_anak["vis_15"],20,30,"",$ext);
			$f->text("f_vis_16",$visit_ri_pengawasan_pasien_anak["vis_16"],20,30,"",$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			...
			echo "<div align=right><b>RM 09.a</b></div>";
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$sql = "SELECT A.NO_REG,to_char(A.TANGGAL_REG,'DD MON YYYY HH24:MI:SS')as tgl_reg,A.VIS_17,A.VIS_6,A.VIS_7,A.VIS_8,A.VIS_9,
			A.VIS_10,A.VIS_11,A.VIS_12,A.VIS_13,A.VIS_14,A.VIS_15,A.VIS_16,A.TANGGAL_REG,'DUMMY'". 
					   "FROM C_VISIT_RI A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					   "WHERE B.MR_NO = '".$_GET["mr"]."' AND A.ID_ri = '{$_GET["mPOLI"]}' ORDER BY A.VIS_17 ";
				$t = new PgTable($con, "100%");
			    $t->SQL = $sql ;
			    $t->setlocale("id_ID");
			    $t->ShowRowNumber = true;
			    ...
			    $t->ColHidden[15]=true;
			    $t->RowsPerPage = $ROWS_PER_PAGE;
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL/JAM PENCATATAN","JAM","SUHU (&deg;C)","NADI (/MENIT)","PERNAPASAN (/MENIT)","MUNTAH","EDIT");
			   	$t->ColAlign = array("center","center","left","left","left","left","left","center","center","center","center","center","center","center","center","center");
				$t->ColFormatHtml[15] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&ri=".$_GET["mPOLI"]."&act=edit&mr=".$_GET["mr"]."&rg=<#0#>
				&tmp1=<#2#>&tmp2=<#3#>&tmp3=<#4#>&tmp4=<#5#>&tmp5=<#6#>&tmp6=<#7#>&tmp7=<#8#>&tmp8=<#9#>&tmp9=<#10#>&tmp10=<#11#>&tmp11=<#12#>&tmp12=<#13#>&tmp13=<#14#>'>".icon("edit","Edit")."</A>";	
				$t->execute();
				echo "<br>";
			...
			$f->text("f_vis_5",$visit_ri_pengawasan_pasien_anak["vis_5"],30,50,$d2["vis_5"],$ext);
			$f->title1("<U>DETAIL PENGAWASAN</U>");
			$f->text("f_vis_17",$visit_ri_pengawasan_pasien_anak["vis_17"],2,2,$d2["vis_17"],$ext);
			$f->textinfo("f_vis_6",$visit_ri_pengawasan_pasien_anak["vis_6"],10,50,$d2["vis_6"],"&deg;C",$ext);
			$f->textinfo("f_vis_7",$visit_ri_pengawasan_pasien_anak["vis_7"],10,50,$d2["vis_7"],"/Menit",$ext);
			$f->textinfo("f_vis_8",$visit_ri_pengawasan_pasien_anak["vis_8"],10,50,$d2["vis_8"],"/Menit",$ext);
			$f->text("f_vis_9",$visit_ri_pengawasan_pasien_anak["vis_9"],15,30,$d2["vis_9"],$ext);
			$f->title1("<U>FAECES</U>");
			$f->textinfo("f_vis_10",$visit_ri_pengawasan_pasien_anak["vis_10"],20,50,$d2["vis_10"],"",$ext);
			$f->textinfo("f_vis_11",$visit_ri_pengawasan_pasien_anak["vis_11"],20,50,$d2["vis_11"],"",$ext);
			$f->textinfo("f_vis_12",$visit_ri_pengawasan_pasien_anak["vis_12"],20,50,$d2["vis_12"],"",$ext);
			$f->textinfo("f_vis_13",$visit_ri_pengawasan_pasien_anak["vis_13"],20,50,$d2["vis_13"],"",$ext);
			$f->title1("PENGAWASAN LAIN");
			$f->text("f_vis_14",$visit_ri_pengawasan_pasien_anak["vis_14"],20,30,$d2["vis_14"],$ext);
			$f->text("f_vis_15",$visit_ri_pengawasan_pasien_anak["vis_15"],20,30,$d2["vis_15"],$ext);
			$f->text("f_vis_16",$visit_ri_pengawasan_pasien_anak["vis_16"],20,30,$d2["vis_16"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
