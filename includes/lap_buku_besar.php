...
*/
if($_GET['mGDP']){
	$nm_depo = getFromTable("select tdesc from rs00001 where tc='".$_GET["mGDP"]."' and tt='GDP'");
}
if(!$GLOBALS['print']){
title("<img src='icon/akuntansi-bukubesar.png' align='absmiddle' >  KARTU STOCK ".strtoupper($nm_depo));
}else{
title("KARTU STOCK ".strtoupper($nm_depo));
}
title_print("");
title_excel("lap_buku_besar&mGDP=".$_GET["mGDP"]."&obat=".$_GET["obat"]."&tanggal1D=".$_GET["tanggal1D"]."&tanggal1M=".$_GET["tanggal1M"]."&tanggal1Y=".$_GET["tanggal1Y"]."&tanggal2D=".$_GET["tanggal2D"]."&tanggal2M=".$_GET["tanggal2M"]."&tanggal2Y=".$_GET["tanggal2Y"]."&mDETIL=S");
...
$f->selectSQL("mGDP", "Depo",
        ...
        "select tc, tdesc ".
        "from rs00001 ".
        "where tt = 'GDP' and tc_tipe='1' ".
        "order by tdesc asc", $_GET["mGDP"],
        "");
//$f->textauto_all("obat", "AUTOTEXT_OBAT", "Nama Obat", 50, 100, $_GET["obat"],$ext);

//$f->hidden("e",$_GET['e'],"id=id_obat");

$f->text("obat", "Obat", 50, 50, $_GET['obat'], " id='obat' ");
...
if ($_GET["mDETIL"] == 'S') {
  ...
  */	}
    ...
    */
    ...
    //$t = new PgTable($con, "100%");
   /* $t->SQL = "SELECT a.item_id,a.nama_obat,c.tdesc as satuan,case when d.gudang>=0 then null else null end as saldo_awal,sum(a.qty_terima),sum(a.qty_keluar),sum(a.qty_ret_msk),sum(a.qty_ret_kel),sum(a.qty_adj),case when d.gudang>=0 then null else null end as saldo_akhir,a.hna,0*a.hna as jumlah_saldo from v_buku_besar a ".
	...
	if($_GET['obat']!=''){
		...
		$obatTmp = $_GET['obat'];
		$arrParamObat = explode(' ', $obatTmp);
		$obat = $arrParamObat[0];
		$sql_add = " AND a.id::INTEGER = ". $obat;
		
	?>
	<table id="list-pasien" width="100%" >
    <thead>
        <tr>
            <td align="CENTER" class="TBL_HEAD" width="10" rowspan="2" <?=$font ?>>NO	</td>
            <td align="CENTER" class="TBL_HEAD" width="50" rowspan="2" <?=$font ?>>TANGGAL</td>
            <td align="CENTER" class="TBL_HEAD" width="90" rowspan="2" <?=$font ?>>NO BUKTI</td>
            <td align="CENTER" class="TBL_HEAD" width="40" colspan="2" <?=$font ?>>SALDO AWAL</td>
            <td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>MASUK </td>
            <td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>KELUAR </td>
            <td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>RETUR MASUK </td>
            <td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>RETUR KELUAR </td>
            <!--td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>ADJUSMENT </td-->
            <td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>SALDO AKHIR </td>
            <td align="CENTER" class="TBL_HEAD" width="200" rowspan="2" <?=$font ?>>KETERANGAN </td>
        </tr>
		<TR>
            <td align="CENTER" class="TBL_HEAD" width="40" <?=$font ?>>QTY </td>
		<td align="CENTER" class="TBL_HEAD" width="100" <?=$font ?>>HARGA</td>
            <td align="CENTER" class="TBL_HEAD" width="40" <?=$font ?>>QTY </td>
		<td align="CENTER" class="TBL_HEAD" width="100" <?=$font ?>>HARGA</td>
            <td align="CENTER" class="TBL_HEAD" width="40" <?=$font ?>>QTY </td>
		<td align="CENTER" class="TBL_HEAD" width="100" <?=$font ?>>HARGA</td>
		...
		<td align="CENTER" class="TBL_HEAD" width="100" <?=$font ?>>HARGA</td>
            <td align="CENTER" class="TBL_HEAD" width="40" <?=$font ?>>QTY </td>
		<td align="CENTER" class="TBL_HEAD" width="100" <?=$font ?>>HARGA</td>
            <!--td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>QTY </td-->
            <!--td align="CENTER" class="TBL_HEAD" width="100" colspan="2" <?=$font ?>>HARGA </td-->
		</TR>
    </thead>
    <tbody>
<?php
	if($_GET["mGDP"]==''){
		$field2 = "(i.gudang+i.qty_ri+i.qty_igd+";
	 for ($i = 1; $i <= 43; $i++) {
        $field2 .= str_pad($i, 9, "i.qty_00", STR_PAD_LEFT) ;
		if($i!=43){
			$field2 .='+';
		}else{
			$field2 .=') as stok';
		...
		$field3 = "(h.gudang+h.qty_ri+h.qty_igd+";
	 for ($i = 1; $i <= 43; $i++) {
        $field3 .= str_pad($i, 9, "h.qty_00", STR_PAD_LEFT) ;
		if($i!=43){
			$field3 .='+';
		}else{
			$field3 .=') as stok';
	...
	}else if($_GET["mGDP"]=="003"){
		$field2 = "i.gudang";
		$field3 = "h.gudang";
	}else if($_GET["mGDP"]=="020"){
		$field2 = "i.qty_ri";
		$field3 = "h.qty_ri";
	}else{
		$field2 = "i.qty_".$_GET["mGDP"];
		$field3 = "h.qty_".$_GET["mGDP"];
	}
if($ts_check_in2==date('Y-m-d')){
		$field="(SELECT $field2 FROM rs00016a i where i.obat_id='$obat')";
	}else{
		$field="(SELECT $field2 FROM daily_stock i where i.obat_id='$obat' and date(i.timestamp)=b.tanggal_entry )";
	}
	
	
	
	$rowsData = pg_query($con,"SELECT to_char(e.date_stock,'dd Mon YYYY')  as tanggal,b.trans_type || '/' || to_char(e.date_stock,'mmyy') || '-' || b.id  as no_bukti,b.trans_type,
							   (SELECT $field3 FROM daily_stock h where h.obat_id='$obat' and h.date_stock=b.tanggal_entry ) as stok_awal, ".
							   "case when b.hna is null then 0 else b.hna end as hja, ".
							   "case when b.qty_msk is null then 0 else b.qty_msk end as qty_terima, ".
							   "case when b.harga_masuk is null then 0 else b.harga_masuk end as harga_terima, ".
							   "case when b.qty_keluar is null then 0 else b.qty_keluar end as qty_keluar, ".
							   "case when b.harga_keluar is null then 0 else b.harga_keluar end as harga_keluar, ".
							   "case when b.qty_ret_msk is null then 0 else b.qty_ret_msk end as qty_ret_msk, ".
							   "case when b.harga_ret_masuk is null then 0 else b.harga_ret_masuk end as harga_ret_masuk, ".
							   "case when b.qty_ret_kel is null then 0 else b.qty_ret_kel end as qty_ret_kel, ".
							   "case when b.harga_ret_kel is null then 0 else b.harga_ret_kel end as harga_ret_kel, ".
							   "case when b.qty_adj is null then 0 else b.qty_adj end as qty_adj, ".
							   "case when b.harga_adj is null then 0 else b.harga_adj end as harga_adj, ".
							   " $field as stok_akhir, b.ket as keterangan from rs00015 a ".
			  "     left join rs00016a d on a.id = d.obat_id ". 
			  "     left join rs00016 g on a.id = g.obat_id ". 
			  "     join daily_stock e on a.id = e.obat_id ". 
			  "     join v_buku_besar2 b on e.obat_id =to_number(b.item_id, '999999999999') and e.date_stock=b.tanggal_entry ".
			  " where a.kategori_id like '%".$_GET["mOBT"]."%' and b.id_depo like '%".$_GET["mGDP"]."%' and a.kategori_id not in('019') and e.date_stock between '$ts_check_in1' and '$ts_check_in2' ".
...
if(!empty($rowsData)){
	...
	// $qty=0;
	 while($row=pg_fetch_array($rowsData)){
		 ...
		 if($i==1){
			...
			$stok = $stok_awal + ( $row["qty_terima"] + $row["qty_ret_msk"] + $row["qty_adj"] ) - ( $row["qty_ret_kel"] + $row["qty_keluar"]);
		 }else{
			$stok_awal = $stok;
			$stok = $stok_awal + ( $row["qty_terima"] + $row["qty_ret_msk"] + $row["qty_adj"] ) - ( $row["qty_ret_kel"] + $row["qty_keluar"]);
		 }
	...
	<tr>
		...
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=number_format($row["hja"]);?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=$row["qty_terima"];?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=number_format($row["harga_terima"]);?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=$row["qty_keluar"];?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=number_format($row["harga_keluar"]);?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=$row["qty_ret_msk"];?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=number_format($row["harga_ret_masuk"]);?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=$row["qty_ret_kel"];?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=number_format($row["harga_ret_kel"]);?></td>
		<!--td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=$row["qty_adj"];?></td-->
		<!--td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=number_format($row["harga_adj"]);?></td-->
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=$stok_akhir;?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=number_format($row["hja"]);?></td>
		<td class='TBL_BODY' style="text-align: right;" <?=$font ?>><?=$row["keterangan"];?></td>
	</tr>
<?php
	...
	<?php
	/*$t->SQL = "SELECT to_char(b.tanggal_entry,'dd Mon YYYY')  as tanggal,b.trans_type || '-' || to_char(e.date_stock,'mmyy')  as no_bukti,e.gudang as stok_awal,case when b.qty_msk is null then 0 else b.qty_msk end as qty_terima,case when b.qty_keluar is null then 0 else b.qty_keluar end as qty_keluar,case when b.qty_ret_msk is null then 0 else b.qty_ret_msk end as qty_ret_msk,case when b.qty_ret_kel is null then 0 else b.qty_ret_kel end as qty_ret_kel,case when b.qty_adj is null then 0 else b.qty_adj end as qty_adj,f.gudang as stok_akhir from rs00015 a ".
	...
	$r2 = pg_query($con, "SELECT sum(b.qty_terima) as jum_terima,sum(b.qty_keluar) as jum_keluar,sum(b.qty_ret_msk) as jum_ret_msk,sum(b.qty_ret_kel) as jum_ret_kel,sum(b.qty_adj) as jum_adj from rs00015 a ".
			  "     left join v_buku_besar b on a.id =to_number(b.item_id, '999999999999') and (b.tanggal_entry between '$ts_check_in1' and '$ts_check_in2') ".
			  //"     left join rs00001 c on a.kategori_id = c.tc and c.tt='SAT' ".
			  "     left join rs00016a d on a.id = d.obat_id ". 
			  "     left join rs00016 g on a.id = g.obat_id ". 
...
$("#obat").autocomplete("lib/get_obat.php",{
                        width: 260,
                        selectFirst: false,
                        autoFill:false,
                        type: "GET",
                        extraParams: {q: function() { return $('input[name=obat]').val(); } }
                });
});
</script>
