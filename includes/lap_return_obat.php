...
//--start
if (!$GLOBALS['print']){
    title_print("<img src='icon/keuangan-2.gif' align='absmiddle' > <b>LAPORAN RETURN OBAT</b>");
    title_excel("lap_return_obat&tanggal1D=".$_GET["tanggal1D"]."&tanggal1M=".$_GET["tanggal1M"]."&tanggal1Y=".$_GET["tanggal1Y"]."".
    ...
    title("<img src='icon/keuangan.gif' align='absmiddle' > Laporan Return Obat");
    title_excel("lap_return_obat&tanggal1D=".$_GET["tanggal1D"]."&tanggal1M=".$_GET["tanggal1M"]."&tanggal1Y=".$_GET["tanggal1Y"]."".
        "&tanggal2D=".$_GET["tanggal2D"]."&tanggal2M=".$_GET["tanggal2M"]."&tanggal2Y=".$_GET["tanggal2Y"]."".
        "&mUNIT=".$_GET["mUNIT"]."&mPASIEN=".$_GET["mPASIEN"]."&mKATEGORY=".$_GET["mKATEGORY"]."&mDOKTER=".$_GET["mDOKTER"]."&mSPESIALIS=".$_GET["mSPESIALIS"]."");
}
...
$rRacikan = pg_query($con,
          "select ".
          "		CASE WHEN e.kategori_id::numeric=020 THEN (a.qty_return * 0)
		            ELSE 0
		       	END as harga_tuslah ".
...
$rowsObatTotal = pg_query($con,
          ...
          "sum((a.qty_return * a.harga)+(a.referensi::numeric)) as jum ".
          "from rs00008_return a ".
          "     left join rs00006 b ON a.no_reg = b.id ".
          "     left join rs00001 d ON (b.tipe = d.tc and d.tt = 'JEP') ".
          "     left join rs00016 f ON a.item_id = f.obat_id::character varying ".
...
//-- rows semua obat
$rowsObat = pg_query($con, "select a.tanggal_trans as tgl, a.no_reg as no_reg, c.mr_no as mr_no, c.nama as nama, ".
          "     e.obat as obat, a.qty_awal as qty_awal, a.qty_return as qty_return, f.harga::integer as harga_pokok, ".
          "		CASE WHEN e.kategori_id::numeric=020 THEN (a.harga - 0)
		            ELSE a.harga::numeric
		       	END as harga_jual, ".
          "		CASE WHEN e.kategori_id::numeric=020 THEN (a.qty_return * 0)
		            ELSE a.referensi::numeric
		       	END as harga_tuslah, ".
          "		sum((a.qty_return * a.harga)+(a.referensi::numeric)) as harga_total ".
          "from rs00008_return a ".
          "     left join rs00006 b ON a.no_reg = b.id ".
          ...
          "     (a.trans_type::text = 'OB1'::text OR a.trans_type::text = 'RCK'::text) ".
          "     ".$addParam." and (b.waktu_reg between '$jam1' and '$jam2') ".
          "group by a.tanggal_trans, c.mr_no, c.nama, a.no_reg, e.obat, a.qty_awal, a.qty_return, a.harga, f.harga, a.referensi, e.kategori_id ".
          "order by a.tanggal_trans, a.no_reg");
//--

//-- jumlah row tuslah obat grand total
$rowsTotalTuslahObats = pg_query($con, "select sum(a.harga) as jual, ".
          "		sum(CASE WHEN e.kategori_id::numeric=020 THEN (a.qty_return * 0)
		            ELSE a.referensi::numeric
		       	END) as tuslah, ".
          ...
          "where (a.tanggal_trans between '$ts_check_in1' and '$ts_check_in2') and ".
          "     (a.trans_type::text = 'OB1'::text OR a.trans_type::text = 'RCK'::text) ".
          "     ".$addParam." and (b.waktu_reg between '$jam1' and '$jam2') group by a.tanggal_trans, a.no_reg order by a.tanggal_trans, a.no_reg ");
...
$sumHargaTuslahObats = $hargaJualObats;
$hargaTuslahObats 	 = $hargaTuslahObats;
$hargaJumObats 		 = $hargaJumObats;
//--
...
<table id="list-pasien" width="100%">
    <thead>
        <tr>
            ...
            <td align="CENTER" class="TBL_HEAD" width="40">QTY AWAL</td>
            <td align="CENTER" class="TBL_HEAD" width="40">QTY RETURN</td>
    ...
    <tbody>
        <?php
            if(!empty($rowsObat)){
                 ...
                 while($row=pg_fetch_array($rowsObat)){
					 ...
					 if ($ii == '' || $newNoReg == '' || $newNoMR == '' || $newNama == '') {
						$hargaTuslah = '';
						$hargaTotal  = '';
					 } else {
        ...
        <tr>
            ...
            <td align="right"><?php echo $row['qty_awal'];?>&nbsp;&nbsp;&nbsp;</td>
            <td align="right"><?php echo $row['qty_return'];?>&nbsp;&nbsp;&nbsp;</td>
