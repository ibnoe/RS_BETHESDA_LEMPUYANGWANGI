...
//-------------------------------       	
$_GET["mPOLI"]=$setting_ri["hasil_radiologi"];
...
$reg1 = (int) $_GET["rg1"];
	...
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr ", "Hasil Radiologi"	, $tab_disabled["pemeriksaan"]);
	...
	$T->addTab("$SC?p=$PID&list=riwayat&rg1={$_GET["rg1"]}&rg=$rg&ri=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Hasil Radiologi"	, $tab_disabled["riwayat"]);
...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_hasil_radiologi&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_hasil_radiologi.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_hasil_radiologi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        ...
        echo "<form name='Form10' action='actions/p_hasil_radiologi.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
			...
			echo"<div class=form_subtitle>HASIL RADIOLOGI / KEDOKTERAN NUKLIR</div>";
			//echo "</td></tr>";
    		echo "<tr><td  valign=top>";
    		$f = new ReadOnlyForm();
			$f->text($visit_ri_hasil_radiologi["vis_1"],$d[4]);		
			$f->execute();	
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>HASIL RADIOLOGI / KEDOKTERAN NUKLIR</div>";
		//detail riwayat
		...
		$sql = "SELECT A.NO_REG,to_char(A.TANGGAL_REG,'DD MON YYYY HH24:MI:SS')as tgl_reg,'DUMMY' ". 
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL/JAM PENCATATAN","DETAIL");
			   	$t->ColAlign = array("center","center","center");
				$t->ColFormatHtml[2] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=riwayat&rg1=".$_GET["rg1"]."&ri=".$_GET["mPOLI"]."&act=detail&mr=".$_GET["mr"]."&rg=<#0#>'>".icon("view","View")."</A>";	
				$t->execute();
    	...
    	$T->show(0);
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_hasil_radiologi.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			$f->textarea("f_vis_1",$visit_ri_hasil_radiologi["vis_1"] ,4, $visit_ri_hasil_radiologi["vis_1"."W"],$d2["vis_1"],$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			
			echo"</div>";
			echo "<div align=right><b>RM HS.R</b></div>";
