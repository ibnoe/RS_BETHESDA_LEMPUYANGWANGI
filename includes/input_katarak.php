...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl100020a where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/katarak.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Katarak");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/katarak.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Katarak");
        $f->hidden("id","new");
    ...
    $f->text("f_baik","Kualitas Baik",30,30,$d->baik);
	$f->text("f_sedang","Kualitas Sedang",30,30,$d->sedang);
	$f->text("f_buruk","Kualitas Buruk",30,30,$d->buruk);
	$f->text("f_kuantitas","Jumlah Kuantitas",30,30,$d->kuantitas);
	 $f->submit(" Simpan ");
    $f->execute();
} else {
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan Pembedahan Mata - Katarak");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select kegiatan, baik, sedang, buruk, kuantitas,id as href FROM rl100020a order by id";            
              
    $t->ColHeader = array("Jenis Kegiatan", "Kualitas Baik","Kualitas Sedang","Kualitas Buruk"," Jumlah Kuantitas", "Edit");
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
    ...
    $t->ColFormatHtml[5] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#5#>'>".icon("edit","Edit")."</A>".
            ...
            "actions/katarak.delete.php?p=$PID".
            "&e=<#5#>".
            "'>".icon("delete","Hapus")."</A>".
            "</nobr>"; 
    /*
    ...
    */
	...
	$data = getFromTable("select kegiatan from rl100020a where id='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/katarak.delete.php' method='get'>";
