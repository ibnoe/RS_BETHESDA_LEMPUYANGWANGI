...
if ($_GET['act'] ==  "detail"){
		if (!$GLOBALS['print']){
			title_print("<img src='icon/medical-record-2.gif' align='absmiddle' > REKAM MEDIS KLINIK MATA");
			echo "<DIV ALIGN=RIGHT OnClick='window.history.back()'>".icon("back","Kembali")."</a></DIV>";
		}else{
			title_print("<img src='icon/medical-record.gif' align='absmiddle' > REKAM MEDIS KLINIK MATA");
		}
		    //echo "<br>";
			$sql = "select a.*,b.nama 
		    		from c_visit a
		    ...
		    $f->execute();
		    echo "</td><td class='tbl_body' valign=top align=left width='33%'>";
			$f = new ReadOnlyForm();
			$f->text("Tgl Masuk",$d2->tanggal_reg);	
		    $f->text("Umur",$d2->umur);
			$f->text("Seks",$d2->jenis_kelamin);
			$f->execute();
		    echo "</td><td class='tbl_body' valign=top align=left width='35%'>";
		    $f = new ReadOnlyForm();
		...
		$f->text_mata("KEDUDUKAN/GERAK BOLA MATA","OD","OS","<IMG BORDER=0 SRC='images/bg_tbh_04.gif'>","<IMG BORDER=0 SRC='images/bg_tbh_05.gif'>","KETERANGAN GAMBAR",$d[44],$d[45],"PALPEBRA",$d[5],$d[6],
		...
		$f->text_mata("","","","","","","","","VISUS",$d[25],$d[26],"KOREKSI",$d[27],$d[28],"KACAMATA LAMA",$d[29],$d[30],"APLANSI",$d[31],$d[32],"TONOMETRI",$d[33],$d[34],"ANEL ",$d[35],$d[36],"","","","","","","","","","","","","CENTER");
		$f->text("BACA",$d[37]);
