...
title("Transaksi Pengadaan Farmalkes");
echo "<br>";
...
echo "<table border=0 width='100%'><tr><td>";
    if (isset($_SESSION["resep"])) {
        ...
        pg_free_result($r2);
        $f = new ReadOnlyForm();
        $f->text("Nomor Bukti", $d2->no_referensi);
        $f->text("Tanggal", $d2->tanggal_pesan);
        $f->text("Nama Supplier", $d2->nama);
        $f->execute();
    }
    /*
...
if (isset($_SESSION["resep"])) {
    ...
    if ($ref == "F") {
        info("Informasi:", "Bukti dengan nomor ".$_SESSION["resep"]." sudah pernah dilayani.");
        echo "<br>";
    }
}
...
if (is_array($_SESSION["ob2"])) {
    ...
    echo "<form action='actions/819.insert.php' method=POST name=Form10>";
