...
if ($reg > 0) {
	...
	title_print("<img src='icon/rawat-inap-2.gif' align='absmiddle' >  $d1->tdesc");
                title_excel("p_asuhan_keperawatan&tblstart=".$_GET['tblstart']."&list=".$_GET['list']."&rg1=".$_GET['rg1']."&ri=".$_GET['ri']."&act=".$_GET['act']."&mr=".$_GET['mr']."&rg=".$_GET['rg']."&oid=".$_GET['oid']."");
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_asuhan_keperawatan.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
        $f->hidden("ri",$_GET["ri"]);
	...
	$T->show(1);
        ...
        echo "<form name='Form9' action='actions/p_asuhan_keperawatan.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
	...
	// --- eof 27-12-2006 ---
     ...
     /*   if (isset($_SESSION["SELECT_LAYANAN"]) && $is_range) {
        ...
        echo "<form name='Form10' action='actions/p_asuhan_keperawatan.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<input type=hidden name=rg1 value='".$_GET["rg1"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
...
left join rs00017 h on a.vis_2 = 'h.id'".
			...
			$f->text($visit_ri_asuhan_keperawatan["vis_1"],$d[4] );
			$f->text($visit_ri_asuhan_keperawatan["vis_2"],$d[5] );
			$f->text($visit_ri_asuhan_keperawatan["vis_3"],$d[6] );
			$f->text($visit_ri_asuhan_keperawatan["vis_4"],$d[7] );
			if($d["vis_5"] != ''){
			$f->text($visit_ri_asuhan_keperawatan["vis_5"],$d[8] );
			}else{}
			if($d["vis_6"] != ''){
			$f->text($visit_ri_asuhan_keperawatan["vis_6"],$d[9] );
			}else{}
			if($d["vis_7"] != ''){
			$f->text($visit_ri_asuhan_keperawatan["vis_7"],$d[10] );
			}else{}
			if($d["vis_8"] != ''){
			$f->text($visit_ri_asuhan_keperawatan["vis_8"],$d[11] );
			}else{}
			if($d["vis_9"] != ''){
			$f->text($visit_ri_asuhan_keperawatan["vis_9"],$d[12] );
			}else{}
    		...
    		echo "</td></tr>";
  			...
  			echo "</td></tr></table>";
				...
				echo"<div align=center class=form_subtitle1>ASUHAN KEPERAWATAN</div>";
		//detail riwayat
    	...
    	if ($_GET["act"] == "detail_klinik") {
    		...
    		elseif ($poli == $setting_poli["radiologi"]){
				...
				echo"<div align=center class=form_subtitle1>RIWAYAT KLINIK</div>";
		//detail riwayat
		echo "<table border=0 width='100%' cellspacing=0 cellpadding=0><tr><td valign=top width='33%'  colspan=2>";
    	...
    	$T->show(0);
			...
			pg_free_result($r);
				...
				if ($_GET['act'] == "edit") {
						...
						$f = new Form("actions/p_asuhan_keperawatan.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
			...
			$f->textarea("f_vis_1",$visit_ri_asuhan_keperawatan["vis_1"] ,1, $visit_ri_asuhan_keperawatan["vis_1"."W"],$d2["vis_1"],$ext);
			$f->textarea("f_vis_2",$visit_ri_asuhan_keperawatan["vis_2"] ,1, $visit_ri_asuhan_keperawatan["vis_2"."W"],$d2["vis_2"],$ext);
			$f->textarea("f_vis_3",$visit_ri_asuhan_keperawatan["vis_3"] ,1, $visit_ri_asuhan_keperawatan["vis_3"."W"],$d2["vis_3"],$ext);
			$f->title1("<U>ANALISA</U>");
			$f->textarea("f_vis_4",$visit_ri_asuhan_keperawatan["vis_4"] ,1, $visit_ri_asuhan_keperawatan["vis_4"."W"],$d2["vis_4"],$ext);
			$f->textarea("f_vis_5",$visit_ri_asuhan_keperawatan["vis_5"] ,1, $visit_ri_asuhan_keperawatan["vis_5"."W"],$d2["vis_5"],$ext);
			$f->textarea("f_vis_6",$visit_ri_asuhan_keperawatan["vis_6"] ,1, $visit_ri_asuhan_keperawatan["vis_6"."W"],$d2["vis_6"],$ext);
			$f->textarea("f_vis_7",$visit_ri_asuhan_keperawatan["vis_7"] ,1, $visit_ri_asuhan_keperawatan["vis_7"."W"],$d2["vis_7"],$ext);
			$f->textarea("f_vis_8",$visit_ri_asuhan_keperawatan["vis_8"] ,1, $visit_ri_asuhan_keperawatan["vis_8"."W"],$d2["vis_8"],$ext);
			...
			echo"</div>";
			echo "<div align=right><b>RM 08</b></div>";
