...
$mr = isset($_GET["mr"])? $_GET["mr"] : $_POST["mr"];
		...
		if (isset($_GET["del"])) {
		    ...
		    unset($_SESSION[""]);
		    foreach ($temp as $k => $v) {
		        if ($k != $_GET["del-"]) $_SESSION[""][count($_SESSION[""])] = $v;
		    }
		    	header("Location: $SC?p=" . $_GET["p"] . "&list=&rg=" . $_GET["rg"]."&mr=" . $_GET["mr"] . "&sub=");
		    	exit;
		    ...
		    $r = pg_query($con,"SELECT * FROM RSV0005 WHERE DIAGNOSIS_CODE = '" . $_GET[""] . "'");
		    $d = pg_fetch_object($r);
		    pg_free_result($r);
		    ...
		    if (strlen($d->description) > 0) {
		        $_SESSION[""][$cnt]["id"]   = $_GET[""];
		        $_SESSION[""][$cnt]["desc"] = $d->description;
		        $_SESSION[""][$cnt]["kate"] = $d->category;
		        unset($_SESSION["SELECT_"]);
		    }
		    header("Location: $SC?p=" . $_GET["p"] . "&list=&rg=" . $_GET["rg"]."&mr=" . $_GET["mr"] . "&sub=");
		    exit;
...
title("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  INSTALASI BEDAH SENTRAL");
...
$reg = $_GET["rg"];
$reg2 = $_GET["rg"];
	$tab_disabled = array("pemeriksaan"=>true, "layanan"=>true, "riwayat"=>true,"riwayat_klinik"=>true,"unit_rujukan"=>true,"konsultasi"=>true,"resepobat"=>true);
	if ($_GET["act"] == "del" ) {
	$tab_disabled = array("pemeriksaan"=>false, "layanan"=>false,"riwayat"=>false,"riwayat_klinik"=>false,"unit_rujukan"=>false,"konsultasi"=>false,"resepobat"=>false);
	...
	//$T->addTab("$SC?p=$PID&list=&rg=$rg&poli=".$_GET["mPOLI"]."&mr=$mr&sub=", "Pilih I C D"	, $tab_disabled[""]);
	$T->addTab("$SC?p=$PID&list=riwayat&rg=$rg&poli=".$_GET["mPOLI"]."&mr=$mr", "Riwayat Klinik"	, $tab_disabled["riwayat"]);
	//Fpemr$T->addTab("$SC?p=$PID&list=riwayat_klinik&rg=$rg&mr=$mr", "Riwayat Medis"	, $tab_disabled["riwayat_klinik"]);
...
if ($_GET["sub"] == "byr") {
	...
	$f->hidden("sub",$_GET["sub"]);
        ...
        echo "<INPUT TYPE=HIDDEN NAME=list VALUE=''>";
        echo "<INPUT TYPE=HIDDEN NAME=httpHeader VALUE='1'>";
        echo "</form>";
        echo "<td valign=top>";

        $nama = getFromTable("SELECT DESCRIPTION FROM RSV0005 WHERE DIAGNOSIS_CODE = '".$_SESSION["SELECT_"]."'");
        $kat = getFromTable("SELECT CATEGORY FROM RSV0005 WHERE DIAGNOSIS_CODE = '".$_SESSION["SELECT_"]."'");
        ...
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=httpHeader VALUE='1'>";
        $t->printTableHeader(Array("KODE ", "KETERANGAN","KATEGORI", "&nbsp;"));
        
        if (is_array($_SESSION[""])) {
            foreach($_SESSION[""] as $k => $l) {
                $t->printRow(
                    Array($l["id"], $l["desc"],$l["kate"], "<A HREF='$SC?p=$PID&list=&rg=".$_GET["rg"]."&mr=".$_GET["mr"]."&del-=$k&httpHeader=1'>".icon("del-left")."</A>"), 
        ...
        $t->printRow(
            Array("<INPUT OnKeyPress='refreshSubmit()' NAME= STYLE='text-align:center' TYPE=TEXT SIZE=5 MAXLENGTH=10 VALUE='".$_SESSION["SELECT_"]."'>&nbsp;<A HREF='javascript:select()'><IMG BORDER=0 SRC='images/icon-view.png'></A>", $nama,"$kat", "<INPUT NAME='submitButton' TYPE=SUBMIT VALUE='OK'>"),
            Array("CENTER", "LEFT", "LEFT","CENTER")
        );
		// --- eof 27-12-2006 ---
        echo "</FORM>";
        ...
        echo "sWin = window.open('popup/.php', 'xWin', 'top=0,left=0,width=500,height=400,menubar=no,scrollbars=yes');\n";
        echo "sWin.focus();\n";
        echo "}\n";
        echo "</script>\n";
        
        echo "<form name='Form9' action='actions/napza.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=poli VALUE='".$_GET["mPOLI"]."'>";
        ...
        echo "<form name='Form10' action='actions/napza.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<input type=hidden name=rawatan value='".$rawatan."'>";
	...
	*/
    	...
    	$f = new Form("actions/napza.insert.php", "POST", "NAME=Form2");
					$f->hidden("act","new1");
		...
		$T->show(0);
	    	...
	    	$n = pg_num_rows($r);		    	
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
					...
					echo"<div align=left class=FORM_SUBTITLE1><U> PASIEN</U></div>";
					//unset($_SESSION["SELECT_EMP"]);
					if (isset($_SESSION["SELECT_EMP"])) {
			            ...
			            $f->textAndButton3("f_id_dokter","Dokter Pemeriksa",2,10,$_SESSION["DOKTER"]["id"],$ext,"nm2",30,70,$_SESSION["DOKTER"]["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");$i = 1;echo "<font size='3pt'><b> : </font></b><select><option>regular</option><option>therapy comunity</option><option>detoksifikasi</option></select></td></tr></table>";	
			            
    					unset($_SESSION["SELECT_EMP"]);
					}elseif ($d2["id_dokter"] != '') {
							$f->textAndButton3("f_id_dokter","Dokter Pemeriksa",2,10,$d2["id_dokter"],$ext,"nm2",30,70,$d2["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");echo "<font size='3pt'><b> : </font></b><select><option>regular</option><option>therapy comunity</option><option>detoksifikasi</option></select></td></tr></table>";
					}else{
						$f->textAndButton3("f_id_dokter","Dokter Pemeriksa",2,10,0,$ext,"nm2",30,70,$d2["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
						$f->textAndButton3("f_id_dokter","Dokter Bedah",2,10,0,$ext,"nm2",30,70,$d2["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
						$f->textAndButton3("f_id_dokter","Dokter Anastesi",2,10,0,$ext,"nm2",30,70,$d2["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
						$f->textAndButton3("f_id_dokter","Dokter Internist",2,10,0,$ext,"nm2",30,70,$d2["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
					}
					
					
					$max = count($visit_jantung) ; 
					$i=1;
								$f->textarea("f_vis_".$i,"Keluhan Utama",1, $visit_jantung["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
								$f->textarea("f_vis_".$i,"Riwayat Penyakit Dahulu",1, $visit_jantung["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
								$f->textarea("f_vis_".$i,"Riwayat Penyakit Sekarang",1, $visit_jantung["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
								$f->textarea("f_vis_".$i,"Faktor Resiko",1, $visit_jantung["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
								$f->textarea("f_vis_".$i,"Alergi",1, $visit_jantung["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
					while ($i<= $max) {		
						if ($visit_jantung["vis_".$i."F"] == "memo1") {
								$f->textarea1("<U>DIAGNOSA KERJA</U>","f_vis_".$i,$visit_jantung["vis_".$i] ,1, $visit_jantung["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
						
						}elseif ($visit_jantung["vis_".$i."F"] == "edit") {
		        				$f->text("f_vis_".$i,$visit_jantung["vis_".$i],$visit_jantung["vis_".$i."W"],$visit_jantung["vis_".$i."W"],ucfirst($d2[3+$i]),$ext);
		    			
						}elseif ($visit_jantung["vis_".$i."F"] == "edit4") {
					    		$f->text_4("<U>PEMERIKSAAN FISIK</U>","f_vis_10",$visit_jantung["vis_10"],10,20,$d2["vis_10"],"Kg","f_vis_6",$visit_jantung["vis_6"],10,20,$d2["vis_6"],"mm Hg","f_vis_7",$visit_jantung["vis_7"],10,20,$d2["vis_7"],"/Menit","f_vis_8",$visit_jantung["vis_8"],10,20,$d2["vis_8"],"",$ext);
						    		
					    }elseif ($visit_jantung["vis_".$i."F"] == "edit5") {
								$f->text_4("","f_vis_9",$visit_jantung["vis_9"],10,20,ucfirst($d2["vis_9"]),"","f_vis_5",$visit_jantung["vis_5"],10,20,ucfirst($d2["vis_5"]),"","f_vis_11",$visit_jantung["vis_11"],10,20,$d2["vis_11"],"&deg;C","f_vis_12",$visit_jantung["vis_12"],10,20,$d2["vis_12"],"",$ext);			    	
					    }
			 		$i++ ; 	
					}
					//$f->PgConn=$con;
