...
echo "<br>";
echo"<div align=center class=form_subtitle1>RINCIAN LAYANAN TINDAKAN</div>";
echo "<br>";
?>
<table width="100%">
	<tr>
		...
		<td align="center" class="TBL_HEAD" width="5%">BATAL</td>
		<td align="center" class="TBL_HEAD" width="5%">Cetak</td>
	</tr>
...
if ($rec > 0){
$sqla="select distinct a.trans_form,a.is_bayar,a.id,to_char(a.tanggal_trans,'dd-mm-yyyy') as tanggal_trans,a.no_reg,b.id as item_id, 
		upper(b.description) as description, a.qty, a.diskon, a.tagihan, c.nama
from rs00008 a
	   ...
	   ?>
		...
		<tr>
			...
			<?
				...
				//echo "<a href='actions/$PID.delete.php?rg=$rg&mr=$mr&tbl=del_paket&del=$row1[id]&poli=".$_GET["poli"]."&sub2=".$_GET["sub2"]."'>".icon("del-left","Hapus")."</a>";
                echo "<a href='actions/p_pelayanan.delete.php?rg=$rg&mr=$mr&tbl=del_paket&del=$row1[id]&poli=".$_GET["poli"]."&sub2=".$_GET["sub2"]."&pid=$PID'>".icon("del-left","Hapus")."</a>";
				}else{
				echo " &nbsp; ";
				}
			?>
				</td>
                                <td class="TBL_BODY" align="center"><input type="checkbox" name="check_to_print"></td>
		</tr>
		<?
		...
		$sqlc= "select z.item_id,z.preset_id,to_char(tanggal_trans,'DD-MM-YYYY') as tanggal_trans,  
				...
				where to_number(a.no_reg,'999999999999')= $reg  and a.referensi = 'P' and z.preset_id = $row1[item_id]
				group by  z.preset_id,z.item_id,d.tdesc, a.tanggal_trans, a.id, b.obat, z.qty, a.pembayaran, a.trans_group, c.tdesc, a.trans_form ORDER BY a.tanggal_trans DESC";
		...
		if ($rec1 > 0){
		$sqle= "select distinct  f.trans_form,f.id,f.item_id, a.layanan, f.referensi, f.diskon, 
		...
		<tr>
			...
			<td class="TBL_BODY" align="right"><?=number_format($row4["referensi"],2,",",".")!=0?number_format($row4["referensi"],2,",","."):"" ?></td>
			...
			<?
				...
				echo "<a href='actions/$PID.delete.php?rg=".$_GET["rg"]."&mr=".$_GET["mr"]."&sub=".$_GET["sub"]."&tbl=tindakan&del=$row4[id]&sub2=".$_GET["sub2"]."'>".icon("del-left","Hapus")."</a>";
                }else{
				echo "&nbsp;";
				}
			?>
                                ...
                                <td class="TBL_BODY" align="center">
                                    <?php if(($row4["trans_form"] == 'p_laboratorium') || $row4["trans_form"] == 'p_radiologi') {?>
                                    <input type="checkbox" name="check_to_print_<?php echo $j4?>" id="check_to_print_<?php echo $j4?>" value="<?php echo $row4['id']?>"> 
                                    <?php }?>
                                </td>
		<?
		...
		group by  a.is_bayar, d.tdesc, a.tanggal_trans, a.id, b.obat, a.qty, a.pembayaran, a.trans_group, c.tdesc, a.trans_form ORDER BY tanggal_trans desc";
		...
		<tr>
		<td bgcolor="#8ADFD3" align="center">&nbsp;</td>
		<td bgcolor="#8ADFD3" align="left"><b><u>RINCIAN BHP DI POLI</u></b></td>
		<td bgcolor="#8ADFD3" align="center">&nbsp;</td>
		<td bgcolor="#8ADFD3" align="center">&nbsp;</td>
		<td bgcolor="#8ADFD3" align="right">&nbsp;</td>
		<td bgcolor="#8ADFD3" align="right">&nbsp;</td>
		...
		<tr>
			...
			<td class="TBL_BODY" align="center">
				...
				echo "<a href='actions/$PID.delete.php?rg=".$_GET["rg"]."&mr=".$_GET["mr"]."&sub=layanan&tbl=bhp&del=$row7[id]&sub2=bhp'>".icon("del-left","Hapus")."</a>";
                }else{
				echo "&nbsp;";
				}
			?>
		...
		and b.kategori_id = d.tc and d.tt = 'GOB' 
		and to_number(a.no_reg,'999999999999')= $reg  and referensi != 'F'
		group by  d.tdesc, a.tanggal_trans, a.id, b.obat, a.qty, a.pembayaran, a.trans_group, c.tdesc, a.trans_form ORDER BY tanggal_trans DESC";
...
// Pembelian Obat Racikan
		...
		<tr>
			<td class="TBL_BODY" align="right" colspan="6"><b>T O T A L &nbsp;&nbsp;&nbsp;&nbsp; T A G I H A N</b></td>
			<td class="TBL_BODY" align="right"><b><?=number_format($tagihan + $tagihan2 + $tagihan5 + $tagihan7 + $tagihan6,2,",",".") ?>&nbsp;</b></td>
			<td class="TBL_BODY" align="right">&nbsp;</td>
			<td class="TBL_BODY" align="right">&nbsp;</td>
		</tr>
		<tr>
			<td class="TBL_BODY" align="right" colspan="6"><b>T O T A L &nbsp;&nbsp;&nbsp;&nbsp; P E M B A Y A R A N</b></td>
			<td class="TBL_BODY" align="right"><b><?=number_format($d2->bayar,2,",",".") ?></b>&nbsp;</td>
			<td class="TBL_BODY" align="right">&nbsp;</td>
			<td class="TBL_BODY" align="right">&nbsp;</td>
		</tr>
		<tr>
			<td class="TBL_BODY" align="right" colspan="6"><b>S I S A &nbsp;&nbsp;&nbsp;&nbsp; P E M B A Y A R A N</b></td>
			<td class="TBL_BODY" align="right"><b><?=number_format($d2->sisa,2,",",".") ?></b>&nbsp;</td>
			<td class="TBL_BODY" align="right">&nbsp;</td>
			<td class="TBL_BODY" align="right">&nbsp;</td>
		</tr>
</table>
<br/>
<input type="button" value="Cetak Rincian Laboratorium" onClick="cetakLab('<?php echo $_GET["rg"]?>')"> &nbsp;&nbsp;
<input type="button" value="Cetak Rincian Radiologi" onClick="cetakRad('<?php echo $_GET["rg"]?>')">
<script>
function cetakLab(no_reg){
    maxI = <?php echo $j4?>;
    selectedToPrint = '&max_cetak='+maxI;
    for(i=0;i<=maxI;i++){
        if($('#check_to_print_'+i).is(':checked') == true){
            obatSelected = $('#check_to_print_'+i).val();
            selectedToPrint = selectedToPrint+'&cetak_'+i+'='+obatSelected;
        }
    }
    window.open('includes/cetak.tindakan_laboratorium.php?no_reg='+no_reg+''+selectedToPrint, 'xWin','top=0,left=0,width=750,height=550,menubar=no,location=no,scrollbars=yes');
}
function cetakRad(no_reg){
    maxI = <?php echo $j4?>;
    selectedToPrint = '&max_cetak='+maxI;
    for(i=0;i<=maxI;i++){
        if($('#check_to_print_'+i).is(':checked') == true){
    ...
    window.open('includes/cetak.tindakan_radiologi.php?no_reg='+no_reg+''+selectedToPrint, 'xWin','top=0,left=0,width=750,height=550,menubar=no,location=no,scrollbars=yes');
}
</script>
