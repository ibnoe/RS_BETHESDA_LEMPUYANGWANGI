...
//print
if (!$GLOBALS['print']){
    title_print("<img src='icon/keuangan-2.gif' align='absmiddle' >  Data Master Tarif Layanan");
    title_excel("834&L1=".$_GET["L1"]."&L2=".$_GET["L2"]."&L3=".$_GET["L3"]."&L4=".$_GET["L4"]."");
} else {
    title_print("<img src='icon/keuangan-2.gif' align='absmiddle' >  Data Master Tarif Layanan");
    title_excel("834&L1=".$_GET["L1"]."&L2=".$_GET["L2"]."&L3=".$_GET["L3"]."&L4=".$_GET["L4"]."");
}
...
if ($_GET["action"] == "new") {
    ...
    if ($_GET["f_is_group"] == "Y") {
		...
		$f->selectSQL("f_tipe_pasien_id","Tipe Pasien",
            ...
            "order by tdesc DESC", $_GET["f_tipe_pasien_id"],$ext);
...
*/
		...
		$f->text("f_dll","Administrasi",12,12,"0.00","style='text-align:right'");
...
*/
	...
	$f->text("f_dll","Administrasi",12,12,$d->dll,"style='text-align:right'");
...
if (empty($_GET[sure])) {
	...
	if ($_SESSION[gr]=="RSBL-LABORATORIUM" || $_SESSION[gr]=="RSBL-FISIOTERAPI"){
	$_GET["L1"] = "004000000000000";
	$ext="DISABLED";
	}if ($_SESSION[gr]=="RSBL-RADIOLOGI"){
	$_GET["L1"] = "004000000000000";
	$_POST["L1"] = "004000000000000";
	$ext2="DISABLED";
    ...
    if (getFromTable(
		...
		if ($_SESSION[gr]=="RSBL-LABORATORIUM"){
		$_GET["L2"] = "004003000000000";
		$ext="DISABLED";
		}else if ($_SESSION[gr]=="RSBL-FISIOTERAPI"){
		$_GET["L2"] = "004001000000000";
		$ext="DISABLED";
		}else if ($_SESSION[gr]=="RSBL-RADIOLOGI"){
		$ext2="DISABLED";
		$_GET["L2"] = "004002000000000";
    ...
    $SQL1 = "select a.layanan, e.tdesc as pendapatan, ".
			//"d.tdesc as golongan, ". 
			"f.tdesc as pasien, ".
			//"c.tdesc as klasifikasi_tarif, ".
			"b.tdesc as satuan, ".
            //"a.harga_atas, a.harga_bawah, ". 
            "a.harga, a.jasa_dokter, a.jasa_asisten, a.jasa_rs, a.alat, a.bahan, a.dll, g.jasa_medis, ".
 ...
 //   echo "<input type=submit value=' CARI '>";
    ...
    if ($isGroup == "N") {
        ...
        */
        ...
        if (isset($_GET[L1])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/834.delete.php?f_is_group=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#13#>&L1=".$_GET[L1]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L2])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/834.delete.php?f_is_group=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#13#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L3])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/834.delete.php?f_is_group=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#13#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&L3=".$_GET[L3]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L4])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/834.delete.php?f_is_group=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#13#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&L3=".$_GET[L3]."&L4=".$_GET[L4]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        if (isset($_GET[L5])) {
           $del_link = "<A CLASS=TBL_HREF HREF='actions/834.delete.php?f_is_group=N&sort=".$_GET[sort]."&order=".$_GET[order]."&e=<#13#>&L1=".$_GET[L1]."&L2=".$_GET[L2]."&L3=".$_GET[L3]."&L4=".$_GET[L4]."&L5=".$_GET[L5]."&tblstart=".$_GET[tblstart]."'>".icon("delete","Hapus")." ";
        }
        $del_link .= "</a>";
        $t->ColFormatHtml[13] =
            "<A CLASS=TBL_HREF HREF='".
            "$SC?p=$PID&action=edit&parent=".$_GET["L$level"]."&f_is_group=N&e=<#13#>&sort=".$_GET[sort]."&order=".$_GET[order]."&tblstart=".$_GET[tblstart].
            "'>".icon("edit","Edit")."</A> ".
			//"$del_link".
			"";
        $t->ColHeader = Array("LAYANAN","PENDAPATAN", 
							//"GOL. TINDAKAN",
							"TIPE PASIEN",
							//"KLASIFIKASI TARIF", 
							"SATUAN",
                            //"HARGA ATAS", "HARGA BAWAH", 
                            "HARGA", "JASA DOKTER", "JASA ASISTEN", "JASA RS", "ALAT", "BAHAN", "ADMINISTRASI", "SUMBER DANA", 
