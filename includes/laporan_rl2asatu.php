...
subtitle_print("DATA KEADAAN MORBIDITAS RAWAT INAP SURVEILANS TERPADU RUMAH SAKIT");
subtitle_print("Bulan : ".$set_triwulan);
subtitle_rs("FORMULIR RL2a1");
subtitle_rs($set_header[0]." ".$set_header[1]);
subtitle_rs("No. Kode RS : ".$set_kode_rs); 	  
		
	if(!$GLOBALS['print']){
	 	title_print("");
		title_excel("laporan_rl2asatu");
        edit_laporan("input_rl2asatu");
	}else {
	
			
	}
	
	$SQL = "select * from rl200002a1 order by no_dtd";
