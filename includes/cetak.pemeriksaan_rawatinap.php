...
$rt = pg_query($con,"select DISTINCT a.*,(b.nama)as periksa,to_char(a.tanggal_reg,'dd Month yyyy')as tanggal_reg,
				(h.nama)as pengirim,(i.nama)as operator,f.nama as nm_pasien,f.mr_no,f.alm_tetap, f.kota_tetap,g.tdesc as poli_asal, (f.tgl_lahir::timestamp with time zone) AS umur, d.rawat_inap ,case when f.jenis_kelamin='L' then 'Laki-laki' else 'Perempuan' end as jk, z.tdesc as tipe, tanggal(CURRENT_DATE,0) as tgl_cetak,(CURRENT_TIME) as wkt
						...
						where a.no_reg='$reg' AND (a.id_konsul = '203' OR a.id_poli = 203)");
    ...
    $r12 = pg_query($con,
                    "select a.id, a.ts_check_in::date, e.bangsal, d.bangsal as ruangan, b.bangsal as bed, 
                        ...
                        a.ts_calc_stop 
                    from rs00010 as a 
                        LEFT join rs00012 as b on a.bangsal_id = b.id
                        LEFT join rs00012 as d on substr(b.hierarchy,1,6) || '000000000' = d.hierarchy 
                        LEFT join rs00012 as e on substr(b.hierarchy,1,3) || '000000000000' = e.hierarchy 
                        LEFT join rs00001 as c on d.klasifikasi_tarif_id = c.tc and c.tt = 'KTR' 
                    where a.no_reg = '$reg' and ts_calc_stop is null");
    ...
    <table cellpadding="0" cellspacing="0" class="items"><tbody><tr>
			...
			<td width="50%">
			 <table class="none" cellpadding="0" cellspacing="0"> 
			    ...
			    <tr>
                                <td><font size="1"  face="Arial"><? echo $rawatan; ?></td>
                                <td><font size="1"  face="Arial">:</td>
...
$id_print = implode(',',$_GET['cetak']);
		  ...
		  where a.no_reg ='".$_GET['rg']."' and a.id_poli = '203' and is_inap='I' AND a.id IN (".$id_print.") order by id";
			...
			{?><tr>
			    <td  align="left"><font size="1"  face="Arial"><?= $row5["parameter"] ?></td>
                            <td  align="left"><font size="1"  face="Arial"><B><?= $row5["hasil"] ?></B></td>
			    <td  align="left"><font size="1"  face="Arial"><?= $row5["rentang_normal"]?></td>
			    <td  align="left"><font size="1"  face="Arial"><?= $row5["satuan"]?></td>
			    <td  align="left"><font size="1"  face="Arial"><?= $row5["keterangan"]?></td>
                        </tr><?}else{ ?>
                        <tr>
