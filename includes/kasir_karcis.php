...
if (!$GLOBALS['print']){
    	...
    	title_print("");
    title_excel("kasir_karcis&tblstart=".$_GET['tblstart']);
    }
...
if (isset($_SESSION["SELECT_KARCIS1"])) {

    $_SESSION["jmk"]["rg"]   = $_SESSION["SELECT_KARCIS1"];
    //$efri = getFromTable("select jmk from master_karcis where id = '".$_SESSION["SELECT_KARCIS"]."'");
    $_SESSION["jmk"]["nama"] =
        getFromTable("select a.nama from rs00002 a, rs00006 b where a.mr_no=b.mr_no and b.id='".$_SESSION["SELECT_KARCIS1"]."'");
    $_SESSION["jmk"]["alamat"] =
        getFromTable("select a.alm_tetap from rs00002 a, rs00006 b where a.mr_no=b.mr_no and b.id='".$_SESSION["SELECT_KARCIS1"]."'");
    $_SESSION["jmk"]["harga"] =
    	getFromTable("select harga  from master_karcis where id = '".$_SESSION["SELECT_KARCIS"]."'");
...
echo "<tr><td class=FORM>No. Reg</td><td class=FORM>:</td>";
echo "    <td class=FORM width=5><input style='text-align:left' type=TEXT name=reg1 size=20 maxlength=20 value='".$_SESSION["jmk"]["rg"]."' disabled></td>";
echo "    <td class=FORM width=500><a href='javascript:selectreg()'>".icon("view")."</a></td></tr>";
echo "<input style='text-align:left' type=hidden name=reg size=20 maxlength=20 value='".$_SESSION["jmk"]["rg"]."'>";
...
echo "<tr><td class=FORM>No Karcis</td><td class=FORM>:</td>";
echo "    <td class=FORM colspan=2><input type=TEXT name=no_karcis size=50 maxlength=50 value='".$_SESSION["jmk"]["no_karcis"]."'></td></tr>";
...
echo "    <td class=FORM colspan=2><input type=SUBMIT value='SIMPAN'></td></tr>";
echo "</tr></table>";
echo "</form>";
...
echo "function selectreg() {\n";
echo "    sWin = window.open('popup/registrasi.php', 'xWin',".
     "    'width=500,height=400,menubar=no,scrollbars=yes');\n";
echo "    sWin.focus();\n";
echo "}\n";
echo "</script>\n";
...
if (isset($_SESSION["SELECT_KARCIS"])) {
    ...
    $_SESSION["jmk"]["alamat"] = $d2->alamat;
     $_SESSION["jmk"]["no_karcis"] = $d2->no_karcis;
    //unset($_SESSION["SELECT_KARCIS"]);
	}else{
...
//    echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = 
        "select a.no_karcis, a.no_reg,a.no_mr, a.nama, a.alamat, a.tanggal_reg, c.tdesc, b.code, a.id as dummy ".
        "from kasir_karcis a ".
        ...
        "GROUP BY a.no_karcis,a.no_reg,a.no_mr,  a.nama, a.alamat, a.tanggal_reg, c.tdesc, b.code, a.id  ".
		"ORDER by (a.tanggal_reg,a.no_karcis) desc".
		"";
    $t->setlocale("id_ID");
    ...
    $t->ColFormatHtml[8] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#8#>'>".icon("edit","Edit")."</A>".
            "<A CLASS=TBL_HREF HREF='actions/kasir_karcis.delete.php?p=$PID&id=<#8#>'
onclick=\"return confirm('Apakah Anda benar-benar akan menghapus ?')\"
>".icon("delete","Hapus")."</A>".
			"<A CLASS=TBL_HREF HREF='includes/cetak.karcis.php?rg=<#8#>'>".icon("print","Cetak")."</A>";;;
    $t->ColHeader = array("NO. KARCIS","NO.REG","NO.MR","NAMA", "ALAMAT", "TANGGAL/WAKTU","KARCIS","POLIKLINIK","");
