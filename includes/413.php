...
if($_GET["tc"] == "view") {
    ...
    title_print("LAPORAN PENDAPATAN RAWAT JALAN");
...
*************************************************************/
	...
	$SQL = "select no_reg, mr_no, to_char(tgl_masuk, 'DD MON YYYY') as tgl_in  ".
		"   , nama, pangkat_gol, nrp_nip, kesatuan, poli, pasien  ".
		"from rsvrj_new ".
		"   left join rs00001 a on a.tdesc = rsvrj_new.pasien and a.tt = 'JEP' ".
		"where (tgl_masuk between '$ts_check_in1' and '$ts_check_in2') ".
		"   and a.tc like '%".$_GET[mPASIEN]."%'";
    ...
    if ($_GET[geser] == "kiri" || empty($_GET[geser])) {
    $t->ColHeader = array("NO.REG","NO.MR","TGL DATANG","NAMA","PANGKAT", "NRP/NIP","KESATUAN","RAWATAN","TIPE PASIEN");
    $t->ColAlign[7] = "CENTER";
    } elseif ($_GET[geser] == "kanan") {
    $t->ColHeader = array("NO.REG","NO.MR","TGL MASUK","NAMA",
			"SARANA","BAHAN","PEMERIKSAAN","OBAT PAKET","OBAT LANJUT","JML BAYAR");
