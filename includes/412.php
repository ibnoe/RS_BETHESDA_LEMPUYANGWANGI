...
*************************************************************/
	...
	$SQL = "select no_reg, mr_no, to_char(tgl_masuk, 'DD MON YYYY') as tgl_in, to_char(tgl_pulang, 'DD MON YYYY')  as tgl_out".
		"   , nama, umur, alm_tetap, bangsal, kelas ".
		"from rsvmondok ".
		"   left join rs00001 a on a.tdesc = rsvmondok.pasien and a.tt = 'JEP' ".
		"where (tgl_pulang between '$ts_check_in1' and '$ts_check_in2') ".
		"   and a.tc like '%".$_GET[mPASIEN]."%'";
...
*/
	...
	$SQL = "select no_reg, mr_no, to_char(tgl_masuk, 'DD MON YYYY') as tgl_in, to_char(tgl_pulang, 'DD MON YYYY') as tgl_out ".
		"   , nama, sarana, bahan, periksa, obat, jmlbayar, jmlhari, pasien ".
		"from rsvmondok ".
		"   left join rs00001 a on a.tdesc = rsvmondok.pasien and a.tt = 'JEP' ".
		"where (tgl_pulang between '$ts_check_in1' and '$ts_check_in2') ".
		"   and a.tc like '%".$_GET[mPASIEN]."%'";
...
*/
    ...
    if ($_GET[geser] == "kiri" || empty($_GET[geser])) {
    $t->ColHeader = array("NO REG","MR/NO","TGL MASUK","TGL PULANG","NAMA","UMUR","ALAMAT",
			"ZAAL", "KELAS");
    $t->ColAlign[5] = "CENTER";			
    $t->ColAlign[7] = "CENTER";
    $t->ColAlign[8] = "CENTER";
    } elseif ($_GET[geser] == "kanan") {
    $t->ColHeader = array("NO REG","MR/NO","TGL MASUK","TGL PULANG","NAMA",
    ...
    $t->ColFooter[5] =  number_format($d2->sarana,2,',','.');
    $t->ColFooter[6] =  number_format($d2->bahan,2,',','.');
    $t->ColFooter[7] =  number_format($d2->periksa,2,',','.');
    $t->ColFooter[8] =  number_format($d2->obat,2,',','.');
    $t->ColFooter[9] =  number_format($d2->jmlbayar,2,',','.');
    $t->ColFooter[10] =  number_format($d2->jmlhari,0,',','.');
