...
if (strlen($_GET["id"]) > 0 && empty($_GET[sure])) {
    if ($_GET["id"] == "new") {
        ...
        $f = new Form("actions/998.3.insert.php?configtype=".$_GET[configtype], "POST");
        $f->text("description","Nama Paket Radiologi",40,100,"");
        $f->submit(" Simpan ");
        $f->execute();
    } else {
        ...
        echo "<br>";
        $f = new Form("actions/998.3.update.php?configtype=".$_GET[configtype], "POST");
        $f->hidden("id", $_GET["id"]);
        $f->text("description","Nama Paket",40,100,$description);
        $f->submit(" Simpan ");
        $f->execute();

        echo "<div align=right><a href='$SC?p=$PID&configtype=".$_GET[configtype]."'>Kembali Ke Paket Radiologi</a></div>";
        echo "<hr size=1 noshade>";
        echo "<b>\"$description\" terdiri dari layanan berikut:</b><br><br>";

        echo "<FORM ACTION='actions/998.3.insert.php?configtype=".$_GET[configtype]."' METHOD=POST>";
        echo "<INPUT TYPE=HIDDEN NAME=id VALUE='" . $_GET["id"] . "'>";
        $t = new BaseTable("100%");
        $t->printTableOpen();
        $t->printTableHeader(Array("KODE", "Layanan", "Jumlah", "Satuan", "Harga Satuan", "Harga Total", ""));
        $r1 = pg_query($con,
            "select z.item_id, c.layanan, z.qty, c.harga, c.harga_atas, c.harga_bawah, a.tdesc, z.id, b.tdesc as jenis_jasa " .
            "from rs99997 z " .
            "    left join rs99996 d on d.id = z.preset_id and d.trans_type = 'RAD' ".
            "    left join rs00034 c on c.id = z.item_id ".
            "    left join rs00001 a on c.satuan_id = a.tc and a.tt = 'SAT' ".
			"	 left join rs00001 b on b.tc = c.sumber_pendapatan_id and b.tt = 'SBP' ".
            "where z.preset_id = '".$_GET["id"]."'"
            );
        while ($d1 = pg_fetch_object($r1)) {
        $t->printRow(
            Array($d1->item_id, $d1->layanan." - ".$d1->jenis_jasa, $d1->qty, $d1->tdesc,
                number_format($d1->harga,2), number_format($d1->harga * $d1->qty,2),
                "<A HREF='actions/998.3.delete.php?id=$d1->id&return=".$_GET["id"]."&configtype=".$_GET[configtype]."'>".icon("del-left")."</A>"),
            Array("CENTER", "LEFT", "RIGHT", "LEFT", "RIGHT", "RIGHT", "CENTER")
            );
        }
        pg_free_result($r1);
        //echo "xxxxxx: ".$_SESSION["SELECT_LAYANAN"];
        ...
        $t->printRow(
			...
			"'>&nbsp;<A HREF='javascript:selectLayanan()'><IMG BORDER=0 SRC='images/icon-view.png'></A>", $d->layanan." - ".$d->jenis_jasa,
			"<INPUT VALUE='".(isset($_GET["jumlah"]) ? $_GET["jumlah"] : "1")."'NAME=jumlah OnKeyPress='refreshSubmit()' TYPE=TEXT SIZE=5 MAXLENGTH=10 VALUE='1' STYLE='text-align:right'>", 
    ...
    echo "<form action='actions/998.3.delete.php' method='get'>";
    ...
    echo "<input type=hidden name=id value=".$_GET[id].">";
    echo "<input type=hidden name=configtype value=3>";
  ...
  //  echo "<input type=submit value=' CARI '>";
    ...
    $t->ColFormatHtml[1] = "<nobr>".
                           ...
                           "<A CLASS=TBL_HREF HREF='actions/998.3.delete.php?configtype=".$_GET[configtype]."&id=<#1#>'>".icon("delete","Hapus")."</A>".
                           "</nobr>";
    $t->execute();

    echo "<div align=LEFT><img src='icon/group.gif' align='absmiddle'> <a href='$SC?p=$PID&configtype=".$_GET[configtype]."&id=new'>Tambah Paket Radiologi</a></div>";
