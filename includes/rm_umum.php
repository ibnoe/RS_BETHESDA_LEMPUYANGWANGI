...
if ($_GET['act'] ==  "detail"){
		if (!$GLOBALS['print']){
			title_print("<img src='icon/medical-record-2.gif' align='absmiddle' > REKAM MEDIS KLINIK UMUM");
			echo "<DIV ALIGN=RIGHT OnClick='window.history.back()'>".icon("back","Kembali")."</a></DIV>";
		}else{
			title_print("<img src='icon/medical-record.gif' align='absmiddle' > REKAM MEDIS KLINIK UMUM");
		}
			    ...
			    //echo $bangsal;
						$sql2 = "select a.mr_no,a.nama,a.umur,a.tgl_lahir,a.tmp_lahir,a.status_akhir, ".
	...
	if ($_GET["v"]){ // v from 430.php
    	...
    	if (!$GLOBALS['print']){
			...
			$SQL = "select distinct a.mr_no,a.nama,a.pangkat_gol,a.nrp_nip,a.kesatuan,a.alm_tetap ".
					"from rsv_pasien2 a ".
			...
			echo "<br>";
			    ...
			    $t->setlocale("id_ID");
			   	$t->ShowRowNumber = true;
			   	$t->ColHeader = array( "NO.RM", "NAMA","PANGKAT","NRP/NIP","KESATUAN","ALAMAT");
				$t->ColAlign = array("center","left","left","left","left","left");
				if (!$GLOBALS['print']){
					$t->RowsPerPage = $ROWS_PER_PAGE;
