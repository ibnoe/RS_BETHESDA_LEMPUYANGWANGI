...
<HTML>
    ...
    $nt1 = pg_num_rows($rt1);
    $dt1 = pg_fetch_object($rt1);

    $rt = pg_query($con,
            "select id as code, mr_no, tanggal_reg, waktu_reg, nama, tlp_tetap, alm_tetap, kota_tetap, diagnosa_sementara from rsv_pasien2 where id::text= '$reg'  ");

    $nt = pg_num_rows($rt);
    $dt = pg_fetch_object($rt);

    $rowsPemakaianObat      = pg_query($con, "SELECT id, tanggal_entry, waktu_entry, item_id, qty, tagihan, referensi, dibayar_penjamin, diskon  
                             FROM rs00008 
                             WHERE trans_type = 'OB1' AND rs00008.no_reg = '".$_GET["rg"]."' order by id ");
	$rowsAdministrasi      = pg_query($con, "SELECT id, tanggal_entry, waktu_entry, item_id, qty, tagihan, referensi, dibayar_penjamin, diskon  
                             FROM rs00008 
                             WHERE trans_type = 'ADA' AND rs00008.no_reg = '".$_GET["rg"]."' order by id ");
    $rowsPemakaianRacikan   = pg_query($con, "SELECT id, tanggal_entry, waktu_entry, item_id, qty, tagihan, referensi, dibayar_penjamin, diskon  
                             FROM rs00008 
	...
	if ($rawatan == "Rawat Inap") {
		$dirawat = 'RAWAT INAP '.'<br>'.'COPY RESEP '.$bangsal;
	} else if ($rawatan == "Rawat Jalan") {
		$dirawat = 'RAWAT JALAN '.'<br>'.'COPY RESEP '.$dt1->poli;
	} else {
		$dirawat = 'RAWAT JALAN '.'<br>'.'COPY RESEP '.$dt1->poli;
	}
	//end header kwitansi
...
if(pg_num_rows($rowsAdministrasi) > 0){
    echo '<tr id="list_obat_to_print" ><td class="" colspan="6"><span style="font-weight: bold;">Administrasi</span></td></tr>';
        $iData          = 0;
        $iObat          = 0;
        $total          = 0;
        $totalPenjamin  = 0;
...
//            $totalSelisih   = $totalSelisih + ($row["tagihan"]-$row["dibayar_penjamin"]);
            ...
            $sqlObat = pg_query($con, "SELECT  tdesc  FROM
...
?>
    <tr>
        <!--<td class="" align="left" height="30" ><?=tanggal($row["tanggal_entry"]). ' &nbsp; ' .$arrJamEntry[0].':'.$arrJamEntry[1]?></td>-->	<td valign="top" class="" align="left" height="15" ><?=$noUrut?>.</td>
...
<table width="47%" border="0" cellpadding="0" cellspacing="0" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;">
    <tr>    
        <td width="47%" colspan="6" align="left" class="TITLE_SIM3">&nbsp;</td>
    </tr>
    <tr>
        <td width="35%" colspan="4" align="center" class="TITLE_SIM3">&nbsp;</td>
        <td width="15%" colspan="2" align="center" class="TITLE_SIM3" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;"><? echo $client_city.", ".$tgl_now."<br>".$_SESSION["nama_usr"]; ?></td>
    </tr>
    <tr>    
        <td width="47%" colspan="6" align="left" class="TITLE_SIM3">&nbsp;</td>
    </tr>
    ...
    <tr>
        <td width="47%" colspan="6" align="left" class="TITLE_SIM3" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;">** Terima Kasih ** <br /> Dokumen dicetak komputer, tidak perlu stempel</td>
    </tr>
</table>
