...
if ($_GET["httpHeader"] == "1") {
    if (isset($_GET["nomor_bukti"])) {
        $_SESSION["ob5"]["nomor-bukti"] = $_GET["nomor_bukti"];
    }
...
title("Transaksi Penerimaan Obat");
echo "<br>";

if (isset($_SESSION["SELECT_SUPPLIER"])) {
    $_SESSION["ob5"]["supplier"]["id"]   = $_SESSION["SELECT_SUPPLIER"];
    $_SESSION["ob5"]["supplier"]["name"] =
...
echo "    <td class=FORM width=1><input style='text-align:center' type=TEXT name=supplier size=5 maxlength=10 value='".$_SESSION["ob5"]["supplier"]["id"]."' DISABLED></td>";
...
echo "    <td class=FORM colspan=2>".$_SESSION["ob5"]["supplier"]["name"]."</td></tr>";
echo "<tr><td class=FORM>Nomor Bukti</td><td class=FORM>:</td>";
echo "    <td class=FORM colspan=2><input type=TEXT name=nomor_bukti size=30 maxlength=30 value='".$_SESSION["ob5"]["nomor-bukti"]."'></td></tr>";
...
if (isset($_SESSION["ob5"]["supplier"]["id"]) && isset($_SESSION["ob5"]["nomor-bukti"])) {
