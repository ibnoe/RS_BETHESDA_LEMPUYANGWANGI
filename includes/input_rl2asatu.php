...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl200002a1 where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/rl2asatu.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Laporan RL2a1");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/rl2asatu.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Laporan RL2a1");
        $f->hidden("id","new");
    ...
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan RL2a1");
lihat_laporan("laporan_rl2asatu");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select no_dtd, no_daftar, golongan, a, b, c, d, e, f, g, h,i,j,k,l, id as href FROM rl200002a1 order by no_dtd";            
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
    ...
    $t->ColFormatHtml[15] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#15#>'>".icon("edit","Edit")."</A>".
            ...
            "actions/rl2asatu.delete.php?p=$PID".
            "&e=<#15#>".
            "'>".icon("delete","Hapus")."</A>".
            "</nobr>"; 
    /*
    ...
    */
	...
	$data = getFromTable("select golongan from rl200002a1 where no='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/rl2asatu.delete.php' method='get'>";
