...
if($_GET["tc"] == "view") {
    if (!$GLOBALS['print']){
        title_print("<img src='icon/keuangan-2.gif' align='absmiddle' > Laporan Penjualan Farmasi (Obat) Per Tanggal");
		title_excel("lap_penj_frms");
    } else {
        title("<img src='icon/keuangan.gif' align='absmiddle' > Penjualan dari Farmasi (Obat) Per Tanggal");
    }
    ...
    $r2 = pg_query($con,
              ...
              "     (a.trans_type::text = 'OB1'::text OR a.trans_type::text = 'RCK'::text) and b.tipe = '".$_GET["u"]."'");

    $d2 = pg_fetch_object($r2);
    pg_free_result($r2);

    $t->SQL = "select c.mr_no, a.no_reg,c.nama, ".
              "     e.obat, a.harga, a.qty, sum(a.tagihan) as tagih ".
              "from rs00008 a  ".
              "     left join rs00006 b ON a.no_reg = b.id ".
    ...
    $t->ColHeader = array("NO.RM","NO.REG","NAMA","NAMA OBAT","HARGA SATUAN","QTY","TOTAL (Rp.)");
    ...
    $f->selectSQL("mUNIT", "U N I T",
        ...
        "where (a.trans_type::text = 'OB1'::text OR a.trans_type::text = 'RCK'::text) and a.no_reg = b.id ", $_GET["mUNIT"],
        $ext);
    */
    ...
    if (strlen($_GET["search"]) > 0) {
        ...
        $r2 = pg_query($con,
                ...
                "from rs00008 a ".
                "   left join rs00006 b ON a.no_reg = b.id ".
                "where (a.trans_type::text = 'OB1'::text OR a.trans_type::text = 'RCK'::text) and ".
    ...
    $SQL =  "select to_char(a.tanggal_trans,'DD Mon YYYY') as tanggal_trans_str, ".
            ...
            "from rs00008 a ".
            "     left join rs00006 b ON a.no_reg = b.id ".
            "where (a.trans_type::text = 'OB1'::text OR a.trans_type::text = 'RCK'::text) ".
    ...
    $t->ColHeader = array("TANGGAL TRANSAKSI","RAWATAN","JUMLAH TRANSAKSI (Rp.)", "V i e w");
