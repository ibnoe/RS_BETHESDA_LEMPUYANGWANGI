<?php 	// Nugraha, Sun Apr 18 18:58:42 WIT 2004
        ...
        // Agung Sunandar	16:22 26/06/2012 MEnambahkan inputan untuk pelayanan Paket
...
if (!$GLOBALS['print']){
title_print("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  PILA KLINIK");
    }
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_pila_klinik.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
	...
	*/
    	if ($_GET["act"] == "detail") {
			...
			$f->text($visit_umum["vis_1"],(ucfirst($d[3])));
			$f->text($visit_umum["vis_2"],(ucfirst($d[4])));
			$f->text($visit_umum["vis_3"],(ucfirst($d[5])));
			$f->text($visit_umum["vis_4"],(ucfirst($d[6])));
    		$f->title1("<U>PEMERIKSAAN FISIK</U>","LEFT");	
			$f->text($visit_umum["vis_5"],(ucfirst($d[7])));
			$f->text($visit_umum["vis_6"],$d[8]."&nbsp;mm Hg");
			$f->text($visit_umum["vis_7"],$d[9]."&nbsp;/Menit");
			$f->text($visit_umum["vis_8"],(ucfirst($d[10]))."&nbsp;/Menit" );    
			$f->execute();
			echo "</td><td valign=top>";
    		$f = new ReadOnlyForm();
			$f->text($visit_umum["vis_9"],(ucfirst($d[11])));
			$f->text($visit_umum["vis_10"],$d[12]."&nbsp;Kg" );
			$f->text($visit_umum["vis_11"],$d[13]."&deg;C");
			$f->text($visit_umum["vis_12"],(ucfirst($d[14])));
			$f->title1("<U>DIAGNOSA KERJA</U>","LEFT");	
			$f->text($visit_umum["vis_13"],(ucfirst($d[15])));
			$f->text($visit_umum["vis_14"],(ucfirst($d[16])));
			$f->text($visit_umum["vis_15"],(ucfirst($d[17])));
 ...
 include("rincianobat.php"); 
		...
		$T->show(0);
    		...
    		//cek lagi scriptnya!!
    		$sql2 =	"SELECT A.*,B.NAMA,c.nama as perawat FROM C_VISIT A 
	    	...
	    	$n = pg_num_rows($r);		    	
				...
				if ($_GET['act'] == "edit") {
					...
					$f = new Form("actions/p_pila_klinik.insert.php", "POST", "NAME=Form2");
					$f->hidden("act","edit");
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
					...
					$max = count($visit_umum) ; 
					$i = 1;
					while ($i<= $max) {		
						if 		($visit_umum["vis_".$i."F"] == "memo") {
								$f->textarea("f_vis_".$i,$visit_umum["vis_".$i] ,1, $visit_umum["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
								//Febri 24112012
								if($visit_umum["vis_".$i]=="Anamnesa"){
									$f->textarea("f_vis_12",$visit_umum["vis_12"] ,1, $visit_umum["vis_12W"],ucfirst($d2[2+$i]),$ext);
								}//Febri 24112012
						}elseif ($visit_umum["vis_".$i."F"] == "memo1") {
								$f->textarea1("<U>DIAGNOSA KERJA</U>","f_vis_".$i,$visit_umum["vis_".$i] ,1, $visit_umum["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
						
						}elseif ($visit_umum["vis_".$i."F"] == "edit") {
		        				$f->text("f_vis_".$i,$visit_umum["vis_".$i],$visit_umum["vis_".$i."W"],$visit_umum["vis_".$i."W"],ucfirst($d2[3+$i]),$ext);
		    			
						}elseif ($visit_umum["vis_".$i."F"] == "edit4") {
						//Febri 241120212
					    		$f->text_4("<U>PEMERIKSAAN FISIK</U>","f_vis_12",$visit_umum["vis_12"],10,20,$d2["vis_12"],"","f_vis_7",$visit_umum["vis_7"],10,20,$d2["vis_7"],"Kg","f_vis_3",$visit_umum["vis_3"],10,20,$d2["vis_3"],"mm Hg","f_vis_4",$visit_umum["vis_4"],10,20,$d2["vis_4"],"/Menit",$ext);
						    		
					    }elseif ($visit_umum["vis_".$i."F"] == "edit5") {//Febri 241120212
								$f->text_4("","f_vis_8",$visit_umum["vis_8"],10,20,$d2["vis_8"],"&deg;C","f_vis_5",$visit_umum["vis_5"],10,20,ucfirst($d2["vis_5"]),"","f_vis_9",$visit_umum["vis_9"],10,20,ucfirst($d2["vis_9"]),"","f_vis_11",$visit_umum["vis_11"],10,20,$d2["vis_11"],"",$ext);			    	
					    }//Febri 241120212
			 		$i++ ; 	
					}
