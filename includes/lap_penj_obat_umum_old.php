...
$r2 = pg_query($con,
          ...
          "sum(c.referensi::numeric) as tuslah ".
          "from apotik_umum a ".
...
$t->SQL = "select tanggal(a.tanggal_entry,0)||' '||to_char(a.waktu_entry,'hh:mi:ss') as tgl, ".
          ...
          "     (c.trans_type::text = 'OB2'::text) ".
          "     ".$addParam." ".
          "group by tgl, a.no_reg, a.nama, b.obat, a.banyaknya, d.harga, a.harga, c.referensi, a.jumlah";
...
$t->ColHeader = array("TANGGAL TRANSAKSI","NO.REG","NAMA","NAMA OBAT","QTY","HARGA POKOK","HARGA JUAL","TUSLAH","TOTAL (Rp.)");
...
if(!$GLOBALS['print']){
    $t->RowsPerPage = 300;
    $t->ColFooter[6] =  "Total";
	$t->ColFooter[7] =  number_format($d2->tuslah,2);
