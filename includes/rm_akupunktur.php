...
if ($_GET['act'] ==  "detail"){
		if (!$GLOBALS['print']){
			title_print("<img src='icon/medical-record-2.gif' align='absmiddle' > REKAM MEDIS KLINIK AKUPUNKTUR");
			echo "<DIV ALIGN=RIGHT OnClick='window.history.back()'>".icon("back","Kembali")."</a></DIV>";
		}else{
			title_print("<img src='icon/medical-record.gif' align='absmiddle' > REKAM MEDIS KLINIK AKUPUNKTUR");
		}
		    //echo "<br>";
			$sql = "select a.vis_1,a.vis_2,a.vis_3,a.vis_4,a.vis_5,a.vis_6,a.vis_7,a.vis_8,a.vis_9,a.vis_10, ".
					"a.vis_11,a.vis_12,a.vis_13,a.vis_14,a.vis_15,b.nama ". 
		    		"from c_visit a ".
		...
		while ($i<= $max) {		
			...
			if ($visit_akupunktur["vis_".$i."F"] == "memo") {
				...
				$f->text($visit_akupunktur["vis_".$i],$d[$i-1]);
			...
			if ($visit_akupunktur["vis_".$i."F"] == "edit4") {
				...
				$f->text4($visit_akupunktur["vis_5"],$d[$i-1],$visit_akupunktur["vis_6"],$d[$i],$visit_akupunktur["vis_7"],$d[$i+1],$visit_akupunktur["vis_8"],$d[$i+2],"");
				
			}
			if ($visit_akupunktur["vis_".$i."F"] == "edit5") {
			
				$f->text4($visit_akupunktur["vis_9"],$d[$i-1],$visit_akupunktur["vis_10"],$d[$i],$visit_akupunktur["vis_11"],$d[$i+1],$visit_akupunktur["vis_12"],$d[$i+2],"");			    	
			}	
			$i++ ; 	
		}	
