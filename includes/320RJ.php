...
//cek status pasien
$status_bayar = getFromTable("select statusbayar from rsv0012 where id='$_GET[rg]'");
$cek_tagih = getFromTable("select tagih from rsv0012 where id='$_GET[rg]'");
$cek_ri = getFromTable("select rawat_inap from rs00006 where id='$_GET[rg]'");
if($cek_ri == 'I'){
...
$f->hidden("p", $PID);
$f->hidden("tt", $_GET["tt"]);
$f->search("search", "Pencarian Nama / No.MR / No.Reg", 20, 20, $_GET["search"], "icon/ico_find.gif", "Cari", "OnChange='Form2.submit();'");
...
if ($reg > 0) {
    ...
    if($n > 0)
		...
		if($StatusCHK==0){
			$stat = "<font size='2' style='color:#0000FF'> Masih Dirawat </font>";
		}else{
			$stat = "<font size='2' style='color:#FF0000'>".$status_bayar."</font>";
		}
		$f->text("<font size='2'>Status </font>", "<blink>".$stat."</blink>");
        $f->execute();
        echo "  </td>";
	...
	$SQL12 = "SELECT a.id, b.tc from rs00008 a 
    		  ...
    		  where a.trans_type='OBM' and a.no_reg='$reg' order by a.id desc"; 
	...
	$rr = pg_query($con, "SELECT b.id, b.nama
			...
			WHERE a.no_reg = '$reg' order by a.id desc");
	
	$nr = pg_num_rows($rr);
	$dr = pg_fetch_object($rr);
	//---------------------------------
	$rr2 = pg_query($con, "SELECT b.id, b.nama, a.tanggal_reg
			FROM rs00006 a
	...
	$rr3 = pg_query($con, "SELECT b.id, b.nama
			FROM rs00008 a
			JOIN rs00017 b ON b.id = a.konsulen
			WHERE a.no_reg = '$reg' order by a.id desc");
	...
	if($n > 0){
						echo "<div align=right><input type=button value=' Edit ' OnClick=\"window.location = './index2.php?p=320RJ&tt=".$_GET['tt']."&rg=".$_GET['rg']."&sub=obat&nama_relasi=".$_GET['nama_relasi']."&jenis_transaksi=".$_GET['jenis_transaksi']."&nama_dokter=".$_GET['nama_dokter']."&nama_dokter_konsulen=".$_GET['nama_dokter_konsulen']."&iter_resep=".$_GET['iter_resep']."&id=".$d2->id."&act=edit';\">\n";
					}else {
						$ext = "";
	...
	if ($_GET['act'] == "edit") {
						...
						echo "<font color='#000000' size='2'> >>> Silahkan Pilih Nama Relasi !</font>";
						...
						$f->hidden("no_reg",$d2["no_reg"]);
					    	$f->hidden("tanggal_reg",$dr2->tanggal_reg);
						$f->hidden("sub","obat");

						$f->hidden("nama_relasi",$_GET['nama_relasi']);
						$f->hidden("jenis_transaksi",$_GET['jenis_transaksi']);
						$f->hidden("nama_dokter",$_GET['nama_dokter']);
						$f->hidden("nama_dokter_konsulen",$_GET['nama_dokter_konsulen']);
						$f->hidden("iter_resep",$_GET['iter_resep']);
	...
	$f = new Form("actions/320RJ.insert.php", "POST", "NAME=Form2 id='relasi_apotik'");
					$f->hidden("act","new2");
					$f->hidden("rg",$_GET['rg']);
					...
					$f->selectSQL("nama_relasi", "<I><B><font color='orange'><font size='2'>Nama Relasi</B>", "select '' as tc, '' as tdesc union select tc, tdesc from rs00001 where tt = 'RAP' and tc != '000' AND  (tc >= '022' OR tc = '004') ORDER BY tdesc ASC",$margin ,$ext,$_GET["nama_relasi"],$ext);
					...
					$f->selectSQL("nama_dokter_konsulen", "<I><B><font color='orange'><font size='2'>Nama Dokter Konsulen</B>", "select id, nama from rs00017 WHERE pangkat LIKE '%DOKTER%' Order By nama Asc ;", $nama_dokter_konsulen, $ext);
					$f->selectSQL("iter_resep", "<I><B><font color='orange'><font size='2'>Iter Resep</B>", "select tc_poli, tdesc from rs00001 where tt = 'ITR' order by tc ASC", $_GET["iter_resep"], $ext);
            ...
            <tbody>
                <tr>
                    <td>
                        ...
                        <input type="hidden" name="user_id" id="user_id" size="40" value="<?php echo $_SESSION["uid"];?>">
                        <input type="hidden" name="nama_usr" id="nama_usr" size="40" value="<?php echo $_SESSION["nama_usr"];?>">
                    </td>
...
?>
<script>
	/*menambahkan alert jika nama relasi tidak dipilih */
    ...
    $(function() {
        ...
        var data = [
            <?php 
	    //rs00016.harga_car_rsrj  AS harga_karyawan_rj,rs00016.harga_car_rsri AS harga_karyawan_ri,
	    //rs00016.harga_inhealth_drs AS harga_jpk_rj,rs00016.harga_inhealth_rs AS harga_jpk_ri
		...
		//RELASI TERBARU BRAY TRENDY 06/03/2014
		else if($margin=='022'){
				   $harga = (int)$row["harga_ranap"];
				   $jasa = (int)$row["tuslah_car_drs"];}

				   /** Rawat Jalan - Karyawan */
		else if($margin=='023'){
				   $harga = (int)$row["harga_khusus_i"];
				   $jasa = (int)$row["tuslah_car_rsrj"];}
		else if($margin=='024'){
				   /** Rawat Inap - Karyawan */
				   $harga = (int)$row["harga_khusus_ii"];
                    $jasa = (int)$row["tuslah_car_rsri"];}
		else if($margin=='025'){
				   /** Rawat Jalan - JPK */
				   $harga = (int)$row["harga_pamper"];
				   $jasa = (int)$row["tuslah_inhealth_drs"];
				  }
		else if($margin=='026'){
				   $harga = (int)$row["harga_kel_kry"];
				   $jasa = (int)$row["tuslah_umum_ri"];
				   }
		else if($margin=='027'){
				   $harga = (int)$row["harga_obat_rajal"];
				   $jasa = (int)$row["tuslah_jam_ri"];}
		else if($margin=='028'){
			           $harga = (int)$row["harga_obat_tjp"];
                                   $jasa = (int)$row["tuslah_jam_rj"];}
		else if($margin=='029'){
				   $harga = (int)$row["harga_bpjs"];
                                   $jasa = (int)$row["tuslah_kry_kelinti"];}
		else if($margin=='030'){
				   $harga = (int)$row["harga_aswas"];
				   $jasa = (int)$row["tuslah_kry_kelbesar"];}
		else if($margin=='031'){
				   $harga = (int)$row["harga_vaksin"];
				   $jasa = (int)$row["tuslah_kry_kelgratisri"];}
		else if($margin=='032'){
				   $harga = (int)$row["harga_alkes_bhp"];
				   $jasa = (int)$row["tuslah_kry_kelgratisri"];}
		else if($margin=='033'){
				   $harga = (int)$row["harga_nempil_rajal"];
				   $jasa = (int)$row["tuslah_kry_kelgratisri"];}
		else if($margin=='034'){
				   $harga = (int)$row["harga_langganan"];
				   $jasa = (int)$row["tuslah_kry_kelgratisri"];}
		else if($margin=='035'){
				   $harga = (int)$row["harga_klinik"];
				   $jasa = (int)$row["tuslah_kry_kelgratisri"];}
		else{
        ...
        $( "#obat_nama" ).autocomplete({
            ...
            select: function (event, ui) {
				...
				var obatKategori = ui.item.kategoriId;
                var obatSatuan = ui.item.satuan;
                var obatJasa = ui.item.jasa;
                var obatHarga = parseFloat(ui.item.harga);
                var obatStok = ui.item.stok;
                
				
				
				
                if(parseFloat(obatStok) < 0.001){
                    alert('stok kosong !');
                    return false;
                }
                ...
                $('#obat_id').val(obatId);
				$("#kategori_id").val(parseInt(obatKategori));
		...
		*/
		 $("#qty").keyup( function(){
                ...
                var obatHarga = parseFloat($('#harga').val());   
                var obatPenjamin = parseFloat($('#penjamin').val());  
                var is_return = parseFloat($('#is_return').val()); 
                var sisa_stok = parseFloat($("#stok").html());   				
                // jumlah = obatQty*obatHarga+obatJasa;
                if (isNaN($("#qty").val())){
				...
				if(obatQty > sisa_stok && is_return==0){
				    alert('Sisa Stok Tidak Mencukupi');
					$('#qty').val(0);
                    $('#qty').focus();
				}
                jumlah = (obatQty*obatHarga)+1000;
                selisih = jumlah-obatPenjamin;
                $('#jumlah').val(jumlah);
                $('#selisih').val(selisih);
				if(is_return==1){
					if(obatQty > sisa_stok)
					{
						alert('Quantity Return tidak boleh melebihi dari Quantity Beli');
						$('#qty').val(0);
						$('#qty').focus();
					}
				}
		...
		//start define embalance
		$("#is_racikan").change( function(){
			var obatKategori = parseFloat($('#kategori_id').val());
			var obatQty = parseFloat($('#qty').val());
			var obatJasa = parseFloat($('#jasa').val());
			var obatHarga = parseFloat($('#harga').val());   
			var obatPenjamin = parseFloat($('#penjamin').val());   
			var isRacikan = parseFloat($('#is_racikan').val());
			
			if(parseInt(isRacikan) == 0){
				jumlah = obatQty*obatHarga+obatJasa;
				selisih = jumlah-obatPenjamin;
				$('#jumlah').val(jumlah);
				$('#selisih').val(selisih);
            } else if (parseInt(isRacikan) == 1 && parseInt(obatKategori) == '028'){
				jumlah = ((obatQty*obatHarga+obatJasa)-1000)+1500;
				selisih = jumlah-obatPenjamin;
				$('#jumlah').val(jumlah);
				$('#selisih').val(selisih);
            } else if(parseInt(isRacikan) == 1 && parseInt(obatQty) >= 1 && parseInt(obatQty) <= 20){
				jumlah = ((obatQty*obatHarga+obatJasa)-1000)+3000;
				selisih = jumlah-obatPenjamin;
				$('#jumlah').val(jumlah);
				$('#selisih').val(selisih);
            } else if (parseInt(isRacikan) == 1 && parseInt(obatQty) >= 21 && parseInt(obatQty) <= 30){
				jumlah = ((obatQty*obatHarga+obatJasa)-1000)+3500;
				selisih = jumlah-obatPenjamin;
				$('#jumlah').val(jumlah);
				$('#selisih').val(selisih);
            } else if (parseInt(isRacikan) == 1 && parseInt(obatQty) >= 31 && parseInt(obatQty) <= 60){
				jumlah = ((obatQty*obatHarga+obatJasa)-1000)+4500;
				selisih = jumlah-obatPenjamin;
				$('#jumlah').val(jumlah);
				$('#selisih').val(selisih);
            } else if (parseInt(isRacikan) == 1 && parseInt(obatQty) >= 61){
				jumlah = ((obatQty*obatHarga+obatJasa)-1000)+5500;
				selisih = jumlah-obatPenjamin;
				$('#jumlah').val(jumlah);
				$('#selisih').val(selisih);
            } else if(parseInt(isRacikan) == 2){
				jumlah = obatQty*obatHarga+obatJasa;
				selisih = jumlah-obatPenjamin;
				$('#jumlah').val(jumlah);
				$('#selisih').val(selisih);
...
//        $('#penjamin').val(penjamin);
    ...
    function cetakkwitansi1(tag) {
       maxI = $('#max_i').val();
        ...
        sWin = window.open('includes/cetak.rincian_obat_apotek.php?rg=<?php echo $_GET['rg']?>&kas=<?php echo $_GET['rg']?>'+selectedToPrint, 'xWin','top=0,left=0,width=750,height=550,menubar=no,location=no,scrollbars=yes');
