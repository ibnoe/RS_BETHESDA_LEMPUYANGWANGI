...
subtitle_rs("FORMULIR RL1 - KUNJUNGAN RUMAH");
subtitle_rs($set_header[0]." ".$set_header[1]);
subtitle_rs("No. Kode RS : ".$set_kode_rs); 	  
		
	if(!$GLOBALS['print']){
		title_print("");
		title_excel("kunjungan_rumah");	
		edit_laporan("input_kunjungan_rumah");	
	}else {
		
	}
	
$SQL = "select * from rl100008 group by jenis_layanan, kunjungan, id order by id";
