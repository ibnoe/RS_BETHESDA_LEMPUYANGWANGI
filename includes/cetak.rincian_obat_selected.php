...
<HTML>
   ...
   $rt = pg_query($con,
    ...
    $rowsPemakaianObat      = pg_query($con, "SELECT DISTINCT item_id, SUM(qty) as qty, SUM(tagihan::integer) AS tagihan, SUM(referensi::integer) as referensi, SUM(dibayar_penjamin::integer) as dibayar_penjamin, trans_type, diskon  
							 FROM rs00008
                             WHERE trans_type = 'OB1' AND rs00008.no_reg = '".$_GET["rg"]."' GROUP BY item_id, trans_type, diskon   ");
	$rowsReturnObat      = pg_query($con, "SELECT DISTINCT item_id, SUM(qty_return) as qty, SUM(tagihan::integer) AS tagihan, SUM(referensi::integer) as referensi, SUM(dibayar_penjamin::integer) as dibayar_penjamin, trans_type, diskon  
							 FROM rs00008_return
                             WHERE trans_type = 'OB1' AND rs00008_return.no_reg = '".$_GET["rg"]."' GROUP BY item_id, trans_type, diskon   ");
    $rowsPemakaianRacikan   = pg_query($con, "SELECT DISTINCT item_id, SUM(qty) as qty, SUM(tagihan::integer) AS tagihan, SUM(referensi::integer) as referensi, SUM(dibayar_penjamin::integer) as dibayar_penjamin, trans_type, diskon  
							 FROM rs00008
                             WHERE trans_type = 'RCK' AND rs00008.no_reg = '".$_GET["rg"]."' GROUP BY item_id, trans_type, diskon  ");
	$rowsPemakaianBHP      = pg_query($con, "SELECT DISTINCT item_id, SUM(qty) as qty, SUM(tagihan::integer) AS tagihan, SUM(referensi::integer) as referensi, SUM(dibayar_penjamin::integer) as dibayar_penjamin, trans_type, diskon  
							 FROM rs00008
                             WHERE trans_type = 'BHP' AND rs00008.no_reg = '".$_GET["rg"]."' GROUP BY item_id, trans_type, diskon  ");
	$rowsAdministrasi      = pg_query($con, "SELECT id, tanggal_entry, waktu_entry, item_id, qty, tagihan, referensi, dibayar_penjamin  
                             FROM rs00008 
	...
	$rr1 = pg_query($con, "SELECT b.id, b.nama
			...
			WHERE a.no_reg = '".$_GET["rg"]."' and a.trans_type::text = 'OBM'::text order by a.id desc");
	$nr1 = pg_num_rows($rr1);
	...
	$rr4 = pg_query($con, "SELECT b.id, b.nama
			FROM rs00008 a
			JOIN rs00017 b ON b.id = a.konsulen
			WHERE a.no_reg = '".$_GET["rg"]."' and a.trans_type::text = 'OBM'::text order by a.id desc");
	$nr4 = pg_num_rows($rr4);
	$dr4 = pg_fetch_object($rr4);
			$nama_dokter_konsulen = $dr4->nama;
	...
	$inap = "select distinct ( SELECT ts_check_in FROM rs00010 x WHERE x.id = 
			(SELECT max(id) FROM rs00010 WHERE no_reg = a.no_reg AND awal::text = '1'))  AS check_in,
            a.ts_calc_stop as check_out
			from 
            rs00010 as a
    ...
    $rowInap = pg_fetch_array(pg_query($inap));
	$cekin = $rowInap['check_in']; 
	$cekot = $rowInap['check_out']; 
	//end header kwitansi
...
<table width="50%" border="0" cellpadding="0" cellspacing="0" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;">
    ...
    <tr>
        ...
        <td colspan="4"><? echo $dt->tanggal_reg; ?></td>
    </tr>
    <tr>
	...
	<?PHP
	if ($nama_dokter_konsulen!="" || $nama_dokter_konsulen=="-"){
	?>
	<tr>
        <td valign="top">Dokter Konsulen</td>
        <td colspan="5">: <? echo $nama_dokter_konsulen; ?></td>
    </tr>
	<?PHP
	}
	?>
	<tr>
        <td valign="top">Lama Rawatan</td>
        <td colspan="5">: <? echo date("d-M-Y H:i:s", strtotime($cekin)); ?> - <? echo date("d-M-Y H:i:s", strtotime($cekot)); ?></td>
    </tr>
    <tr>
        <td colspan="6">&nbsp;</td>
    </tr>
...
<?php
if(pg_num_rows($rowsAdministrasi) > 0){
    echo '<tr><td class="" colspan="6"><span style="font-weight: bold;">Administrasi</span></td></tr>';
        $iObat          = 0;
        $total          = 0;
        $totalPenjamin  = 0;
        $totalSelisih   = 0;
        ...
        while($row=pg_fetch_array($rowsAdministrasi)){
            ...
            $sqlObat = pg_query($con, "SELECT DISTINCT  tdesc  from
                                        rs00001 WHERE tt = 'ADA' AND tc = '". $row["item_id"]."'" );
            $obat = pg_fetch_array($sqlObat);
?>
	<tr>
	    <td valign="top" class="" align="left" height="15" ><?=$noUrut?>.</td>
		<td class="" align="left" height="15" ><?=$obat["tdesc"]?></td>
...
if(pg_num_rows($rowsPemakaianObat) > 0){
    echo '<tr><td class="" colspan="6"><span style="font-weight: bold;">Obat Resep</span></td></tr>';
        $iObat          = 0;
        while($rowObat=pg_fetch_array($rowsPemakaianObat)){
            $noUrut++;
            $iObat++;
            $total          = $total + $rowObat["tagihan"];
            $totalPenjamin  = $totalPenjamin + $rowObat["dibayar_penjamin"];
            $totalSelisih   = $totalSelisih + ($rowObat["tagihan"]-$rowObat["dibayar_penjamin"]);
            $totalDiskon   = $totalDiskon + $rowObat["diskon"];
            $sqlObat = pg_query($con, "SELECT DISTINCT rs00015.id, rs00015.obat, rs00001.tdesc AS satuan, rs00016.harga 
                                        ...
                                        WHERE rs00001.tt = 'SAT' AND rs00015.id = ". $rowObat["item_id"] );
            $obat = pg_fetch_array($sqlObat);
?>
	<tr>
	    <td valign="top" class="" align="left" height="15" ><?=$noUrut?>.</td>
		<td class="" align="left" height="15" ><?=$obat["obat"]?></td>
		<td class="" align="right" height="15" style="text-align: right;"><?=$rowObat["qty"]?> <? //=$obat["satuan"]?></td>
		<td class="" align="right" height="15" ><?=number_format($rowObat["tagihan"],'0','','.')?></td>
		<td class="" align="right" height="15" ><?=number_format($rowObat["dibayar_penjamin"],'0','','.')?></td>
		<td class="" align="right" height="15" ><?=number_format(($rowObat["tagihan"]-$rowObat["dibayar_penjamin"]),'0','','.')?>&nbsp;</td>
		<td class="" align="right" height="15" ><?=number_format($rowObat["diskon"],'0','','.')?>&nbsp;</td>
	</tr>
<?php
        }
}
...
if(pg_num_rows($rowsReturnObat) > 0){
    echo '<tr><td class="" colspan="6"><span style="font-weight: bold;">Obat Retur</span></td></tr>';
	$noUrut = 0;
        while($row=pg_fetch_array($rowsReturnObat)){
            $noUrut++;
            $iObat++;
            $total          = $total - $row["tagihan"];
            $totalPenjamin  = $totalPenjamin - $row["dibayar_penjamin"];
...
<table width="47%" border="0" cellpadding="0" cellspacing="0" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;">
    ...
    <tr>
        <td width="35%" colspan="2" align="center" class="TITLE_SIM3" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;"><?php echo $client_city.", ".$tgl_now."<br>Kepala Instalasi Farmasi"; ?></td>
        <td width="10%" colspan="4" align="center" class="TITLE_SIM3">&nbsp;</td>
        <td width="15%" colspan="2" align="center" class="TITLE_SIM3" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;"><?php echo $_SESSION["nama_usr"];?></td>
    </tr>
    <tr>    
        <td width="50%" colspan="6" align="left" class="TITLE_SIM3">&nbsp;</td>
        <td width="50%" colspan="6" align="left" class="TITLE_SIM3">&nbsp;</td>
    ...
    <tr>
        <td width="15%" colspan="2" align="center" class="TITLE_SIM3" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;">AGUNG SUPRIHATIN, S.Si, Apt.</td>
    ...
    <tr>
        <td width="50%" colspan="6" align="left" class="TITLE_SIM3" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;">&nbsp;</td>
    </tr>
    <tr>
        <td width="50%" colspan="6" align="left" class="TITLE_SIM3" style="font-family: Tahoma; font-size: 11px; letter-spacing: 2px;"><b><font size="2.5">Keterangan: <br /> Bukan Merupakan Kwitansi</font><b></td>
    </tr>
</table>
<!--
