...
title_print("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK BEDAH");
title_excel("p_bedah&list=".$_GET['list']."&rg=".$_GET['rg']."&poli=".$_GET['poli']."&mr=".$_GET['mr']."&sub=".$_GET['sub']."&act=".$_GET['act']."&polinya=".$_GET['polinya']."&tblstart=".$_GET['tblstart']."");

//title("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK BEDAH");
echo "</td></tr></table>";
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_bedah.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
	...
	$T->show(2);
        ...
        echo "<form name='Form9' action='actions/p_bedah.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=list VALUE='icd'>";
	...
	if ($rec1 > 0) {
      ...
      /*  if (isset($_SESSION["SELECT_LAYANAN"]) && $is_range) {
        ...
        echo "<form name='Form10' action='actions/p_bedah.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
		...
		*/
    	if ($_GET["act"] == "detail") {
			...
			$f->text($visit_bedah["vis_8"],$d[10]."&nbsp;"."/Menit");
			$f->text($visit_bedah["vis_9"],$d[11]);
			$f->text($visit_bedah["vis_10"],$d[12]."&nbsp;"."Kg" );
			$f->text($visit_bedah["vis_11"],$d[13]."&nbsp;"."&deg;C");
			...
			$f->title1("<U>DOKTER AHLI BEDAH</U>","LEFT");
		...
		/* - edited 110210 -
		- menghapus fungsi trim() dan mengganti type data entitas f.id menjadi character varying
		- mengganti type data a.id_dokter menjadi varying character
		*/
...
//    			include(detail_pengawasan_khusus_pasien_dewasa);
    	...
    	$f = new Form("actions/p_bedah.insert.php", "POST", "NAME=Form2");
					$f->hidden("act","new1");
...
echo "<table border=0 width='100%'><tr>";
  ...
  echo "<form name='Form9' action='actions/320RJBEDAH.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<input type=hidden name=rawatan value='".$rawatan."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
			...
			*/
			...
			while ($i<= $max) {		
				if 		($visit_bedah["vis_".$i."F"] == "memo") {	
						$f->textarea("f_vis_".$i,$visit_bedah["vis_".$i] ,1, $visit_bedah["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
				
				}elseif ($visit_bedah["vis_".$i."F"] == "memo1") {
						$f->textarea1("<U>LABORATORIUM DAN DIAGNOSA</U>","f_vis_".$i,$visit_bedah["vis_".$i] ,1, $visit_bedah["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
				
				}elseif ($visit_bedah["vis_".$i."F"] == "edit") {
        				$f->text("f_vis_".$i,$visit_bedah["vis_".$i],$visit_bedah["vis_".$i."W"],$visit_bedah["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
    						
				}elseif ($visit_bedah["vis_".$i."F"] == "edit4") {
					    		$f->text_4("<U>PEMERIKSAAN FISIK</U>","f_vis_10",$visit_bedah["vis_10"],10,20,$d2["vis_10"],"Kg","f_vis_6",$visit_bedah["vis_6"],10,20,$d2[2+$i+1],"mm Hg","f_vis_7",$visit_bedah["vis_7"],10,20,$d2[2+$i+2],"/Menit","f_vis_8",$visit_bedah["vis_8"],10,20,ucfirst($d2[2+$i+3]),"",$ext);
						    		
				}elseif ($visit_bedah["vis_".$i."F"] == "edit5") {
								$f->text_4("","f_vis_9",$visit_bedah["vis_9"],10,20,ucfirst($d2[2+$i]),"","f_vis_5",$visit_bedah["vis_5"],10,20,ucfirst($d2["vis_5"]),"","f_vis_11",$visit_bedah["vis_11"],10,20,$d2[2+$i+2],"&deg;C","f_vis_12",$visit_bedah["vis_12"],10,20,ucfirst($d2[2+$i+3]),"",$ext);			    	
				}
	 		$i++ ; 	
			}
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
    ...
    $f->hidden("list2",tab1);
	    ...
	    if (!$GLOBALS['print']){
	    	$f->search("search","Pencarian",20,20,$_GET["search"],"icon/ico_find.gif","Cari","OnChange='Form4.submit();'");
	    	    	//title_excel("p_bedah&tblstart=".$_GET['tblstart']);
