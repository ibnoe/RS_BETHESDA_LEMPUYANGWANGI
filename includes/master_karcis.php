...
if(strlen($_GET["e"]) > 0) {
    ...
    if($_GET["e"] == "new") {
		...
		$f->text("f_js","Jasa Sarana",15,15,"0");
		$f->text("f_jp","Jasa Pelayanan ",15,15,"0");
		...
		$f->text("f_js","Jasa Sarana ",15,15,$d2->js);
		$f->text("f_jp","Jasa Pelayanan ",15,15,$d2->jp);
...
//    echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = 
        "select a.code, b.tdesc, a.js,a.jp, a.harga, a.id as dummy ".
        "from master_karcis a ".
        "left join rs00001 b ON b.tt = 'JMK' and b.tc = a.jmk ".
        "where ".
        "(upper(code) LIKE '%".strtoupper($_GET["search"])."%' ".
        ") group by a.code, b.tdesc, a.js,a.jp, a.harga, a.id order by a.id ";
    $t->setlocale("id_ID");
    ...
    $t->ColFormatHtml[5] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#5#>'>".icon("edit","Edit")."</A>".
            "<A CLASS=TBL_HREF HREF='actions/master_karcis.delete.php?p=$PID&id=<#5#>'>".icon("delete","Hapus")."</A>";;
    $t->ColHeader = array("LAYANAN","KARCIS", "JASA SARANA","JASA PELAYANAN","HARGA","");
