	...
	if(!$GLOBALS['print']){
       ...
       include("rincian2.php"); 
    	...
    	if ($_GET["act"] == "detail") {
			...
			$f->text($visit_laboratorium["vis_4"],$d[6] );
			echo "</td><td valign=top>";
			$f->title1("<U>DIAGNOSA</U>","LEFT");	
			$f->text($visit_laboratorium["vis_5"],$d[7] );
			$f->text($visit_laboratorium["vis_6"],$d[8]);
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,TO_CHAR(A.TANGGAL_REG,'DD MON YYYY')AS TANGGAL,TO_CHAR(A.TANGGAL_REG,'HH:MM:SS') AS WAKTU,A.VIS_3,'DUMMY' ". 
					   "FROM C_VISIT A ".
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL PEMERIKSAAN","WAKTU KUNJUNGAN","laboratoriumS","DETAIL");
	    	...
	    	$n = pg_num_rows($r);		    	
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
					...
					if 		($visit_laboratorium["vis_".$i."F"] == "memo") {
							$f->textarea("f_vis_".$i,$visit_laboratorium["vis_".$i] ,1, $visit_laboratorium["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
				
					}elseif ($visit_laboratorium["vis_".$i."F"] == "memo1") {
							$f->textarea1("DIAGNOSA KERJA","f_vis_".$i,$visit_laboratorium["vis_".$i] ,1, $visit_laboratorium["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
				
					}elseif ($visit_laboratorium["vis_".$i."F"] == "edit") {
	        				$f->text("f_vis_".$i,$visit_laboratorium["vis_".$i],$visit_laboratorium["vis_".$i."W"],$visit_laboratorium["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
	    			
					}elseif ($visit_laboratorium["vis_".$i."F"] == "edit4") {
				    		$f->text_4("PEMERIKSAAN FISIK","f_vis_5",$visit_laboratorium["vis_5"],7,10,$d2[2+$i],"","f_vis_6",$visit_laboratorium["vis_6"],7,10,$d2[2+$i+1],"mm Hg","f_vis_7",$visit_laboratorium["vis_7"],7,10,$d2[2+$i+2],"/Menit","f_vis_8",$visit_laboratorium["vis_8"],7,10,$d2[2+$i+3],"",$ext);
				    		
				    }elseif ($visit_laboratorium["vis_".$i."F"] == "edit5") {
							$f->text_4("","f_vis_9",$visit_laboratorium["vis_9"],7,10,$d2[2+$i],"","f_vis_10",$visit_laboratorium["vis_10"],7,10,$d2[2+$i+1],"Kg","f_vis_11",$visit_laboratorium["vis_11"],7,10,$d2[2+$i+2],"Celcius","f_vis_12",$visit_laboratorium["vis_12"],7,10,$d2[2+$i+3],"",$ext);			    	
				    }
	 		$i++ ; 	
			}
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
