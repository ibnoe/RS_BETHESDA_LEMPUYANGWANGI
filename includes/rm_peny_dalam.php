...
if ($_GET['act'] ==  "detail"){
		if (!$GLOBALS['print']){
			title_print("<img src='icon/medical-record-2.gif' align='absmiddle' > REKAM MEDIS KLINIK PENYAKIT DALAM");
			echo "<DIV ALIGN=RIGHT OnClick='window.history.back()'>".icon("back","Kembali")."</a></DIV>";
		}else{
			title_print("<img src='icon/medical-record.gif' align='absmiddle' > REKAM MEDIS KLINIK PENYAKIT DALAM");
		}
		    //echo "<br>";
			$sql = "select a.*,b.nama 
		    		from c_visit a
	...
	if ($_GET["v"]){ // v from 430.php
    	subtitle2("KLINIK PENYAKIT DALAM","left");	
