...
subtitle_print("DATA KEADAAN MORBIDITAS PASIEN RAWAT INAP RUMAH SAKIT");
subtitle_print("Triwulan : ".$set_triwulan);
subtitle_rs("FORMULIR RL2a");
subtitle_rs($set_header[0]." ".$set_header[1]);
subtitle_rs("No. Kode RS : ".$set_kode_rs); 	  
		
	if(!$GLOBALS['print']){
	 	title_print("");
		title_excel("laporan_rl2a");
        edit_laporan("input_rl2a");
	}else {
	
			
	}
	
	$SQL = "select * from rl200002a order by no_dtd";
