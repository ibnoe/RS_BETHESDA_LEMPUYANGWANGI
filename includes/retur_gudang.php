...
if ($_GET["action"] == "new") {
title("<img src='icon/apotik-2.gif' align='absmiddle' >  RETURN OBAT (EDIT)");
echo "<br>";
echo "<form action=$SC>";
...
if (isset($depo)==$_GET["poli_tujuan"]) {
    ...
    if (is_array($_SESSION["ob4"]["obat"])) {
        ...
        echo "<form action='actions/retur_gudang.insert.php' method=POST name=Form10>";
        echo "<input type=hidden name=poli_tujuan value=".$_GET[poli_tujuan].">";
		echo "<input type=hidden name=poli_asal value=".$_GET[poli_asal].">";
...
echo "<table>";
echo "<tr>";
echo "<td>".title("<img src='icon/apotik-2.gif' align='absmiddle' >  RINCIAN RETURN OBAT ")."</td>";
echo "<td>".title_print("")."</td>";
echo "</tr>";
echo "</table>";
	
	$cek_status_ver=getFromTable("select status from retur_gudang_m where kode_transaksi=".$_GET["f"]." ");
	...
	$qty_asal = getFromTable(
               "select 'qty_'||poli_asal from retur_gudang_m ".
               "where kode_transaksi='".$_GET["f"]."' ");
	
	$qty_tujuan = getFromTable(
               "select 'qty_'||poli_tujuan from retur_gudang_m ".
               "where kode_transaksi='".$_GET["f"]."' ");
...
echo "<table>";
	...
	echo "</tr>";
	/*echo "<tr>";
		echo "<td bgcolor='B0C4DE'><b> TRANSFER DARI </td>";
...
echo "</table>";
	...
	if($cek_status_ver == 0){
   	$t->SQL = "select a.obat,b.batch_id,b.jumlah||' '||d.tdesc as jumlah, b.keterangan,nm_user,case when b.status='0' then 'Belum di Konfirm' else 'Sudah di Konfirm' end ,verifikator,b.oid as dummy 
	from retur_gudang_d b, rs00015 a, rs00016a c, rs00001 d 
	where b.kode_transaksi='".$_GET["f"]."' and a.id::text=b.item_id and c.obat_id::text=b.item_id and a.satuan_id=d.tc AND d.tt='SAT' ";
    $t->ColHeader = array("NAMA OBAT", "BATCH ID","QTY RETURN","KETERANGAN","USER ENTRY","STATUS","VERIFIKATOR","EDIT");
	$t->ColAlign = array("LEFT","LEFT","CENTER","CENTER","LEFT","LEFT","LEFT","LEFT","CENTER");
    $t->setlocale("id_ID");
    $t->ShowRowNumber = true;
    $t->RowsPerPage = 9999;
	$t->ColFormatHtml[7] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&action=edit&f=".$_GET[f]."&e=".$_GET[e]."&g=".$_GET[g]."&id_obt=<#7#>'>".icon("edit","Edit")."</A>
						&nbsp;<A CLASS=TBL_HREF HREF='$SC?p=$PID&action=verifikasi&f=".$_GET[f]."&e=".$_GET[e]."&g=".$_GET[g]."&id_obt=<#7#>'>".icon("ok","Verifikasi")."</A>";
	}else{
	$t->SQL = "select a.obat,b.batch_id,b.jumlah||' '||d.tdesc as jumlah, b.keterangan,
	nm_user,case when b.status='0' then 'Belum di Konfirm' else 'Sudah di Konfirm' end ,verifikator
	from retur_gudang_d b, rs00015 a, rs00016a c, rs00001 d 
	where b.kode_transaksi='".$_GET["f"]."' and a.id::text=b.item_id and c.obat_id::text=b.item_id and a.satuan_id=d.tc AND d.tt='SAT' ";
    $t->ColHeader = array("NAMA OBAT", "BATCH ID","QTY RETURN","KETERANGAN","USER ENTRY","STATUS","VERIFIKATOR");
	$t->ColAlign = array("LEFT","LEFT","CENTER","CENTER","LEFT","LEFT","LEFT","LEFT","CENTER");
    $t->setlocale("id_ID");
    $t->ShowRowNumber = true;
    $t->RowsPerPage = 9999;
...
$f = new Form("actions/retur_gudang.insert.php", "POST","NAME=Form1");
$f->PgConn = $con;
...
echo "<table>";
echo "<tr>";
echo "<td>".title("<img src='icon/apotik-2.gif' align='absmiddle' >  EDIT JUMLAH RETURN OBAT ")."</td>";
echo "<td>".title_print("")."</td>";
echo "</tr>";
echo "</table>";
...
$r2 = pg_query($con,"select b.item_id, a.obat,b.batch_id,b.jumlah as qty,b.jumlah_minta||' '||d.tdesc as jumlah, c.$qty_asal1||' '||d.tdesc as stok_asal,c.$qty_tujuan1||' '||d.tdesc as stok_tujuan, 
					b.keterangan,nm_user,d.oid as dummy 
					from retur_gudang_d b, rs00015 a, rs00016a c, rs00001 d 
					where b.kode_transaksi='".$_GET["f"]."' and a.id::text=b.item_id and c.obat_id::text=b.item_id and b.oid='".$_GET[id_obt]."'
					and a.satuan_id=d.tc AND d.tt='SAT' ");
...
$f->hidden("qty_asal1",$d2->stok_asal);
$f->hidden("qty_ruang",$d2->stok_tujuan); 
$f->text("minta","Jumlah ",20,20,$d2->jumlah,"readonly");
$f->text("qty","Jumlah Return",20,20,$d2->qty,"");
$f->submit("Simpan");
$f->execute();

}else {
title("<img src='icon/apotik-2.gif' align='absmiddle' >  RETURN OBAT");
echo "<br>"; 
    ...
    $t = new PgTable($con, "100%");
   	$t->SQL = "select kode_transaksi,tanggal(a.tanggal_trans,0),c.tdesc as poli_asal,case when a.status='0' then 'Belum di Konfirmasi' else 'Sudah di Konfirmasi' end, a.kode_transaksi 
	...
	if (!$GLOBALS['print']){
		...
		$t->DisableNavButton = true;
		$t->DisableScrollBar = true;
		$t->ColFormatHtml[4] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&action=view&f=<#4#>&e=<#3#>&g=<#2#>'>".icon("view","View")."</A>";
		}else{
