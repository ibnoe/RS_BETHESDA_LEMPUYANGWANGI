...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl100020c where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/retina.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Retina");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/retina.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Retina");
        $f->hidden("id","new");
        $f->text("id","No",12,12,"<OTOMATIS>","DISABLED");
    }
    $f->PgConn = $con;
    $f->text("f_kegiatan","Jenis Kegiatan",40,50,$d->kegiatan);	
    $f->text("f_anatromik","Kualitas Perbaikan Anatromik",30,30,$d->anatromik);
	$f->text("f_visus","Kualitas Perbaikan Visus",30,30,$d->visus);
	$f->text("f_kuantitas","Jumlah Kuantitas",30,30,$d->kuantitas);
	 $f->submit(" Simpan ");
    $f->execute();
} else {
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan Pembedahan Mata - Retina");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select kegiatan, anatromik, visus, kuantitas, id as href FROM rl100020c order by id";            
              
    $t->ColHeader = array("Jenis Kegiatan", "Kualitas Perbaikan Anatromik", "Kualitas Perbaikan Visus", "Jumlah Kuantitas","Edit");
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
    ...
    $t->ColFormatHtml[4] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#4#>'>".icon("edit","Edit")."</A>".
            ...
            "actions/retina.delete.php?p=$PID".
            "&e=<#4#>".
            "'>".icon("delete","Hapus")."</A>".
            "</nobr>"; 
    /*
    ...
    */
	...
	$data = getFromTable("select kegiatan from rl100020c where id='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/retina.delete.php' method='get'>";
