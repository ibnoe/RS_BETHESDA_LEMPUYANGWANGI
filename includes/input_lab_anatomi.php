...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl100011b where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/lab_anatomi.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Patologi Anatomi");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/lab_anatomi.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Patologi Anatomi");
        $f->hidden("id","new");
        $f->text("id","No",12,12,"<OTOMATIS>","DISABLED");
    }
    $f->PgConn = $con;
    $f->text("f_jenis","Jenis Pemeriksaan",40,50,$d->jenis);
    $f->text("f_sederhana","Sederhana",30,30,$d->sederhana);
	$f->text("f_sedang","Sedang",30,30,$d->sedang);
	$f->text("f_canggih","Canggih",30,30,$d->canggih);
	$f->submit(" Simpan ");
    $f->execute();
} else {
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan Patologi Anatomi");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select jenis, sederhana, sedang, canggih, id as href FROM rl100011b order by id";            
              
    $t->ColHeader = array("Jenis Pemeriksaan", "Sederhana","Sedang","Canggih", "Edit");
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
    ...
    $t->ColFormatHtml[4] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#4#>'>".icon("edit","Edit")."</A>".
            ...
            "actions/lab_anatomi.delete.php?p=$PID".
            "&e=<#4#>".
            "'>".icon("delete","Hapus")."</A>".
            "</nobr>"; 
    /*
    ...
    */
	...
	$data = getFromTable("select jenis from rl100011b where id='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/lab_anatomi.delete.php' method='get'>";
