...
//$reg = (int) $_GET["rg"];
//$r1 = pg_query($con,
//    "select 'GZ'||lpad(".$_GET["rg"].",8,'0') as nomor");
//$d1 = pg_fetch_object($r1);
//pg_free_result($r1);

title("Transaksi Instalasi Gizi");
/*
if ($reg > 0) {
...
//if ($reg > 0) {
    ...
    $r = pg_query($con, "select * from rsv0002 where id ="$d1->nomor);
    $d = pg_fetch_object($r);
    pg_free_result($r);
    // tambahan sfdn
/*
    $umur = getFromTable(
               "select umur from rsv0002 ".
               "where to_number(id,'9999999999') = $reg");
...
/*
    ...
    echo "<input name=b1 type=button value='Transaksi' onClick='window.location=\"$SC?p=$PID&rg=".$_GET["rg"]."\";'>&nbsp;";
...
*/
    ...
    if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/380.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        ...
        if (isset($_SESSION["SELECT_EMP"]) && is_array($_SESSION["pjm"][$_SESSION["tag"]])) {
            ...
            if ($cnt == 1 && $_SESSION["pjm"][$_SESSION["tag"]][0]["id"] == "---") {
                ...
                $_SESSION["pjm"][$_SESSION["tag"]][0]["name"] =
                    getFromTable("select nama from rsv0006 where nip = '".$_SESSION["SELECT_EMP"]."'");
            } else {
...
*/
        ...
        echo "<FORM ACTION='$SC' NAME=Form8>";
        echo "<INPUT TYPE=HIDDEN NAME=p VALUE='$PID'>";
        echo "<INPUT TYPE=HIDDEN NAME=rg VALUE='$reg'>";
        echo "<INPUT TYPE=HIDDEN NAME=httpHeader VALUE='1'>";
        $t = new BaseTable("100%");
        $t->printTableOpen();
    ...
    if ($_GET["sub"] != "byr") {
        echo "<form name='Form9' action='actions/380.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
...
/*
    ...
    $f = new Form($SC, "GET");
    $f->hidden("p", $PID);
    $f->text("rg","Masukkan Nomor Transaksi",10,10,$_GET["rg"]);
    $f->submit(" Submit ");
    $f->execute();
