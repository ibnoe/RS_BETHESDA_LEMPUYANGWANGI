...
if(isset($_GET["e"])) {
    if ($_GET["e"] != "new") {
        $r = pg_query($con, "select * from rl100006 where id = '".$_GET["e"]."'");
    ...
    if($n > 0) {
        $f = new Form("actions/kesehatan_jiwa.update.php", "POST");
        title("Edit Laporan");
        $f->subtitle("Update Kesehatan Jiwa");
        $f->hidden("id","$d->id");
        $f->text("id","NO",6,6,$d->id,"DISABLED");
    } else {
        $f = new Form("actions/kesehatan_jiwa.insert.php");
        title("Edit Laporan");
        $f->subtitle("Tambah Data Kesehatan Jiwa");
        $f->hidden("id","new");
    ...
    // search box
    title("<img src='icon/icon-view.png' align='absmiddle' >  Edit Laporan Kesehatan Jiwa");
lihat_laporan("kesehatan_jiwa");
...
//    echo "<TD><INPUT TYPE=TEXT NAME=search VALUE='".$_GET["search"]."'></TD>";
  //  echo "<TD><INPUT TYPE=SUBMIT VALUE=' Cari '></TD>";
    ...
    $t->SQL = "select jenis_layanan, kunjungan, id as href FROM rl100006 group by jenis_layanan, kunjungan, id order by id";            
 ...
 //   $t->ColAlign[5] = "CENTER";
    ...
    */
    ...
    $t->ColFormatHtml[2] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&e=<#2#>'>".icon("edit","Edit")."</A>".
            ...
            "actions/kesehatan_jiwa.delete.php?p=$PID".
            "&e=<#2#>".
            "'>".icon("delete","Hapus")."</A>".
            "</nobr>"; 
    /*
    ...
    */
	...
	$data = getFromTable("select jenis_layanan from rl100006 where id='".$_GET[e]."'");

    echo "<div align=center>";
    echo "<form action='actions/kesehatan_jiwa.delete.php' method='get'>";
