...
*/
if(!$GLOBALS['print']){
title("<img src='icon/akuntansi-bukubesar.png' align='absmiddle' >  LAPORAN BUKU BESAR GUDANG");
}else{
title("LAPORAN BUKU BESAR GUDANG");
}
title_print("");
echo "<br>";
...
if($ts_check_in2==date('Y-m-d')){
		$field="(SELECT i.gudang FROM rs00016a i where i.obat_id='$_GET[e]')";
	}else{
		$field="(SELECT i.gudang FROM daily_stock i where i.obat_id='$_GET[e]' and date(i.timestamp)=b.tanggal_entry )";
	}
    $rowsData = pg_query($con,"SELECT to_char(e.date_stock,'dd Mon YYYY')  as tanggal,b.trans_type || '/' || to_char(e.date_stock,'mmyy') || '-' || b.id  as no_bukti,
							   (SELECT h.gudang FROM daily_stock h where h.obat_id='$_GET[e]' and h.date_stock=b.tanggal_entry ) as stok_awal,case when b.qty_msk_gd is null then 0 else b.qty_msk_gd end as qty_terima,case when b.qty_keluar_gd is null then 0 else b.qty_keluar_gd end as qty_keluar,case when b.qty_ret_msk_gd is null then 0 else b.qty_ret_msk_gd end as qty_ret_msk,case when b.qty_ret_kel_gd is null then 0 else b.qty_ret_kel_gd end as qty_ret_kel,case when b.qty_adj_gd is null then 0 else b.qty_adj_gd end as qty_adj,$field as stok_akhir from rs00015 a ".
	//		  "     left join rs00001 c on a.kategori_id = c.tc and c.tt='SAT' ".
			  ...
			  "     join v_buku_besar b on e.obat_id =to_number(b.item_id, '999999999999') and b.id_depo='003' and e.date_stock=b.tanggal_entry ".
