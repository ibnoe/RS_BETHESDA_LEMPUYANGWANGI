...
title_print("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK THT");
title_excel("p_tht&list=".$_GET['list']."&rg=".$_GET['rg']."&poli=".$_GET['poli']."&mr=".$_GET['mr']."&sub=".$_GET['sub']."&act=".$_GET['act']."&polinya=".$_GET['polinya']."&tblstart=".$_GET['tblstart']."");

//title("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK THT");
echo "</td></tr></table>";

unset($_GET["layanan"]);

$reg = $_GET["rg"];
$reg2 =  $_GET["rg"];//indikasi reg untuk rincian.php
...
if ($_GET["sub"] == "byr") {
        ...
        echo "<br><br><br>";
        $f = new Form("actions/p_tht.insert.php");
        $f->hidden("rg",$_GET["rg"]);
        $f->hidden("mr",$_GET["mr"]);
	...
	$f->hidden("sub",$_GET["sub"]);
		...
		*/
		...
		//$f = new Form($SC, "GET");
		/* -- edited 150210 
		-- merubah fungsi substr ke to_char 
		*/
 ...
 include("rincianobat.php"); 
    	...
    	$T->show(0);
	    	...
	    	$n = pg_num_rows($r);		    	
				...
				if ($_GET['act'] == "edit"){
						...
						$f = new Form("actions/p_tht.insert.php", "POST", "NAME=Form2");
						$f->hidden("act","edit");
				...
				//$f->calendar("tanggal_reg","Tanggal Registrasi",15,15,$d2[1],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
					...
					while ($i<= $max) {
		    		if 		($visit_tht["vis_".$i."F"] == "memo") {
						//Febri 24112012
						if($i==2){
							$f->textarea("f_vis_25",$visit_tht["vis_25"]  ,1, $visit_tht["vis_25W"],ucfirst($d2[2+$i]),$ext);
						}//Febri 24112012
							$f->textarea("f_vis_".$i,$visit_tht["vis_".$i] ,1, $visit_tht["vis_".$i."W"],ucfirst($d2[2+$i]),$ext);
				
		    		}elseif ($visit_tht["vis_".$i."F"] == "edit") {
							$f->text_gambar("<U>PEMERIKSAAN FISIK</U>","PHARYNX","IMG BORDER=0 SRC='images/bg_tbh_01.gif'","f_vis_5",$visit_tht["vis_5"],48,50,ucfirst($d2["vis_5"]),$ext,"f_vis_6",$visit_tht["vis_6"],48,50,ucfirst($d2["vis_6"]),$ext,"f_vis_7",$visit_tht["vis_7"],48,50,ucfirst($d2["vis_7"]),$ext,"f_vis_8",$visit_tht["vis_8"],48,50,ucfirst($d2["vis_8"]),$ext,"f_vis_9",$visit_tht["vis_9"],48,50,ucfirst($d2["vis_9"]),$ext);
				
		    		}elseif ($visit_tht["vis_".$i."F"] == "edit2") {
							$f->text_gambar3("EPIPHARYNX","IMG BORDER=0 SRC='images/bg_tbh_03.gif'","f_vis_10",$visit_tht["vis_10"],48,50,ucfirst($d2["vis_10"]),$ext,"f_vis_11",$visit_tht["vis_11"],48,50,ucfirst($d2["vis_11"]),$ext,"f_vis_12",$visit_tht["vis_12"],48,50,ucfirst($d2["vis_12"]),$ext);
				
		    		}elseif ($visit_tht["vis_".$i."F"] == "edit3") {
							$f->text_gambar3("LARYNX","IMG BORDER=0 SRC='images/bg_tbh_02.gif'","f_vis_13",$visit_tht["vis_13"],48,50,ucfirst($d2["vis_13"]),$ext,"f_vis_14",$visit_tht["vis_14"],48,50,ucfirst($d2["vis_14"]),$ext,"f_vis_15",$visit_tht["vis_15"],48,50,ucfirst($d2["vis_15"]),$ext);
					}
			
	 		$i++ ; 	
			}
			$f->title1("<U>LABORATORIUM</U>","left");
			$f->text("f_vis_16","HB",63,50,ucfirst($d2["vis_16"]),$ext);
			$f->text("f_vis_17","Masa Pendarahan",63,50,ucfirst($d2["vis_17"]),$ext);
			$f->text("f_vis_18","Masa Pembekuan",63,30,ucfirst($d2["vis_18"]),$ext);
			$f->text("f_vis_19","Leocosit",63,30,ucfirst($d2["vis_19"]),$ext);
			$f->title1("<U>RENCANA PEMERIKSAAN / KONSUL</U>","left");
			$f->text("f_vis_20","Terapi / Obat-obatan",63,30,ucfirst($d2["vis_20"]),$ext);
			$f->text("f_vis_21","Rencana Pemeriksaan",63,30,ucfirst($d2["vis_21"]),$ext);
			$f->text("f_vis_22","Konsul",63,30,ucfirst($d2["vis_22"]),$ext);
			//$f->textinfo("f_vis_23","Dirawat / Tanggal",20,20,$d2["vis_23"]," Ex: 2007-02-05 (Tahun - Bulan - Tanggal)",$ext);
			$f->calendar("f_vis_23","Dirawat / Tanggal",15,15,$d2["vis_23"],"Form2","icon/calendar.gif","Pilih Tanggal",$ext);
			$f->textarea("f_vis_24","Diagnosa Kerja",1,48,ucfirst($d2["vis_24"]),$ext);
    ...
    $f->hidden("list2",tab1);
	    ...
	    if (!$GLOBALS['print']){
	    	...
	    	//title_excel("p_tht&tblstart=".$_GET['tblstart']);
