...
if(!$GLOBALS['print']){
title("<img src='icon/akuntansi-bukubesar.png' align='absmiddle' >  LAPORAN MUTASI SALDO ".strtoupper($nm_depo));

}else{
title("LAPORAN MUTASI SALDO ".strtoupper($nm_depo));
}
title_print("");
title_excel("lap_mutasi_saldo&mGDP=".$_GET["mGDP"]."&mOBT=".$_GET["mOBT"]."&tanggal1D=".$_GET["tanggal1D"]."&tanggal1M=".$_GET["tanggal1M"]."&tanggal1Y=".$_GET["tanggal1Y"]."&tanggal2D=".$_GET["tanggal2D"]."&tanggal2M=".$_GET["tanggal2M"]."&tanggal2Y=".$_GET["tanggal2Y"]."&mDETIL=S");
...
if ($_GET["mDETIL"] == 'S') {
    ...
    */
    ...
    //$t = new PgTable($con, "100%");
   /* $t->SQL = "SELECT a.item_id,a.nama_obat,c.tdesc as satuan,case when d.gudang>=0 then null else null end as saldo_awal,sum(a.qty_msk),sum(a.qty_keluar),sum(a.qty_ret_msk),sum(a.qty_ret_kel),sum(a.qty_adj),case when d.gudang>=0 then null else null end as saldo_akhir,a.hna,0*a.hna as jumlah_saldo from v_buku_besar2 a ".
	  ...
	  //  "a.trans_type='OB1' and ".
		...
		if($_GET["mGDP"]==''){
		$field2 = "(e.gudang+e.qty_ri+e.qty_igd+";
	 for ($i = 1; $i <= 43; $i++) {
        $field2 .= str_pad($i, 9, "e.qty_00", STR_PAD_LEFT) ;
		if($i!=43){
			$field2 .='+';
		}else{
			$field2 .=') ';
		...
		$field3 = "(f.gudang+f.qty_ri+f.qty_igd+";
	 for ($i = 1; $i <= 43; $i++) {
        $field3 .= str_pad($i, 9, "f.qty_00", STR_PAD_LEFT) ;
		if($i!=43){
			$field3 .='+';
		}else{
			$field3 .=') ';
		...
		$field4 = "(d.gudang+d.qty_ri+d.qty_igd+";
	 for ($i = 1; $i <= 43; $i++) {
        $field4 .= str_pad($i, 9, "d.qty_00", STR_PAD_LEFT) ;
		if($i!=43){
			$field4 .='+';
		}else{
			$field4 .=') ';
		...
		$field2 = "e.qty_".$_GET["mGDP"];
		$field3 = "f.qty_".$_GET["mGDP"];
		$field4 = "d.qty_".$_GET["mGDP"];
	...
	$t->SQL = "SELECT a.obat,c.tdesc,$field2 as stok_awal,case when b.qty_msk is null then 0 else b.qty_msk end as qty_terima,case when b.qty_keluar is null then 0 else b.qty_keluar end as qty_keluar,case when b.qty_ret_msk is null then 0 else b.qty_ret_msk end as qty_ret_msk,case when b.qty_ret_kel is null then 0 else b.qty_ret_kel end as qty_ret_kel,case when b.qty_adj is null then 0 else b.qty_adj end as qty_adj,$field as stok_akhir,CASE WHEN b.hna::text IS NULL THEN g.harga_beli
ELSE b.hna END as harga,
CASE WHEN b.hna::text IS NULL THEN ($field*g.harga_beli) 
ELSE b.hna*$field END as jumlah_saldo from rs00015 a ".
			  ...
			  "     LEFT OUTER JOIN (
						SELECT h.item_id , sum(h.qty_msk) as qty_msk,sum(h.qty_keluar) as qty_keluar,sum(h.qty_ret_msk) as qty_ret_msk,sum(h.qty_ret_kel) as qty_ret_kel,sum(h.qty_adj) as qty_adj,h.hna
							from v_buku_besar3 h where h.tanggal_entry between '$ts_check_in1' and '$ts_check_in2' and h.id_depo like '%".$_GET["mGDP"]."%'
							GROUP BY h.item_id,h.hna
					) as b on a.id =to_number(b.item_id, '999999999999') ".
			//		left outer join v_buku_besar3 b on a.id =to_number(b.item_id, '999999999999') and (b.tanggal_entry between '$ts_check_in1' and '$ts_check_in2') group by b.tanggal_entry ".
			  " where a.kategori_id like '%".$_GET["mOBT"]."%' and a.status='1' ".
              " and upper(a.obat) LIKE '%".strtoupper($_GET["search"])."%'".
			  "group by a.id,a.obat,c.tdesc,g.harga_beli,d.gudang,$field2,$field,b.qty_msk,b.qty_keluar,b.qty_ret_msk,b.qty_ret_kel,b.qty_adj,b.hna";
	$r2 = pg_query($con, "SELECT sum(b.qty_msk) as jum_terima,sum(b.qty_keluar) as jum_keluar,sum(b.qty_ret_msk) as jum_ret_msk,sum(b.qty_ret_kel) as jum_ret_kel,sum(b.qty_adj) as jum_adj from rs00015 a ".
			  "     left join rs00016a d on a.id = d.obat_id ". 
			  "     LEFT OUTER JOIN ( SELECT h.item_id ,sum(h.qty_msk) as qty_msk,sum(h.qty_keluar) as qty_keluar ,sum(h.qty_ret_msk) as qty_ret_msk,sum(h.qty_ret_kel) as qty_ret_kel,sum(h.qty_adj) as qty_adj							
					from v_buku_besar3 h where h.tanggal_entry between '$ts_check_in1' and '$ts_check_in2' and h.id_depo like '%".$_GET["mGDP"]."%'
					GROUP BY h.item_id
					) as b on a.id =to_number(b.item_id, '999999999999')".
		      " where a.kategori_id like '%".$_GET["mOBT"]."%' and a.status='1' ".
    ...
    $d2 = pg_fetch_object($r2);
    pg_free_result($r2);
	$r3 = pg_query($con, "SELECT sum($field2) as jum_stok_awal,sum($field) as jum_stok_akhir,sum($field*g.harga_beli) as jumlah_saldo from rs00015 a ".
			//  "     left join v_buku_besar3 b on a.id =to_number(b.item_id, '999999999999') and (b.tanggal_entry between '$ts_check_in1' and '$ts_check_in2') and b.id_depo like '%".$_GET["mGDP"]."%' ".
			  "     left join rs00016a d on a.id = d.obat_id ". 
