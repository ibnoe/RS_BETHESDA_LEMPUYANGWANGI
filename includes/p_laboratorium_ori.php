...
//lab begin
if ($_GET["httpHeader"] == "1" && $_GET['list'] != "layanan") {
...
title("<img src='icon/laboratorium-2.gif' align='absmiddle'>  LAYANAN LABORATORIUM");
...
$reg = $_GET["rg"];
	...
	$tab_disabled = array("pemeriksaan"=>true,"layanan"=>true, "catatan"=>true, "riwayat"=>true,"riwayat_klinik"=>true,"unit_rujukan"=>true,"konsultasi"=>true);
	if ($_GET["act"] == "del" ) {
	$tab_disabled = array("pemeriksaan"=>false, "layanan"=>false, "catatan"=>false, "riwayat"=>false,"riwayat_klinik"=>false,"unit_rujukan"=>false,"konsultasi"=>false);
	$tab_disabled[$_GET["sub"]] = true;
	$tab_disabled[$_POST["sub"]] = true;
	}
	
	$T = new TabBar();
	$T->addTab("$SC?p=$PID&list=pemeriksaan&rg=$rg&poli=".$_GET["mPOLI"]."&mr=$mr ", "Hasil Pemeriksaan"	, $tab_disabled["pemeriksaan"]);
	$T->addTab("$SC?p=$PID&list=layanan&rg=$rg&poli=".$_GET["mPOLI"]."&mr=$mr&sub=layanan", "layanan / Tindakan"	, $tab_disabled["layanan"]);
	$T->addTab("$SC?p=$PID&list=catatan&rg=$rg&poli=".$_GET["mPOLI"]."&mr=$mr&sub=catatan", "Catatan Laboratorium"	, $tab_disabled["catatan"]);
...
if ($reg > 0) {
    $r = pg_query($con,
       "select a.id,a.mr_no,a.nama,a.umur,a.tgl_lahir,a.tmp_lahir,a.tanggal_reg,a.status_akhir,a.diagnosa_sementara, ".
		"a.jenis_kelamin,a.pangkat_gol,a.nrp_nip,a.kesatuan ".
		"from rsv_pasien2 a  ".
		//"where a.id= '{$_GET['rg']}'";
        "WHERE A.ID = $reg");
	...
	$f->hidden("sub",$_GET["sub"]);
        ...
        $sql2 =	"SELECT A.*,B.NAMA FROM C_VISIT A 
	    	...
	    	$n = pg_num_rows($r);		    	
				...
				$f->textarea("f_vis_1",$visit_laboratorium["vis_1"],6,50,$d2["vis_1"],$ext);
        ...
        $t = new BaseTable("100%");
        $t->printTableOpen();
        $t->printTableHeader(Array("KODE", "NAMA LAYANAN", "JUMLAH", "SATUAN",
        ...
        if (is_array($_SESSION["layanan"])) {
            ...
            foreach($_SESSION["layanan"] as $k => $l) {
                ...
                $t->printRow(
                    Array($l["id"], $l["nama"].$tambahan, $l["jumlah"], $l["satuan"],
                    ...
                    Array("CENTER", "LEFT", "RIGHT","CENTER", "RIGHT", "RIGHT", "CENTER")
                );
                $total += $l["total"];
            }
        }
        ...
        if(isset($_SESSION["SELECT_EMP"])){
        	$r3 = pg_query($con,"select nama from rs00017 where id = '" . $_SESSION["SELECT_EMP"] . "'");
        ...
        $t->printRow(
			...
			$d->layanan . " - " . $d->klasifikasi_tarif, "<INPUT VALUE='".(isset($_GET["jumlah"]) ? $_GET["jumlah"] : "1").
            ...
            Array("CENTER", "LEFT", "CENTER","CENTER", "LEFT", "RIGHT", "CENTER")
        );
		// --- eof 27-12-2006 ---
        $t->printRow(
        ...
        echo "<form name='Form12' action='actions/999.1.load.php' method=POST>";
        echo "<input type=hidden name=nmpoli value='".$_GET["mPOLI"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<tr><td>";
		echo"<div class=form_subtitle1>PAKET LABORATORIUM</div>";
        // ------------ paket laboratorium
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        ...
        echo "</td></tr>";
        echo "</form><br>";
        echo "<form name='Form10' action='actions/p_laboratorium.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=poli VALUE='".$_GET["mPOLI"]."'>";
        echo "<input type=hidden name=rawatan value='".$rawatan."'>";
        echo "<input type=hidden name=list value='layanan'><br></br>";
        ...
        $f->hidden("mr",$_GET["mr"]);
    					...
    					$f->textAndButton3("dokter","Pemberi Tindakan",2,10,$_SESSION["DOKTER"]["id"],$ext,"nm2",25,70,$_SESSION["DOKTER"]["nama"],$ext,"...",$ext,"OnClick='selectPegawai();';");
			         
						$f->execute();
        echo "<div align=right><input type=button value='Simpan' onClick='document.Form10.submit()'>&nbsp;";
        echo "</form>";
       include("rincian2.php"); 
    	...
    	if ($_GET["act"] == "detail") {
			...
			//$f->title1("PEMERIKSAAN DAN DIAGNOSA","LEFT");
...
$SQLTR = "select b.jenis,b.parameter,a.vis_2,b.satuan,b.rentang_normal 
		  from c_catatan a
		  left join c_pemeriksaan_lab b on a.vis_1 = b.id 
		  where a.no_reg ='{$_GET['rg']}' and a.id_layanan = '{$_GET['mPOLI']}' order by tanggal";
...
$t->execute();
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,TO_CHAR(A.TANGGAL_REG,'DD MonthYYYY')AS TANGGAL,TO_CHAR(A.TANGGAL_REG,'HH24:MI')AS WAKTU,'DUMMY' ". 
					   "FROM C_VISIT A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
					   "LEFT JOIN C_PEMERIKSAAN_LAB C ON C.ID=A.VIS_1 ".
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,TO_CHAR(A.TANGGAL_REG,'DD MON YYYY')AS TANGGAL,SUBSTR(A.TANGGAL_REG,12,8) AS WAKTU,A.ID_POLI,C.TDESC,D.NAMA,'DUMMY' ". 
					   "FROM C_VISIT A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
			    ...
			    $t->ColHeader = array("NO REGISTRASI","TANGGAL PEMERIKSAAN","WAKTU KUNJUNGAN","KLINIK","","DOKTER PEMERIKSA","DETAIL");
			   	$t->ColAlign = array("center","center","center","left","left","left","center");
				$t->ColFormatHtml[6] = "<A CLASS=TBL_HREF HREF='$SC?p=$PID&list=riwayat_klinik&act=detail_klinik&mr=".$_GET["mr"]."&rg=<#0#>'>".icon("view","View")."</A>";	
				$t->execute();
	...
	$t = new BaseTable("100%");
    $t->printTableOpen();
    $t->printTableHeader(Array("KODE", "JENIS PEMERIKSAAN", "HASIL PEMERIKSAAN", "SATUAN",
                               "RENTANG NORMAL",""));
    if (is_array($_SESSION["ob4"]["obat"])) {
        $total = 0.00;
            ...
            if ($k == $_GET["edit"] && strlen($_GET["edit"]) > 0) {
                ...
                $t->printRow(
                    Array( str_pad($o["id"],6,"0",STR_PAD_LEFT),
                        $o["obat"],
                        "<input type=text size=20 maxlength=20 name=edithasil value='".$o["hasil"]."' style='text-align:left'>",
                        $o["satuan"],
                        
                        $o["rentang_normal"],
                        "<input type=submit value='Simpan'>".
                        " &nbsp; " .
                        "<input type=button value='Batal' onClick='window.location=\"$SC?p=$PID&rg=$rg&mr=$mr\"'>" ),
                    Array( "CENTER",
                        "LEFT",
                        "CENTER",
                        "CENTER",
                ...
                $t->printRow(
                    Array( str_pad($o["id"],6,"0",STR_PAD_LEFT),
                        ...
                        $o["rentang_normal"],
                        "<a href='$SC?p=$PID&rg=$rg&mr=$mr&httpHeader=1&del-lab=$k'>".icon("del-left","Hapus")."</a>".
                        " &nbsp; " .
                        "<a href='$SC?p=$PID&rg=$rg&mr=$mr&edit=$k'>".icon("edit","Edit")."</a>" ),
                    Array( "CENTER",
                        "LEFT",
                        "CENTER",
                        "CENTER",
    ...
    if (strlen($_GET["edit"]) == 0) {
        ...
        $t->printRow(
            Array( 
            	"<input type=text size=5 maxlength=10 name=ob4_id style='text-align:center' value=$d1->id>"."&nbsp;<a href='javascript:selectLaboratorium()'>".icon("view")."</a>",
                $d1->parameter,
                "<input type=text size=20 maxlength=20 name=ob4_hasil value='' style='text-align:left'>",
                $d1->satuan,
                $d1->rentang_normal,
    ...
    if (is_array($_SESSION["ob4"]["obat"])) {
        ...
        echo "<form action='actions/p_laboratorium.insert.php' method=POST name=Form10>";
        echo "<INPUT TYPE=HIDDEN NAME=rg VALUE='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=poli VALUE='".$_GET["mPOLI"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=list VALUE='pemeriksaan'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
    ...
    echo "function selectLaboratorium() {\n";
    echo "    sWin = window.open('popup/laboratorium.php', 'xWin', 'width=550,height=400,menubar=no,scrollbars=yes');\n";
    echo "    sWin.focus();\n";
    echo "}\n";
    echo "</script>\n";
    }
    ...
    $f->hidden("list2",tab1);
		...
		$f = new Form($SC, "GET","NAME=Formtab1");
	    $f->hidden("p", $PID);
	    $f->hidden("list2","tab1");
	    if (!$GLOBALS['print']){
	    	$f->search("search","Pencarian",20,20,$_GET["search"],"icon/ico_find.gif","Cari","OnChange='Formtab1.submit();'");
		}else { 
   ...
   // $n = pg_num_rows($rstr);		    	
    ...
    $f->hidden("poli",$_GET["mPOLI"]);
   		...
   		echo "<div align='right' valign='middle'>";	
		$f = new Form($SC, "GET","NAME=Formtab2");
	    $f->hidden("p", $PID);
	    $f->hidden("list2","tab2");
	    if (!$GLOBALS['print']){
	    	$f->search("search","Pencarian",20,20,$_GET["search"],"icon/ico_find.gif","Cari","OnChange='Formtab2.submit();'");
		}else { 
		   	$f->search("search","Pencarian",20,20,$_GET["search"],"icon/ico_find.gif","Cari","disabled");
		}
	    $f->execute();
