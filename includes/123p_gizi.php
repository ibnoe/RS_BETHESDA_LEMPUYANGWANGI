...
$mr = isset($_GET["mr"])? $_GET["mr"] : $_POST["mr"];
    ...
    if ($d->id) {
        if (($is_range && isset($_GET["harga"])) || (!$is_range)) {
            ...
            header("Location: $SC?p=" . $_GET["p"] . "&listooooo=layanan&rg=" . $_GET["rg"]."&mr=" . $_GET["mr"]. "&jumlah=" . $_GET["jumlah"]);
            exit;
        }
    } else {
        header("Location: $SC?p=" . $_GET["p"] . "&listooooo=layanan&rg=" . $_GET["rg"]."&mr=" . $_GET["mr"]."&sub=layanan");
        exit;
    }
}
echo "<table border=0 width='100%'><tr><td>";
title("<img src='icon/rawat-jalan-2.gif' align='absmiddle' >  POLIKLINIK GIZI");
echo "</td></tr></table>";
...
if ($_GET["sub"] == "byr") {
	...
	$f->hidden("sub",$_GET["sub"]);
        ...
        echo "<form name='Form9' action='actions/p_umum.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=list VALUE='icd'>";
        ...
        echo "<form name='Form10' action='actions/p_umum.insert.php' method=POST>";
        echo "<input type=hidden name=rg value='".$_GET["rg"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=sub VALUE='".$_GET["sub"]."'>";
        echo "<INPUT TYPE=HIDDEN NAME=mr VALUE='".$_GET["mr"]."'>";
        echo "<input type=hidden name=rawatan value='".$rawatan."'>";
       ...
       include("rincian.php"); 
    	...
    	if ($_GET["act"] == "detail") {
			...
			$f->text($visit_umum["vis_5"],$d[7]."&nbsp;/Menit" );    
			$f->text($visit_umum["vis_6"],$d[8]);
			$f->execute();
			echo "</td><td valign=top>";
			$f = new ReadOnlyForm();
			$f->text($visit_umum["vis_7"],$d[9]."&nbsp;Kg" );
			$f->text($visit_umum["vis_8"],$d[10]."&deg;C");
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,TO_CHAR(A.TANGGAL_REG,'DD MON YYYY')AS TANGGAL,SUBSTR(A.TANGGAL_REG,12,8) AS WAKTU,A.VIS_1,'DUMMY' ". 
					   "FROM C_VISIT A ".
		...
		//$f = new Form($SC, "GET");
				$sql = "SELECT A.NO_REG,TO_CHAR(A.TANGGAL_REG,'DD MON YYYY')AS TANGGAL,SUBSTR(A.TANGGAL_REG,12,8) AS WAKTU,C.TDESC,D.NAMA,A.ID_POLI,'DUMMY' ". 
					   "FROM C_VISIT A ".
					   "LEFT JOIN RS00006 B ON A.NO_REG=B.ID ".
	...
	// --- eof 27-12-2006 ---
    	...
    	$T->show(0);
			...
			$f->textarea("f_vis_1",$visit_umum["vis_1"] ,1, $visit_umum["vis_1"."W"],ucfirst($d2["vis_1"]),$ext);
			$f->title1("<U>PEMERIKSAAN FISIK</U>");
			$f->text_4("","f_vis_2",$visit_umum["vis_2"],10,20,ucfirst($d2["vis_2"]),"","f_vis_3",$visit_umum["vis_3"],10,20,$d2["vis_3"],"mm Hg","f_vis_4",$visit_umum["vis_4"],10,20,$d2["vis_4"],"/Menit","f_vis_5",$visit_umum["vis_5"],10,20,ucfirst($d2["vis_5"]),"",$ext);
			$f->text_4("","f_vis_6",$visit_umum["vis_6"],10,20,ucfirst($d2["vis_6"]),"","f_vis_7",$visit_umum["vis_7"],10,20,$d2["vis_7"],"Kg","f_vis_8",$visit_umum["vis_8"],10,20,$d2["vis_8"],"&deg;C","f_vis_9",$visit_umum["vis_9"],10,20,ucfirst($d2["vis_9"]),"",$ext);
			$f->textarea("f_vis_10",$visit_umum["vis_10"] ,1, $visit_umum["vis_10"."W"],ucfirst($d2["vis_10"]),$ext);
			$f->title1("<U>DIAGNOSA</U>");
			$f->textarea("f_vis_11",$visit_umum["vis_11"] ,1, $visit_umum["vis_11"."W"],ucfirst($d2["vis_11"]),$ext);
			$f->submitAndCancel("Simpan",$ext,"Batal","window.history.back()",$ext);
			$f->execute();
			echo"</div>";
