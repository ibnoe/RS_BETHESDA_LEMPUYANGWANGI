...
if (strlen($d->no_reg) > 0 && strlen($_SESSION["SELECT_BANGSAL"]) > 0) {
    $f = new Form("actions/370.insert.php", "POST");
...
*/
	//hery 09072007---------	
		...
		echo "<br>";
	$SQLSTR =
	        "select a.mr_no, a.id, TO_CHAR(a.tanggal_reg,'dd-mm-yyyy') as tanggal_reg , d.nama, ".
	        ...
	        "left join rs00001 c on a.poli = c.tc and c.tt='LYN' ".
...
//echo $SQLSTR.$SQLWHERE ;
    ...
    $t->ColFormatHtml[1] = "<a class=SUB_MENU1 href='$SC?p=$PID&list=daftar&sub=2&rg=<#1#>'><#1#></a>";
