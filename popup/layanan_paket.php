...
if (isset($_GET["e"])) {
    $_SESSION["SELECT_LAYANAN2"] = $_GET["e"];
    ?>
...
$f = new Form("layanan_paket.php", "GET", "NAME=Form1");
$f->PgConn = $con;
...
// $f->selectSQL("L1", "Grup Layanan",
    ...
    // "from rs00034 ".
    // "where klinik_id=".$_SESSION[id_klinik]." And substr(hierarchy,4,12) = '000000000000' ".
    // "and is_group = 'Y' ".
...
// if (getFromTable(
        // "select hierarchy, layanan ".
        // "from rs00034 ".
        // "where klinik_id=".$_SESSION[id_klinik]." And substr(hierarchy,7,9) = '000000000' ".
    ...
    // if (getFromTable(
            // "select hierarchy, layanan ".
            // "from rs00034 ".
            // "where klinik_id=".$_SESSION[id_klinik]." And substr(hierarchy,10,6) = '000000' ".
        ...
        // if (getFromTable(
                // "select hierarchy, layanan ".
                // "from rs00034 ".
                // "where klinik_id=".$_SESSION[id_klinik]." And substr(hierarchy,13,3) = '000' ".
...
// $f->execute();
 //}
    ...
    echo "<DIV ALIGN=RIGHT><TABLE BORDER=0><FORM ACTION='layanan_paket.php'><TR>";
...
/* $SQL1 = "select a.id, a.layanan, c.tdesc as klasifikasi_tarif, e.tdesc as golongan_tindakan, d.tdesc as jasa, b.tdesc as satuan ".
        "from rs00034 as a ".
	...
	"left join rs00001 as d on a.sumber_pendapatan_id = d.tc and d.tt = 'SBP' ".
        ...
        "where a.klinik_id=".$_SESSION[id_klinik]." And substr(a.hierarchy,1,".($level*3).") = '".substr($_SESSION["LAYANAN_L$level"],0,($level*3))."' ".
...
$t->RowsPerPage = 10;
$t->ColFormatHtml[0] =
    "<A HREF='layanan_paket.php?e=<#0#>'><IMG BORDER=0 SRC='../images/icon-ok.png'></A>";
$t->ColHeader = Array("&nbsp;", "LAYANAN", "KLASIFIKASI TARIF", "GOL. TINDAKAN","JENIS JASA","SATUAN");
//$t->ShowSQL = true;
$t->execute(); */
...
$SQL = "select id, description, 'KALI' as satuan, to_char(harga_paket,'999,999,999.99') ".
        "from rs99996 ".
        "where upper(description) LIKE '%".strtoupper($_GET["search"])."%' ";
...
$t->ColFormatHtml[0] =
    "<A HREF='layanan_paket.php?e=<#0#>'><IMG BORDER=0 SRC='../images/icon-ok.png'></A>";
$t->ColHeader = Array("&nbsp;", "LAYANAN", "SATUAN", "HARGA");
$t->execute();
//}
?>
</TD></TR></TABLE>
