...
session_start();
if (isset($_GET["e"])) {
	if((!empty($_GET['tag']))&&((int)$_GET['tag'])){
	unset($_SESSION["SELECT_EMP".$_GET['tag']]);
    $_SESSION["SELECT_EMP".$_GET['tag']] = $_GET["e"];
		}
	else{
	unset($_SESSION["SELECT_EMP"]);
    $_SESSION["SELECT_EMP"] = $_GET["e"];
}
    ?>
...
title("Pilih SMF");
echo "<br>";
$f = new Form("pegawai.php", "GET", "NAME=Form1");
$f->PgConn = $con;
$f->hidden("tag", $_GET['tag']);
...
$f->selectSQL("mJMF","Jabatan Medis Fungsional",
    ...
    "from rs00018 ".
    "where unit_medis_fungsional_id = '".$_SESSION["mUMF"]."' order by tdesc ASC"
    ,$_GET["mJMF"],
    "OnChange = 'Form1.submit();'");
$f->execute();
...
if ($is_selected) {
    ...
    $t->SQL =   
			...
			"where a.status NOT IN ('klr') and b.unit_medis_fungsional_id = '".$_SESSION["mUMF"]."'  and ".
    ...
    $t->ColFormatHtml[0] =
        "<A HREF='pegawai.php?e=<#0#>&tag=".$_GET['tag']."'><IMG BORDER=0 SRC='../images/icon-ok.png'></A>";
