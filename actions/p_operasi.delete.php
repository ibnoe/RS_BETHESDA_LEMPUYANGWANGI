...
$SQL2 = "insert into history_user " .
            ...
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Pelayanan Operasi','Pelayanan -> Pelayanan Operasi','Menghapus Pelayanan Paket No.MR $mr No.REG $rg Total $jml','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
pg_query($con, $SQL2);
//======================
...
$SQL2 = "insert into history_user " .
            "(id_history, tanggal_entry,waktu_entry,trans_form,item_id,keterangan,user_id,nama_user) ".
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Pelayanan Operasi','Pelayanan -> Pelayanan Operasi','Menghapus Pelayanan Non Paket No.MR $mr No.REG $rg Total $jml','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
    pg_query($con, $SQL2);
//======================
...
$cek=getFromTable("select user_id from c_visit_operasi where oid = ".$_GET["oid"]. "");
if ($cek == ''){
$SQL = "delete from c_visit_operasi where ".
    	   "oid = ".$_GET["oid"];
	}else{
	$SQL = "update c_visit_operasi set  id_konsul='' where ".
    	   "oid = ".$_GET["oid"];
	}
}
...
header("Location: ../index2.php?p=$PID&list=konsultasi&rg=".$_GET["rg"]."&poli=209&mr=".$_GET["mr"]."$add");
}else{
header("Location: ../index2.php?p=$PID&list=layanan&rg=".$_GET["rg"]."&poli=209&mr=".$_GET["mr"]."$add");
}
exit;
