...
if ($_GET[grp] == "N") {
   ...
   $SQL = "delete from c_pemeriksaan_lab where ".
            "id = '".$_GET["e"]."'";
   } else {
   ...
   $q = pg_query("select hierarchy from c_pemeriksaan_lab where id = ".$_GET[e]);
   $qr = pg_fetch_array($q);
   $hierar = $qr[0];
 ...
 */
 ...
 //  $SQL = "delete from rs00034 where hierarchy LIKE '$level%'";
   $SQL = "delete from c_pemeriksaan_lab where hierarchy LIKE '$level%'";
