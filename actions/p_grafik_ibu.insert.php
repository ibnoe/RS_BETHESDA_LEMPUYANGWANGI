...
//----tambahan for update.... hery------------------------
	...
	}elseif ($_POST['act'] == "edit1"){
		$SQL2 = "select * from c_catatan where no_reg='$noreg' ".
					"and id_ri='{$_POST["f_id_ri"]}' and tanggal='{$_POST["f_tanggal"]}' ";
			$r2 = pg_query($con,$SQL2);
			//echo $SQL2;		
			if ($d2 = pg_fetch_object($r2)){		
				$qb = New UpdateQuery();
				$qb->HttpAction = "POST";
				$qb->TableName = "c_catatan";
				$qb->VarPrefix = "f_";
				$qb->addPrimaryKey("no_reg", "'$noreg'");
				$qb->addPrimaryKey("id_ri", "'" . $_POST["f_id_ri"] . "'");
				$qb->addPrimaryKey("tanggal", "'" . $_POST["f_tanggal"] . "'");
				$SQL = $qb->build();
