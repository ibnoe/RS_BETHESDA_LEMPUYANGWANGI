...
// Agung SUnandar 15:09 07/06/2012 membetulkan check box
...
$SQL2 = "insert into history_user " .
            ...
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Poli Paru','Pelayanan -> Poliklinik Paru','Menambah Pelayanan NonPaket No.MR $mr No.REG $rg Total ".$v["total"]."','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
pg_query($con, $SQL2);
//======================
...
$SQL2 = "insert into history_user " .
            "(id_history, tanggal_entry,waktu_entry,trans_form,item_id,keterangan,user_id,nama_user) ".
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Poli Paru','Pelayanan -> Poliklinik Paru','Menambah Pelayanan Paket No.MR $mr No.REG $rg Total ".$v["total"]."','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
pg_query($con, $SQL2);
//======================
    }
	// Insert BHP
...
$SQL2 = "insert into history_user " .
            "(id_history, tanggal_entry,waktu_entry,trans_form,item_id,keterangan,user_id,nama_user) ".
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Poli Paru','Pelayanan -> Poliklinik Paru','Menambah BHP No.MR $mr No.REG $rg Total ".$v["total"]."','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
pg_query($con, $SQL2);
//======================
    }  
	...
	if ($_POST['act'] == "edit") { 
		...
		//if ($_POST['row'] > 0)	{
			...
			if ($d2 = pg_fetch_object($r2)){		
				...
				pg_query($con, $SQL);
				pg_query("update c_visit set vis_21='{$_POST["check1"]}' where no_reg ='$noreg'");
...
if ($tr->execute()) {
    ...
    unset($_SESSION["icd"]);
    if($_POST['sub'] != "retur"){  
    header("Location: ../index2.php?p=$PID&list={$_POST["list"]}&rg=".$_POST[rg]."&poli=113&mr=".$_GET["mr"]."$add&sub=".$_POST[sub]);
    exit;
    }else{
    	header("Location: ../index2.php?p=$PID&list={$_POST["list"]}&rg=".$_POST[rg]."&poli=113&mr=".$_GET["mr"]."$add&sub=".$_POST[sub]);
    exit;
