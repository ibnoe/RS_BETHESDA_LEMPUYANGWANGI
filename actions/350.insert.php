...
$jam_po = $_SESSION["ob4"]["jam-pengadaan"];
$id_supp=$_SESSION["ob4"]["supplier1"]["id"];
$jenis=$_SESSION["ob4"]["jenis"];
$regAkun=reg_id();
...
if (is_array($_SESSION["ob4"]["obat"])) {
	...
	foreach ($_SESSION["ob4"]["obat"] as $v){
		...
		$tr->addSQL("insert into c_po (po_id,supp_id,po_tanggal,po_jam,reg_akun,po_status,po_personal,ppn,tanggal_entry,user_entry,id)".
				"values ('".$no_po."','".$id_supp."','".$tgl_po."','".$jam_po."','".$regAkun."',0,'".$pen_jwb."','".$ppn."',CURRENT_DATE,'$_SESSION[uid]','".$jenis."')");
...
if ($tr->execute()) {
	...
	header("Location: ../index2.php?p=print_po&po_id=$no_po&jenis=$jenis&act=pengadaan");
	exit;
    }else{
