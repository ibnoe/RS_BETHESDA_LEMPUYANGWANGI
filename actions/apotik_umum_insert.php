...
if($_POST["del"] == 'true' ){
     ...
     $qty_asal = getFromTable("select banyaknya::int from apotik_umum where id=".$_POST['id']);
	
	$id_obt = getFromTable("select obat_id from apotik_umum where id='".$_POST['id']."'");
	
	pg_query($con, "UPDATE rs00016a SET qty_ri = qty_ri+$qty_asal WHERE obat_id = '".$id_obt."'");
	
	// delete setelah update stok 
    ...
    pg_query($con, "DELETE FROM buku_besar WHERE kode_transaksi = '" . $_POST["id"]."' and trans_form='apotik_umum'");
...
if($_POST["id"] != '' ){
	...
	pg_query($con, "UPDATE rs00008 SET 
                ...
                ");
	// ---- update juga ke buku besar
		
		pg_query($con,"UPDATE buku_besar SET item_id = '" . $obatId . "', 
                        qty = " . $qty . " 
                        WHERE kode_transaksi = '" . $_POST["id"]."' and trans_form = 'apotik_umum'  "); 
	//---
}
...
if($_POST['no_reg'] != '' && $_POST["id"] == ''){
    ...
    */
	...
	pg_query($con,"INSERT INTO buku_besar (tanggal_entry,waktu_entry,item_id,trans_type,id_depo,qty,kode_transaksi,trans_form,ket,harga) 
									values (CURRENT_DATE,CURRENT_TIME,'$obatId','OBK','020',$qty,'$idApotikUmum','apotik_umum','UMUM / $nama',$harga)");
