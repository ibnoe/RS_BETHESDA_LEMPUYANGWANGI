...
if ($tr->execute()) {
    ...
    unset($_SESSION["icd9"]);
    if($_POST['sub'] != "retur"){  
    header("Location: ../index2.php?p=$PID&list={$_POST["list"]}&rg1=".$_POST[rg]."&rg=".$_POST[rg]."&ri=$ri&mr=".$_GET["mr"]."&sub=".$_POST[sub]);
    exit;
    }else{
    	header("Location: ../index2.php?p=$PID&list={$_POST["list"]}&rg=".$_POST[rg]."&rg1=".$_POST[rg]."&ri=$ri&mr=".$_GET["mr"]."&sub=".$_POST[sub]);
    exit;
    }
} else {
     ?>
