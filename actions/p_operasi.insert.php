   ...
   pg_query("INSERT INTO rs00005 VALUES( currval('kasir_seq'), '".$_POST["rg"]."', ".
    ...
    foreach ($_SESSION["layanan"] as $v) {
        ...
        $tr->addSQL(
                ...
                "qty,           harga,       tagihan,    pembayaran, no_kwitansi,user_id, margin_pembayaran ".
            	") values (".
                "nextval('rs00008_seq'), 'LTM', '$PID', currval('rs00008_seq_group'), CURRENT_DATE, " .
                "CURRENT_DATE, CURRENT_TIME, '".$_POST["rg"]."', '".$v["id"]."', '', " .
                $v["jumlah"].",".$v["harga"].",".$total_tagihan.", 0, $dokter,'".$_SESSION["uid"]."', $margin_pembayaran)"
        );
        
        //$tr->addSQL("update c_visit_operasi set id_dokter=$dokter where no_reg = lpad('".$_POST["rg"]."',10,'0')");
    }
...
$SQL2 = "insert into history_user " .
            ...
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Pelayanan Operasi','Pelayanan -> Pelayanan Operasi','Menambah Pelayanan NonPaket No.MR $mr No.REG $rg Total ".$v["total"]."','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
pg_query($con, $SQL2);
//======================
...
$SQL2 = "insert into history_user " .
            "(id_history, tanggal_entry,waktu_entry,trans_form,item_id,keterangan,user_id,nama_user) ".
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Pelayanan Operasi','Pelayanan -> Pelayanan Operasi','Menambah Pelayanan Paket No.MR $mr No.REG $rg Total ".$v["total"]."','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
pg_query($con, $SQL2);
//======================
    }
	// Insert BHP
...
$SQL2 = "insert into history_user " .
            "(id_history, tanggal_entry,waktu_entry,trans_form,item_id,keterangan,user_id,nama_user) ".
            "values".
            "(nextval('rshistory_user_seq'),CURRENT_DATE,CURRENT_TIME,'Ruang Operasi','Pelayanan -> Layanan Operasi','Menambah BHP No.MR $mr No.REG $rg Total ".$v["total"]."','".$_SESSION["uid"]."','".$_SESSION["nama_usr"]."')";
pg_query($con, $SQL2);
//======================
    }  
...
//----tambahan for update.... hery------------------------
	if ($_POST['act'] == "edit") { 
		...
		//if ($_POST['row'] > 0)	{
			...
			$SQL2 = "select * from c_visit_operasi where no_reg='$noreg' ".
...
//--------------------------------------------------------	
	...
	if ($_POST['act'] == "new2") { 	
			//pg_query("update c_visit_operasi set id_konsul='".$_POST["konsultasi"]."' where no_reg =lpad('$noreg',10,'0') and id_poli ='".$_POST["f_id_poli"]."'");
			/*pg_query("update c_visit_operasi set id_konsul='".$_POST["konsultasi"]."' where no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."'");
			header("Location: ../index2.php?p=$PID&list={$_POST["list"]}&rg=$noreg&poli=".$_POST["f_id_poli"]."&mr=".$_POST["mr"]);//end of edit line
		   	exit;*/
			$r = pg_query($con,"SELECT * FROM c_visit_operasi WHERE no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."'");
		    	$d = pg_fetch_object($r);
			$knsl=getFromTable("select id_konsul from c_visit_operasi where no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."'");
			$knsl2=getFromTable("select id_konsul from c_visit_operasi where no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."' and id_konsul='".$_POST["konsultasi"]."'");
			if ($knsl==""){
			pg_query("update c_visit_operasi set id_konsul='".$_POST["konsultasi"]."' where no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."'");
			pg_query("update c_visit_operasi set tanggal_konsul=CURRENT_DATE where no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."'");
			pg_query("update c_visit_operasi set waktu_konsul=CURRENT_TIME where no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."'");
			}
			if ($knsl2=="" and $knsl!=""){
			$SQL = "select * from c_visit_operasi where no_reg ='$noreg' and id_poli ='".$_POST["f_id_poli"]."' ";
			$r2 = pg_query($con,$SQL);
			$d2 = pg_fetch_object($r2);
			pg_free_result($r2);
			$tm1=date("Y-m-d H:i:s");
			$SQLa="insert into c_visit_operasi (no_reg,tanggal_reg,id_poli,id_konsul,id_dokter,tanggal_konsul,waktu_konsul) 
			values ('$noreg', '$d2->tanggal_reg', '".$_POST['f_id_poli']."', '".$_POST["konsultasi"]."','$d2->id_dokter',CURRENT_DATE,CURRENT_TIME)";
			pg_query($con, $SQLa);
...
if ($tr->execute()) {
    ...
    unset($_SESSION["icd"]);
    if($_POST['sub'] != "retur"){  
    header("Location: ../index2.php?p=$PID&list={$_POST["list"]}&rg=".$_POST[rg]."&poli=209&mr=".$_GET["mr"]."$add&sub=".$_POST[sub]);
    exit;
    }else{
    	header("Location: ../index2.php?p=$PID&list={$_POST["list"]}&rg=".$_POST[rg]."&poli=209&mr=".$_GET["mr"]."$add&sub=".$_POST[sub]);
    exit;
