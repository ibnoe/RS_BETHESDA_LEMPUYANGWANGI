...
subtitle_rs("FORMULIR RL1 - KEGIATAN KESEHATAN GIGI & MULUT");
subtitle_rs($set_header[0]." ".$set_header[1]);
subtitle_rs("No. Kode RS : ".$set_kode_rs);   
		
	if(!$GLOBALS['print']){
	 	title_print("");
		title_excel("kes_gigi_mulut");
       edit_laporan("input_kes_gigi_mulut");
	}else {
			
		
	}
	
$SQL = "select * from rl100016 order by id";
...
<table align="center" CLASS=TBL_BORDER WIDTH='50%' BORDER=1 CELLSPACING=1 CELLPADDING=2>
 <?	
  ...
  <tr>
    ...
    <td class="TBL_BODY" align="center"><?=$row1["satuan"] ?></td>
  </tr>
  <?;$j++;					
				}
				$i++;
